...
test.describe('TB v3 Walkthrough Phase 8', () => {
  ...
  // v6.5.8 — Phase 8b Topology fundamentals: 6 new walkthroughs (star/mesh/3-tier/
  // ring/p2p/spine-leaf). Spot-check one (star) end-to-end; the UAT v6.5.8
  // structural + data-integrity guards cover the other 5 statically.
  test('TB v3 WALK v6.5.8 — fundamentals: start star-topology-shape, walk all 6 steps, completion writes PROGRESS', async ({ page }) => {
    await page.goto('/?cert=netplus');
    await page.waitForFunction(() => typeof window.showPage === 'function');
    await page.evaluate(async () => {
      window._gateProOnly = () => true;
    ...
    await page.evaluate(() => {
      var tb3 = window._certanvilFeatures['topology-builder-v3'];
      if (tb3 && typeof tb3.walkStart === 'function') tb3.walkStart('star-topology-shape');
      else if (typeof window.walkStart === 'function') window.walkStart('star-topology-shape');
    });

    await page.waitForSelector('.tb3-walk-card', { timeout: 5000 });
    await expect(page.locator('.tb3-walk-card-pos')).toContainText('1 / 6');
    await expect(page.locator('.tb3-walk-card-title')).toContainText('Everything points to the middle');
  ...
  // v6.5.12 — Phase 8f SMB/Office: 3 new walkthroughs. Spot-check
  // enterprise-ids-lb-defence (6 steps, defence-in-depth — densest).
  test('TB v3 WALK v6.5.12 — SMB: start enterprise-ids-lb-defence, walk all 6 steps, completion writes PROGRESS', async ({ page }) => {
    await page.goto('/?cert=netplus');
    await page.waitForFunction(() => typeof window.showPage === 'function');
    await page.evaluate(async () => {
      window._gateProOnly = () => true;
    ...
    await page.evaluate(() => {
      var tb3 = window._certanvilFeatures['topology-builder-v3'];
      if (tb3 && typeof tb3.walkStart === 'function') tb3.walkStart('enterprise-ids-lb-defence');
      else if (typeof window.walkStart === 'function') window.walkStart('enterprise-ids-lb-defence');
    });
    ...
    expect(progress['enterprise-ids-lb-defence']).toBeTruthy();
    expect(progress['enterprise-ids-lb-defence'].completedAt).toBeTruthy();
  });

  // v6.5.13 — Phase 8g Wireless: spot-check wireless-ess-roaming (densest, 6 steps).
  test('TB v3 WALK v6.5.13 — Wireless: start wireless-ess-roaming, walk all 6 steps, completion writes PROGRESS', async ({ page }) => {
    await page.goto('/?cert=netplus');
    await page.waitForFunction(() => typeof window.showPage === 'function');
    await page.evaluate(async () => {
      window._gateProOnly = () => true;
    ...
    await page.evaluate(() => {
      var tb3 = window._certanvilFeatures['topology-builder-v3'];
      if (tb3 && typeof tb3.walkStart === 'function') tb3.walkStart('wireless-ess-roaming');
      else if (typeof window.walkStart === 'function') window.walkStart('wireless-ess-roaming');
    });
