...
// ══════════════════════════════════════════
// v4.99.36 (MOBILE_OPTIMIZATION_PLAN Phase 11b) — Network Analysis Drill
// EXTRACTED to features/network-analysis.js. Loaded lazily on first navigation
// to #page-network-analysis. Saves ~1,053 LOC (~50 KB transfer) from the shell
// on first paint for users who never open the drill.
//
// The original code lived here (lines 11418-12493 in v4.99.35) — full content
// preserved verbatim inside features/network-analysis.js IIFE wrapper. See
// PHASE_11B_CUT_POINTS.md for the extraction contract + lazy-load pattern.
// ══════════════════════════════════════════
async function startNetworkAnalysisDrill() {
  if (typeof _gateActivityForQuota === "function" && !_gateActivityForQuota("Network Analysis Drill")) return;
  try {
    var mod = await _loadFeature("network-analysis");
    return mod.enter();
  } catch (err) {
    if (typeof showErrorToast === "function") showErrorToast("Network Analysis Drill failed to load. Please refresh and try again.");
    if (typeof logError === "function") logError("feature-load:network-analysis", err);
  }
}
...
// to #page-subnet. Original ~896 LOC at lines 27955-28850 (pre-extract).
// Saves ~50 KB transfer from the shell on first paint.
//
// CRITICAL invariant: index.html static onclicks at lines 1307-1309 call
// setSubnetTab(...) directly. The feature module exposes setSubnetTab on
// window after lazy-load completes; by the time the user can click those
// tab buttons, they have already triggered startSubnetTrainer via sidebar.
// ══════════════════════════════════════════
async function startSubnetTrainer() {
  if (typeof _gateProOnly === "function" && !_gateProOnly("Subnet Trainer")) return;
  try {
    var mod = await _loadFeature("subnet-trainer");
    return mod.enter();
  } catch (err) {
    if (typeof showErrorToast === "function") showErrorToast("Subnet Trainer failed to load. Please refresh and try again.");
    if (typeof logError === "function") logError("feature-load:subnet-trainer", err);
  }
}

// ══════════════════════════════════════════
// v4.99.38 (MOBILE_OPTIMIZATION_PLAN Phase 11b session 3) — Port Drill
// EXTRACTED to features/port-drill.js. Loaded lazily on first navigation
// to #page-ports. Original ~1,267 LOC at lines 28666-29932 (pre-extract).
// Saves ~50 KB transfer from the shell on first paint.
//
// Note: pt* prefix here means "port timer", NOT "packet trace". Packet
// Trace uses ptr* and lives separately, still in shell pending Phase 11b
// session 4 extraction (Wednesday).
// ══════════════════════════════════════════
async function startPortDrill(tab) {
  if (typeof _gateProOnly === "function" && !_gateProOnly("Port Drill")) return;
  try {
    var mod = await _loadFeature("port-drill");
    return mod.enter(tab);
  } catch (err) {
    if (typeof showErrorToast === "function") showErrorToast("Port Drill failed to load. Please refresh and try again.");
    if (typeof logError === "function") logError("feature-load:port-drill", err);
  }
}
...
const APP_SIDEBAR_DRILLS_SECPLUS = [
  { page: 'acronyms', label: 'Acronym Blitz', handler: () => { showPage('acronyms'); if (typeof startAcronymBlitz === 'function') startAcronymBlitz(); } }
  // MVP-QUIZ-ONLY (Ship 1): IRW + PHT + AMM + CTS entries all deleted.
  // Only Acronym Blitz remains in the Sec+ drills sidebar for the MVP.
];
...
const TOPBAR_CRUMBS = {
  ...
  'ports': 'Port Drill',
  'acronyms': 'Acronym Blitz',
  // MVP-QUIZ-ONLY (Ship 1): 'amm'/'cts'/'irw'/'pht' crumb labels removed.
