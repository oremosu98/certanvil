...
window.CERT_PACKS.secplus = {
  ...
  questionExemplars: [
    ...
    {"type":"mcq","question":"After an audit, a logistics firm disables eleven unused network services and closes their ports. Six months later an attacker still breaches the firm through a phishing email to a dispatcher. Which pairing correctly labels the two facts?","difficulty":"Foundational","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"The service closures reduced the threat vector; the email was the attack surface","B":"The service closures reduced the attack surface; the email was the threat vector used","C":"Both describe threat vectors","D":"Both describe the attack surface"},"answer":"B","explanation":"Attack surface is the whole set of points an attacker could try — every service, port, mailbox and person. A threat vector is the specific route actually used. Closing unused services removed entry points that were never used in the end; the breach travelled through one route that remained: a message to a human. Reducing the surface narrows the options; it does not eliminate the vectors that stay open. (A) Wrong — reversed. (C) Wrong — the closures are not routes anyone used; they are a shrinking of what could be tried. (D) Wrong — the email names the route in, which is precisely what vector means. Memory: surface = every door and window; vector = the unlocked window they climbed through.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A stock-photo marketplace lets users upload graphics. An attacker uploads an SVG file with script embedded in its markup; when a moderator reviews the file, her outdated browser executes the script. Which threat vector class does this attack use?","difficulty":"Exam Level","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"File-based, because an upload is involved","B":"Message-based","C":"Removable device","D":"Image-based"},"answer":"D","explanation":"SVG is the reason image-based exists as its own SY0-701 category: unlike a flat bitmap, it is XML — structured markup that can carry links, references and script — and an application that processes it insecurely can be made to execute what it carries. The lure IS the image. (A) Wrong — the tempting near-miss. Upload was the transport, but the category the objectives name for malicious content smuggled inside image formats is image-based; file-based is the better label when the object is a document, archive or executable. (B) Wrong — no message was sent to the victim; she opened the file in the course of her job. (C) Wrong — nothing physical was connected. Exam clue: script inside an image format that executes on processing = image-based.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A junior analyst runs a port scan against the company's public web server and reports TCP 443 open as a security finding requiring remediation. Which statement BEST describes the situation?","difficulty":"Exam Level","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"The analyst is right: any open port is an exploitable vulnerability","B":"The analyst is wrong: open ports carry no security relevance on a hardened server","C":"An open port is exposure, not a vulnerability — 443 is this server's job; the risk lives in unneeded ports and in vulnerable or misconfigured services behind open ones","D":"The port should be moved to a non-standard number to resolve the finding"},"answer":"C","explanation":"A web server exists to answer on 443; the open port is the service working as designed. What deserves the analyst's attention is different: ports open with no business need, and the state of the software listening — an exposed service that is vulnerable or misconfigured turns ordinary exposure into an attack path. Each listening service adds to the attack surface, which is why unnecessary ones get closed. (A) Wrong — collapses exposure into vulnerability; if it were true, every functioning server would be a finding. (B) Wrong — over-corrects: open ports are exactly where attack surface accrues, so they always merit inventory. (D) Wrong — moving a port relocates the exposure without changing the service behind it. Memory: open port = a doorway, not a break-in; count the doorways, harden what answers the door.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"An architecture practice enables RDP on an internet-facing server so staff can reach CAD software from home, though a VPN was available. A vulnerability in the RDP service is later exploited from the internet. What most directly INCREASED the attack surface here?","difficulty":"Foundational","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"Exposing an unneeded service port (RDP) directly to the internet","B":"The staff working from home","C":"The CAD software's licensing model","D":"Using TCP instead of UDP for remote access"},"answer":"A","explanation":"Attack surface grows with every service reachable by an attacker, and internet-facing RDP is the textbook unnecessary exposure — remote access was already solvable through the VPN, which would have kept 3389 off the public internet entirely. The exploit that followed travelled through exposure that did not need to exist. (B) Wrong — remote work created the requirement, not the exposure; the choice of HOW to meet it did that. (C) Wrong — licensing is commercially interesting and security-irrelevant here. (D) Wrong — the transport protocol does not decide what is exposed. Exam clue: could the service have stayed unreachable from the internet? Then exposing it is what grew the surface.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A company runs two tools: a time-tracking agent installed on every laptop, and a payroll portal staff reach through a browser with nothing installed locally. A researcher reports a vulnerability in each. Which pairing correctly labels the two software exposure models?","difficulty":"Exam Level","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"Time tracker: agentless. Payroll portal: client-based","B":"Both are client-based, because both are software","C":"Both are agentless, because both are business applications","D":"Time tracker: client-based. Payroll portal: agentless"},"answer":"D","explanation":"SY0-701 splits vulnerable software by where it runs. Client-based means code installed on the endpoint — the time-tracking agent — so a flaw in it puts every laptop carrying it at risk, and fixing it means patching every install. Agentless means the user reaches a central service with only a browser — the payroll portal — so the vulnerable code lives server-side, and one compromise of the central application can affect every connecting user at once. Same word, software, two different blast patterns. (A) Wrong — reversed. (B) Wrong — erases the distinction the objective names; the portal installs nothing locally. (C) Wrong — same error from the other side; the agent is on every machine. Memory: client-based = installed on YOUR box; agentless = reached through the browser.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A hotel's ground-floor meeting room, bookable by the public, has live Ethernet wall jacks that connect directly to the hotel's internal corporate LAN. Which vector does this create, and which control MOST directly closes it?","difficulty":"Exam Level","topic":"Attack Vectors & Surfaces","objective":"2.2","options":{"A":"Unsecure wired network; 802.1X port-based network access control","B":"Unsecure wireless network; WPA3","C":"Open service port; a host firewall on each server","D":"Removable device; USB port lockdown"},"answer":"A","explanation":"Wired does not mean safe — it means the attacker needs to reach a socket, and a publicly bookable room hands them one. A live jack straight onto the corporate LAN is the unsecure wired network vector. 802.1X closes it by authenticating the connecting device or user BEFORE the switch port forwards traffic, so an unknown laptop in the meeting room gets no LAN at all. (B) Wrong — no radio is involved, and WPA3 protects Wi-Fi associations, not wall jacks. (C) Wrong — host firewalls narrow what a connected attacker can reach; the vector is being connected at all. (D) Wrong — nothing is plugged into a USB port; the plug goes into the wall. Exam clue: exposed Ethernet + who-can-plug-in = unsecure wired; the fix that names itself is 802.1X.","source":"curated-lesson-threat-vectors","addedVersion":"8.16.0","addedDate":"2026-08-22"},
  ],
