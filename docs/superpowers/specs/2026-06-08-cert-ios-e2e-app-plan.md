...
## 11. Build log & as-built state (updated 2026-06-09)

> §1–§10 above are the original design intent. This section records what was actually
> built and the decisions taken during execution. **Phases 0–4 are done and verified
> in-browser; Phase 5 (fidelity harness) and Phase 6 (Capacitor) are not started.**

### 11a. As-built architecture
- **Shell:** `mockups/cert-ios-app.html` (390×844 canvas, scaled to fit, iOS push/pop
  transitions) + floating **dev HUD** (`‹ Back · Next › · ◈ Pro · ☾/☀ Theme · ↺ Reset · jump-to`).
- **Engine:** `mockups/e2e/shell.js`. Key objects/functions:
  - `SCREENS` — id→file registry (**36 screens** registered).
  - `NAV` — per-screen route map (`{sel|text, to}`; `to` = screen id / `'pop'` / `'next'`).
  - `bindNav(iframe,id)` — binds in-mockup buttons to routes; plus **screen-specific blocks**
    for `pro-iap`, `custom-quiz`, `pro-expired`, `quiz` (daily-15 pre-empt), `free-home-day0`,
    `first-run-diag`.
  - `applyProState(doc,id)` — reflects Free/Pro into `hub`, `home`, `custom-quiz`, **`settings`**.
  - `setDemoPro`/`isDemoPro`; theme (`setTheme`/`ca-theme`); `resetTo`; HUD wiring.
  - **D8 fidelity:** loads each mockup in an iframe, renders only inner `.screen` full-bleed
    (strips `.stage`/`.phone` bezel via injected `STRIP_CSS`).
- **localStorage keys (all demo-only, cleared by HUD ↺ Reset):**
  `e2e_demo_pro` (Pro bypass) · `ca-theme` (theme) · `ca-exam-result` (logged exam → hub Passed) ·
  `e2e_overcap` (custom-quiz → daily-limit count) · `e2e_daily15` (today's-15 session flag) ·
  `e2e_daily_goal` / `e2e_review_size` / `e2e_topup` (persisted Pro Settings choices).

### 11b. Final screen map (36 screens) — added beyond the original plan
The original plan assumed pure stitching of existing mockups. During build these **new
screens** were created (4-pass pipeline) and wired in:
- `cert-ios-log-result` — Pro logs a real exam result → hub flips active cert to **Passed**.
- `cert-ios-review` — spaced-repetition session (recap + what-to-revisit + exam-aware next).
- `cert-ios-review-answers` — post-session answer breakdown (All / Wrong filter).
- `cert-ios-daily-limit` — free over-cap blocker (custom-quiz > daily cap → here).
- `cert-ios-pro-expired` — lapsed-subscription re-entry (Pro ended → Free).
- `onboarding-plan-picker-concept` — rebuilt onto the cert-ios shell (was a visual outlier).

### 11c. Session additions — 2026-06-09
1. **Report screen — screenshot attach.** `cert-ios-report.html`: an "Add screenshot" control
   (real `<input type=file accept=image/* capture>`, opens camera/library on device),
   thumbnail previews with remove, capped at 3, theme-aware.
2. **Day-0 home — both paths wired** (`onboarding-free-home-day0` + `shell.js`):
   - "Take the 30-second check" → `first-run-diag` (was orphaned/unreachable). The mockup's
     designer tab-toggle is hidden in-app; "Next question" walks a 5-Q check → readiness
     reveal → **Start practising → home** (score placed); "Skip the check" → home.
     Copy fix: "Question 3 of 20" → "Question 1 of 5".
   - "Start today's 15" (was a dead button) → `quiz` flagged as the daily session →
     finishing it lands on **`free-capped-home`** (done-for-today). Normal quiz→results
     path unaffected.
3. **Settings — tier-locking** (`cert-ios-settings.html` + `applyProState`):
   - **Free:** Daily goal locked at **15/day**, Daily Review capped at **5/day**, no top-up —
     each card shows a `Pro` pill + lock note + "Go Pro" upsell → `upgrade-sheet`.
   - **Pro:** live controls (goal 10/20/30/50, review 10/20/30/All-due, top-up toggle).
   - Driven by `.plan-free`/`.plan-pro` on `.screen` via `window.setPlan(isPro)`; HUD Pro
     toggle flips it live.
4. **Pro Settings persistence.** Daily goal / review size / top-up persist to localStorage
   and rehydrate on load; chosen review size reflects on the home "Review N cards" tile
   (defaults to "All due"). Pro picker default review size changed 30 → **All due**.

### 11d. Decisions locked during build
- **Drills = out of MVP** (Drill/domain CTAs route to a practice quiz stand-in; real drills later).
- **SR exam-date→cadence algorithm = backend/technical-integration** (reflected in copy only:
  "reviews tighten as exam nears").
- **4-pass pipeline for any NEW screen:** `/design-taste-frontend` → `/emil-design-eng` →
  `/humanizer` → `/marketing-psychology`.
- **Payments parked:** Stripe (desktop, near launch); iOS StoreKit later — paywall UI only now.
...
- `npm run ios:www` assembles `capacitor-www/` from `mockups/` with the shell copied to
  `index.html` (Capacitor loads index.html; shell assets are sibling-relative so the copy
...
- Demo HUD ships in this dummy build; strip `#hud` before any real distribution.

### 11g. Remaining / next steps
