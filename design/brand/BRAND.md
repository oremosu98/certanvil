...
>
> **Shimmer text is the one named exception — source it from `antalik-effects`, not `transitions-dev`.** The two are the *same* implementation (identical `.t-shimmer` class, identical `--shimmer-*` variables, same `attr(data-text)` + `background-clip:text` + 400% band + 2s sweep) — same author, duplicated upstream. `antalik-effects` ships a `html[data-theme="light"]` override block that drops straight into this file's theme system, carries the a11y + lifecycle discipline (aria-label = the state name — "Solving…", never "Loading…"; IntersectionObserver pause offscreen; `visibilitychange` pause on hidden tabs; reduced-motion → one static frame), and is eval-validated at 100% vs a 48% baseline. Switching cost is zero — the class and variable names match. Its `references/primitives.md` §10 recipe stands alone, so it covers plain status text as well as the orb-plus-label pair. This is the **only** overlap between the two skills; the other 26 transitions are unaffected.

> **⚠ The `prototype` picker's easing curve is EXEMPT from this section — never "correct" it.** The `prototype` skill (Emil Kowalski, MIT, added 2026-07-27) builds design variants behind a floating picker whose spec hardcodes `cubic-bezier(0.23, 1, 0.32, 1)`. That is the *exact* off-brand curve removed from the landing page on 2026-07-27 (audit finding **L3**), so a motion audit, a curve grep, or a `transitions-polish` run **will** flag it. Leave it alone. The picker is deliberately harness chrome — never theme-aware, never restyled with project tokens — so it always reads as scaffolding rather than as part of the design being judged. The exemption covers the `.proto-picker*` rules **only**; the variants inside the picker are fully subject to everything below. The skill's own "sub-300ms" duration rule is likewise outranked by the duration table below, exactly as `transitions-dev`'s scale is. Full rationale: `~/.claude/skills/prototype/SOURCE.md`.
