...
> **Older releases** (v3.0 through v4.99.19 — 283 rows) live in [CHANGELOG.md](./CHANGELOG.md). Inline here: just the **last 3 ships**. Most recent trim during the **v4.99.23 night-end consolidation** (2026-05-09) — 42 rows moved to CHANGELOG (v4.93.0 → v4.99.19) alongside tonight's 16-ship sprint that took the cert-app from BYOK API model to fully launch-ready playtest infrastructure (deep E2E sweep punch list closed, notify-me Supabase persistence, SHIP_CHECKLIST + /review-feature workflow, playtest auth with 5 dummy accounts, display-name greeting + Sec+ coming-soon tile). Architecture + lessons in [memory](~/.claude/projects/-Users-simioremosu-Desktop/memory/).
